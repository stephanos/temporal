...
SHOULD cite these rule IDs when refining or applying them. Rule IDs are permanent: add new rules
with new IDs, and reserve the ID of a retired rule rather than reusing or renumbering it.

The terms MUST, MUST NOT, SHOULD, and MAY are normative. A human MUST approve any change that
violates a rule or deliberately plans to violate one.
...
- **SEM-01 — Lean authority.** Lean MUST be the sole authority for behavioral meaning.
...
- **MOD-05 — Deep modules.** Umpire modules SHOULD hide substantial checking, planning, artifact,
  observation, and verification machinery behind small, cohesive interfaces.
- **MOD-06 — Component seams.** Components MUST have narrow responsibilities and communicate through
