...
- The optional choice trace is versioned, bounded, checksummed, and retained in artifacts. Its implementation records three kinds: runnable selection, `select` poll permutation, and the final `select` result ([protocol](../tools/gomad3/choice/schema/choicewire.json), [runtime hooks](../tools/gomad3/toolchain/runtime/go1.26.4.patch), [validator and projection](../tools/gomad3/choice/trace.go)).
