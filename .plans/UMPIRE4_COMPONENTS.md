...
| C4 ExperimentSpec compiler | Partial | Pure planning retains canonical `umpire-experiment/v2` artifacts for one selected trace or an atomic bounded Space batch. The Artifact boundary can add checked execution references and emit complete `umpire-experiment/v3` test artifacts through `umpire-gen-tests`; persisted decoding/validation, runtime configuration, and runtime consumption remain missing. |
...
| C9 SDK participant | Separate | Umpire3 has participant protocols and a Temporal SDK adapter. The current generator emits checked participant-program references, but the concrete program and runtime binding remain downstream. |
...
`umpire-experiment/v3`. Veil belongs at a different seam inside C11: an optional, family-owned
...
| ExperimentSpec | Environment-independent executable specification with a drive plan, properties, requirements, Limits, Known Gaps, provenance, and Behavior Fingerprints | `umpire-experiment/v2` remains the byte-identical planning artifact. `umpire-gen-tests` adds checked participant, setup, ordering, termination, and cleanup references and emits executable `umpire-experiment/v3`; runtime reading and migrations remain explicit Known Gaps. |
...
`umpire-gen-tests` then adds checked participant-program, setup, ordering, termination, and cleanup
references and seals executable v3 artifacts. Operational configuration remains a separate runtime
contract. Artifact reading, migrations, and runtime consumption are not implemented.
