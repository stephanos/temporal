...
Continue [COMPAT-3](GOMADv3_NEXT_COMPATIBILITY.md#compat-3-tiered-temporal-corpus) by using the sixteen-workload Temporal tier-2 corpus to rank exact compatibility packs and adapters by workloads unlocked. [COMPAT-4](GOMADv3_NEXT_COMPATIBILITY.md#compat-4-compatibility-pack-development-kit) is implemented with a v2-only exact-source contract, exact-digest approval, generated changed-version/source rejection tests, and independent qualification. [COMPAT-6](GOMADv3_NEXT_COMPATIBILITY.md#compat-6-safer-handling-of-transitive-forbidden-dependencies) now has an experimental compiler/linker-backed mode, but the evaluated Temporal candidates retain real assembly, linkname, `syscall`, or forbidden-import blockers and support remains 4/16; closure review remains the default. Add only [COMPAT-5](GOMADv3_NEXT_COMPATIBILITY.md#compat-5-targeted-deterministic-adapters-and-io-models) adapters or I/O models justified by this evidence. The immediate compatibility goal is still to increase actual support beyond 4/16 without generic exemptions or host fallback, then add composed tier-3 scenarios.
