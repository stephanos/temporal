# Temporal controller Driver

`New(Options)` freezes the public Profile and configured transport endpoints, creates lazy shared gRPC
channels, and performs no target calls. Supply its `Snapshot` through `testpilot.Prepare`. The root
...
The configured Profile identity must change when authorization or ceilings change. Prepared/Driver
identity separately includes the complete immutable Profile binding fingerprint.
Symbolic endpoint and resource binding IDs never carry network addresses. Addresses, gRPC
transport/per-call credentials and injected metadata remain Driver configuration;
...
Driver, including quarantined effects. Every accepted effect retains that shared capacity until the
actual transport call returns. Wait/Cancel/Drain and all serialized Driver operations honor their contexts, including cancellation
while waiting for the Driver lock. Close cancels effects, destroys
...
and retains existing ownership rather than allocating more capacity. Late completion releases Driver
capacity and can only be read through an independently copied effect result; the Driver owns no Run
recording or Verdict. The internal Executor owns draining, cleanup, closure ordering and late diagnostics.

`Reserve` rejects worker reservations. The composite Driver joins this Session with the worker Session;
...
The internal `delivery` ledger is the handoff between the worker Session and the composite Driver.
...
workflow-source/handler route without changing the complete `testpilot.Value`; first Nexus admission
...
`PayloadSerializer`. The result is the complete `testpilot.Value` protobuf, encoded as a Temporal Payload
with `encoding=binary/protobuf` and `messageType=temporal.server.api.testpilot.v1.Value`. This produces
standard Nexus protobuf content and round-trips through Temporal's default SDK data converter into
`*testpilotpb.Value`. Worker start/await/finish/result paths must preserve that same generic value
