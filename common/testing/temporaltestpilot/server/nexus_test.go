...
func nexusSession(t *testing.T, h *Driver, source *testpilotspb.Case, run string) (*Session, testpilot.Coordinate) {
	t.Helper()
	program := proto.CloneOf(source.Program)
	node := rpcNode("complete", "")
	node.Instruction = &testpilotspb.Instruction{Instruction: &testpilotspb.Instruction_CompleteNexusOperation{CompleteNexusOperation: &testpilotspb.CompleteNexusOperation{CapabilitySlotId: "capability"}}}
	program.Entrypoints[0].Instructions = append(program.Entrypoints[0].Instructions, node)
	program.Entrypoints = append(program.Entrypoints, &testpilotspb.EntrypointDefinition{EntrypointId: "handler", Activation: &testpilotspb.EntrypointDefinition_NexusHandler{NexusHandler: &testpilotspb.NexusHandlerActivation{}}})
	for _, id := range []string{"capability", "other"} {
		program.Slots = append(program.Slots, &testpilotspb.SlotDefinition{SlotId: id, Content: &testpilotspb.SlotDefinition_OpaqueCapability{OpaqueCapability: &testpilotspb.OpaqueCapabilityType{}}})
	}
...
func completionValue() *testpilotspb.Value {
...
func TestCapabilityClosureAndLimits(t *testing.T) {
	...
	require.NoError(t, s.Diagnose(t.Context(), "run", &testpilotspb.RunDiagnostic{}))
