...
func TestEvaluatorFailureViolationOrderingAndReplay(t *testing.T) {
	for _, tc := range []struct {
		...
		{"failure before violation", 1, 2, testpilotspb.VERDICT_STATUS_INCONCLUSIVE, 0},
		{"failure on violating event", 1, 1, testpilotspb.VERDICT_STATUS_INCONCLUSIVE, 0},
		{"violation before failure", 2, 1, testpilotspb.VERDICT_STATUS_VIOLATED, 2},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.stop == 0 {
				...
				require.Equal(t, testpilotspb.RULE_VERDICT_STATUS_VIOLATED, verdict.Rules[0].Status)
			}
		})
	}
}
