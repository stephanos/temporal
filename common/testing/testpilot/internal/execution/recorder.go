...
type recorder struct {
	...
	failure                         error
	seal                            func()
	diagnose                        func(context.Context, string, *testpilotspb.RunDiagnostic) error
...
func (r *recorder) publish(ctx context.Context, facts []*testpilotspb.RunEvent, commit func() error) (Decision, error) {
	return r.publishWithMonitorPolicy(ctx, facts, commit, true)
}

func (r *recorder) publishCleanup(ctx context.Context, facts []*testpilotspb.RunEvent, commit func() error) (Decision, error) {
	return r.publishWithMonitorPolicy(ctx, facts, commit, false)
}

func (r *recorder) publishWithMonitorPolicy(ctx context.Context, facts []*testpilotspb.RunEvent, commit func() error, monitorFailureFatal bool) (Decision, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.closed {
	...
	if commit != nil {
		if err := commit(); err != nil {
			return Stop, r.publicationFailure(monitorFailureFatal, testpilotspb.RUN_DIAGNOSTIC_KIND_INVARIANT, "store_commit_failed", err)
		}
	}
...
func (r *recorder) publicationFailure(fatal bool, kind testpilotspb.RunDiagnosticKind, code string, err error) error {
	if fatal {
...
func (r *recorder) append(ctx context.Context, event *testpilotspb.RunEvent, monitorFailureFatal bool) error {
	...
	if err != nil {
		...
		return r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_MONITOR, "observe_failed", err)
	}
...
func (r *recorder) admit(ctx context.Context, operation func(context.Context) ([]contract.EffectHandle, error), retain func([]contract.EffectHandle)) error {
	...
	if err := ctx.Err(); err != nil {
		return r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_cancelled", err)
	}
	handles, err := operation(ctx)
	retain(handles)
	if err != nil {
		return r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_failed", err)
	}
	if err := ctx.Err(); err != nil {
		return r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_cancelled", err)
	}
...
func (r *recorder) fail(kind testpilotspb.RunDiagnosticKind, code string, err error) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.closed {
		return invalid(ir.Unavailable, "recorder", "Run closed")
...
func (r *recorder) failLocked(kind testpilotspb.RunDiagnosticKind, code string, err error) error {
	r.incomplete = true
	r.signalStop()
	if r.failure == nil {
		if err == nil {
...
func (r *recorder) diagnostic(kind testpilotspb.RunDiagnosticKind, code, detail string) {
...
func (r *recorder) report(kind testpilotspb.RunDiagnosticKind, code string, err error) string {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *recorder) terminalStatus(executionErr error) testpilotspb.RunDisposition {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.stopped {
...
func (r *recorder) close(ctx context.Context, disposition testpilotspb.RunDisposition, cleanup *testpilotspb.CleanupOutcome) (*testpilotspb.Run, *testpilotspb.Verdict, error) {
	...
	if cleanup != nil {
		if err := ir.CheckSurface(cleanup, ir.Limits{Depth: 8, Bytes: 8192, Work: 8192, Fanout: int64(r.diagnosticLimit)}); err != nil {
			closeErr = err
			closeErr = errors.Join(closeErr, r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_RECORDER, "cleanup_recording_failed", err))
		} else {
			r.run.Cleanup = proto.CloneOf(cleanup)
		}
	}
	if int64(len(r.run.Events)) < r.maxEvents {
		if err := r.append(ctx, &testpilotspb.RunEvent{Kind: testpilotspb.RUN_EVENT_KIND_RUN_CLOSED, SourceId: "@recorder/closed", ExecutionIncomplete: r.incomplete}, true); err != nil {
	...
	if err != nil {
		closeErr = errors.Join(closeErr, err, r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_MONITOR, "close_failed", err))
	}
	if verdict == nil {
...
func (r *recorder) completionFailure(ctx context.Context, code string, err error) error {
	...
	return r.failLocked(testpilotspb.RUN_DIAGNOSTIC_KIND_EXECUTION, code, err)
}
