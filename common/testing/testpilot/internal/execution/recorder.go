...
type recorder struct {
	...
	failure                         error
	seal                            func()
	diagnose                        func(context.Context, string, *testpilotpb.RunDiagnostic) error
	diagnosticLimit, postCloseCount int
}
type recordedSource struct {
	event              *testpilotpb.RunEvent
	producerIncomplete bool
}

func newRecorder(view ProgramView, runID, caseID string, monitor Monitor, now func() time.Time, seal func(), diagnose func(context.Context, string, *testpilotpb.RunDiagnostic) error) (*recorder, error) {
...
func (r *recorder) publishWithMonitorPolicy(ctx context.Context, facts []*testpilotpb.RunEvent, commit func() error, monitorFailureFatal bool) (Decision, error) {
	...
	if isNil(ctx) {
		return Stop, r.publicationFailure(monitorFailureFatal, testpilotpb.RUN_DIAGNOSTIC_KIND_INVARIANT, "context_required", errors.New("context required"))
	}
	if err := ctx.Err(); err != nil {
		return Stop, r.publicationFailure(monitorFailureFatal, testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, "publication_cancelled", err)
	}
	staged, err := r.stage(facts)
	if err != nil {
		return Stop, r.publicationFailure(monitorFailureFatal, testpilotpb.RUN_DIAGNOSTIC_KIND_RECORDER, "recording_failed", err)
	}
	if len(staged) == 0 {
	...
	if commit != nil {
		if err := commit(); err != nil {
			return Stop, r.publicationFailure(monitorFailureFatal, testpilotpb.RUN_DIAGNOSTIC_KIND_INVARIANT, "store_commit_failed", err)
		}
	}
...
func (r *recorder) publicationFailure(fatal bool, kind testpilotpb.RunDiagnosticKind, code string, err error) error {
	if fatal {
...
func (r *recorder) stage(facts []*testpilotpb.RunEvent) ([]*testpilotpb.RunEvent, error) {
	if int64(len(facts)) > r.maxEvents {
		return nil, invalid(ir.LimitExceeded, "recorder", "batch event ceiling exceeded")
	}
	staged := make([]*testpilotpb.RunEvent, 0, len(facts))
	batch := make(map[string]*testpilotpb.RunEvent, len(facts))
	for _, fact := range facts {
		if fact == nil || !validID(fact.SourceId) || fact.Kind < testpilotpb.RUN_EVENT_KIND_RUN_OPENED || fact.Kind > testpilotpb.RUN_EVENT_KIND_DIAGNOSTIC || fact.Kind == testpilotpb.RUN_EVENT_KIND_RUN_CLOSED {
		...
		if previous, exists := r.sources[snapshot.SourceId]; exists {
			semantic := testpilotpb.RunEvent{Kind: previous.event.Kind, Coordinates: previous.event.Coordinates, SourceId: previous.event.SourceId, CausalSourceIds: previous.event.CausalSourceIds, Outcome: previous.event.Outcome, Observations: previous.event.Observations, ExecutionIncomplete: previous.producerIncomplete}
		...
		if (len(r.run.Events)+len(staged) == 0) != (snapshot.Kind == testpilotpb.RUN_EVENT_KIND_RUN_OPENED) {
...
func (r *recorder) append(ctx context.Context, event *testpilotpb.RunEvent, monitorFailureFatal bool) error {
	...
	if err != nil {
		r.run.EvaluationFailure = &testpilotpb.Run_EvaluationFailureSequence{EvaluationFailureSequence: event.Sequence}
		if !monitorFailureFatal {
			r.diagnostic(testpilotpb.RUN_DIAGNOSTIC_KIND_MONITOR, "observe_failed", err.Error())
			return err
		}
		return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_MONITOR, "observe_failed", err)
	}
...
func (r *recorder) admit(ctx context.Context, operation func(context.Context) ([]EffectHandle, error), retain func([]EffectHandle)) error {
	...
	if isNil(ctx) || operation == nil || retain == nil {
		return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_INVARIANT, "admission_invalid", errors.New("bounded operation and ownership registration required"))
	}
	if err := ctx.Err(); err != nil {
		return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_cancelled", err)
	}
	handles, err := operation(ctx)
	retain(handles)
	if err != nil {
		return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_failed", err)
	}
	if err := ctx.Err(); err != nil {
		return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, "admission_cancelled", err)
	}
...
func (r *recorder) fail(kind testpilotpb.RunDiagnosticKind, code string, err error) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.closed {
		return invalid(ir.Unavailable, "recorder", "Run closed")
...
func (r *recorder) failLocked(kind testpilotpb.RunDiagnosticKind, code string, err error) error {
	r.incomplete = true
	r.signalStop()
	if r.failure == nil {
		if err == nil {
...
func (r *recorder) diagnostic(kind testpilotpb.RunDiagnosticKind, code, detail string) {
	if len(r.run.Diagnostics) >= r.diagnosticLimit {
		return
	}
	d := &testpilotpb.RunDiagnostic{DiagnosticId: "diagnostic." + strconv.Itoa(len(r.run.Diagnostics)+1), Kind: kind, Code: boundedDiagnostic(code), Detail: boundedDiagnostic(detail)}
	if len(r.run.Events) > 0 {
		d.Support = &testpilotpb.RunDiagnostic_SupportingEventSequence{SupportingEventSequence: int64(len(r.run.Events))}
	}
	r.run.Diagnostics = append(r.run.Diagnostics, d)
}

func (r *recorder) report(kind testpilotpb.RunDiagnosticKind, code string, err error) string {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *recorder) terminalDisposition(executionErr error) testpilotpb.RunStatus {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.stopped {
		return testpilotpb.RUN_STATUS_STOPPED_BY_MONITOR
	}
	if executionErr != nil || r.incomplete {
		return testpilotpb.RUN_STATUS_INCOMPLETE
	}
	return testpilotpb.RUN_STATUS_COMPLETED
}

func (r *recorder) close(ctx context.Context, disposition testpilotpb.RunStatus, cleanup *testpilotpb.CleanupOutcome) (*testpilotpb.Run, *testpilotpb.Verdict, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.closed {
	...
	if disposition < testpilotpb.RUN_STATUS_COMPLETED || disposition > testpilotpb.RUN_STATUS_INCOMPLETE {
		closeErr = invalid(ir.Malformed, "recorder", "terminal disposition required")
	}
	if closeErr != nil {
		closeErr = errors.Join(closeErr, r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, "closure_failed", closeErr))
	}
	if disposition == testpilotpb.RUN_STATUS_INCOMPLETE {
		r.incomplete = true
		r.signalStop()
	}
	if cleanup != nil {
		if err := ir.CheckSurface(cleanup, ir.Limits{Depth: 8, Bytes: 8192, Work: 8192, Fanout: int64(r.diagnosticLimit)}); err != nil {
			closeErr = err
			closeErr = errors.Join(closeErr, r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_RECORDER, "cleanup_recording_failed", err))
		} else {
			r.run.Cleanup = proto.CloneOf(cleanup)
		}
	}
	if int64(len(r.run.Events)) < r.maxEvents {
		if err := r.append(ctx, &testpilotpb.RunEvent{Kind: testpilotpb.RUN_EVENT_KIND_RUN_CLOSED, SourceId: "@recorder/closed", ExecutionIncomplete: r.incomplete}, true); err != nil {
			closeErr = errors.Join(closeErr, err)
		}
	} else {
		err := invalid(ir.LimitExceeded, "recorder", "no capacity for closure event")
		closeErr = errors.Join(closeErr, err, r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_LIMIT, "closure_capacity", err))
	}
	r.run.Status = disposition
	if r.incomplete {
		r.run.Status = testpilotpb.RUN_STATUS_INCOMPLETE
	}
	if r.stopped || disposition == testpilotpb.RUN_STATUS_STOPPED_BY_MONITOR {
		r.run.Status = testpilotpb.RUN_STATUS_STOPPED_BY_MONITOR
	}
	...
	if err != nil {
		closeErr = errors.Join(closeErr, err, r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_MONITOR, "close_failed", err))
	}
	if verdict == nil {
		verdict = &testpilotpb.Verdict{Status: testpilotpb.VERDICT_STATUS_INCONCLUSIVE}
		err = invalid(ir.Malformed, "recorder", "Monitor returned no Verdict")
		closeErr = errors.Join(closeErr, err, r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_MONITOR, "verdict_missing", err))
	}
	r.run.Verdict = proto.CloneOf(verdict)
	if r.incomplete && r.run.Verdict.Status != testpilotpb.VERDICT_STATUS_VIOLATED {
		r.run.Status = testpilotpb.RUN_STATUS_INCOMPLETE
		r.run.Verdict.Status = testpilotpb.VERDICT_STATUS_INCONCLUSIVE
	}
	if r.run.Verdict.Status == testpilotpb.VERDICT_STATUS_VIOLATED {
		r.run.Status = testpilotpb.RUN_STATUS_STOPPED_BY_MONITOR
	}
...
func (r *recorder) postClose(ctx context.Context) error {
	if r.diagnose != nil && r.postCloseCount < r.diagnosticLimit && !isNil(ctx) {
		r.postCloseCount++
		if err := r.diagnose(ctx, r.run.RunId, &testpilotpb.RunDiagnostic{DiagnosticId: "post-close." + strconv.Itoa(r.postCloseCount), Kind: testpilotpb.RUN_DIAGNOSTIC_KIND_POST_CLOSE_EVENT, Code: "publication_closed", Detail: "publication rejected after Run closure"}); err != nil {
			r.diagnose = nil
		}
	}
	return invalid(ir.Unavailable, "recorder", "Run closed")
...
func (r *recorder) completionFailure(ctx context.Context, code string, err error) error {
	...
	return r.failLocked(testpilotpb.RUN_DIAGNOSTIC_KIND_EXECUTION, code, err)
}
