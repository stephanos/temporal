...
func TestConcurrentRunsRouteReorderedWorkflowAndNexusExactly(t *testing.T) {
	...
	headerB, valueB, err := sessionB.preparedNexusDispatch(workflowB.activation, "start", nil, &testpilotspb.Value{Value: &testpilotspb.Value_Text{Text: "b"}})
	require.NoError(t, err)
	headerA, valueA, err := sessionA.preparedNexusDispatch(workflowA.activation, "start", nil, &testpilotspb.Value{Value: &testpilotspb.Value_Text{Text: "a"}})
	require.NoError(t, err)
...
func TestAdmittedWorkflowUsesImmutableNexusDispatchAfterStop(t *testing.T) {
	...
	header, value, err := session.preparedNexusDispatch(routed.activation, "start", nexus.Header{"user": "value"}, &testpilotspb.Value{Value: &testpilotspb.Value_Text{Text: "request"}})
	require.NoError(t, err)
...
func TestAsyncCompletionAuthorityIsOpaqueReplaySafeAndLateBounded(t *testing.T) {
	...
	require.NoError(t, err)
	dispatchHeader, dispatchValue, err := session.preparedNexusDispatch(workflowRoute.activation, "start", nil, &testpilotspb.Value{Value: &testpilotspb.Value_Text{Text: "request"}})
	require.NoError(t, err)
