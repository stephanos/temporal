...
func TestSDKNexusInboundRoutesRedeliveryThroughLedger(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "accepted", result.(*nexus.HandlerStartOperationResultSync[any]).Value.(*testpilotspb.Value).GetTextValue())
	_, err = reservationForEntrypoint(t, session, "handler").Wait(t.Context())
	require.NoError(t, err)
	replay, err := inbound.StartOperation(t.Context(), input)
	require.NoError(t, err)
	require.Equal(t, "accepted", replay.(*nexus.HandlerStartOperationResultSync[any]).Value.(*testpilotspb.Value).GetTextValue())
