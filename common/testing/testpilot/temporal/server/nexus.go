...
func (s *Session) NewCompletionCapability(ctx context.Context, origin testpilot.Coordinate, info CompletionInfo) (testpilot.OpaqueCapability, error) {
	...
	if origin.RunID != s.runID || origin.ActivationID == "" || len(origin.ActivationID) > 256 || s.entries[origin.EntrypointID] != testpilotspb.ENTRYPOINT_KIND_NEXUS_HANDLER {
...
func (s *Session) CompleteNexusOperation(ctx context.Context, c testpilot.Coordinate, opaque testpilot.OpaqueCapability, value *testpilotspb.Value) (testpilot.EffectHandle, error) {
