...
- A supported run uses one P, disables asynchronous preemption and the system monitor, seeds runtime randomness, and advances virtual time at native-runtime quiescence ([architecture](tools/gomadv3/ARCHITECTURE.md#runtime-choices-and-virtual-time)). Each seed executes in a fresh process with a prepared target and constrained deterministic-I/O envelope.
- The optional choice trace is versioned, bounded, checksummed, and retained in artifacts. Its implementation records three kinds: runnable selection, `select` poll permutation, and the final `select` result ([protocol](tools/gomadv3/protocol/choicewire.json), [runtime hooks](tools/gomadv3/go1.26.4.patch), [validator and projection](tools/gomadv3/internal/choicewire/trace.go)).
...
- **The trace is observational.** Current `replay` reruns the same seed/profile and compares the resulting trace after execution; it does not feed the recorded choices back into the runtime or reject a mismatch before applying a decision ([README](tools/gomadv3/README.md)).
- Current campaigns choose seeds, optionally retain semantically novel outcomes, and can resume durable batch work. They do not maintain a schedule frontier, force prefixes, or enumerate alternatives. The existing roadmap correctly places those after choice-tape replay ([bug-finding roadmap](GOMADv3_NEXT_BUG_FINDING.md)).
- `World` already has canonical snapshots, semantic digests, explicit event ordering, transition records, and adapter-owned semantics ([World architecture](tools/gomadv3/ARCHITECTURE.md#world)). It is a promising explicit-state substrate, but it cannot establish native-runtime quiescence or summarize arbitrary application state.
