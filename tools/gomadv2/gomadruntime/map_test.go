...
func TestCheckMap(t *testing.T) {
	t.Cleanup(gomadruntime.CleanupMockFastrand)
	rapid.Check(t, checkMap)
}

func checkMap(t *rapid.T) {
	gomadruntime.SetupMockFastrand()

	m := gomadruntime.NewMap[string, string]()
	model := make(map[string]string)

	actions := make(map[string]func(t *rapid.T))

	var iter gomadruntime.MapIter[string, string]
