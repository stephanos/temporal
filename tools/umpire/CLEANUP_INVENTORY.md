...
| `tools/umpire/cmd/umpire-gen-case-runtime-conformance` | retained | Make generation/check targets build `umpire-case`, invoke this command, compare the entire managed fixture tree, and run facade conformance. |
...
| Semantic inventory document, renamed to `model/INVENTORY.md` after this snapshot | `e534439582339a330d32562f5c796a2a9736a7db200786653902d0c97c204d25` |
...
files in the `tools/umpire3/model` Lake project, those surfaces returned nothing for any root — the silence
...
| Lean previous generations (Nexus v1, Nexus.Race, Umpire Artifact, Space, Exploration under `model/`) | out of scope | Imported by live modules or reserved by open specs fn-22, fn-33, fn-79, and fn-80. Their removal needs a roadmap decision, not this sweep. |
...
The check is that tidy drops **only modules absent from the retained dependency closure**, so the
