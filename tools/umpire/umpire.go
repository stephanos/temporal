...
func New(cfg Config) (*Umpire, error) {
	...
	registry, err := entity.NewEntityRegistry(cfg.Logger, "")
	if err != nil {
	...
	// Initialize model registry (default models are registered automatically).
	modelRegistry := model.NewRegistry()

	// Initialize all registered models.
	if err := modelRegistry.InitModels(context.Background(), nil, Deps{
		Registry: registry,
		Logger:   cfg.Logger,
	}); err != nil {
...
func (w *Umpire) Check(ctx context.Context) []model.Violation {
