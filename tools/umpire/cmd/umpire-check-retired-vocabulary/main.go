...
func main() {
	...
	flag.Parse()
	if flag.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "umpire-check-retired-vocabulary accepts no positional arguments")
		os.Exit(2)
	}

	violations, err := retiredvocabulary.Check(*repositoryRoot)
	if err != nil {
		fmt.Fprintf(os.Stderr, "check retired Umpire vocabulary: %v\n", err)
		os.Exit(2)
	}
