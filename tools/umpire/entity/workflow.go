...
func (wf *Workflow) OnEvent(_ *identity.Identity, iter event.EventIterator) error {
	iter(func(ev event.Event) bool {
		switch e := ev.(type) {
		case *event.StartWorkflowEvent:
		...
		case *event.RespondWorkflowTaskCompletedEvent:
