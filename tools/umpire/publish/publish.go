// Package publish is the exclusive, idempotent publisher: bytes named by their own identity are
// written under a root once, never overwritten, and found already published when written again.
// Umpire's commands and the canary publish through it.
package publish
...
func Check(root, name string, contents []byte) error {
	...
	return sameContents(final, contents)
}
