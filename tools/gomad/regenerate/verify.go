...
func Verify(ctx context.Context, generator Generator, config Config) (Evidence, error) {
	...
	root, err := os.MkdirTemp("", "gomad-regenerate-verify-*")
	if err != nil {
	...
	return Evidence{
		Schema: "gomad.regeneration-evidence/v1", SemanticDigest: outputs[0].corpus.SemanticDigest,
...
func inspectOutput(root string, maxFiles int, maxBytes uint64) (generatedOutput, error) {
	...
	return generatedOutput{
		canonical: canonical.Bytes(), digest: digest("gomad.generated-output/v1", canonical.Bytes()),
