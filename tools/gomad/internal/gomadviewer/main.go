...
type Server struct {
	...
	Logs       []*gomadlog.Log
	LogsByStep map[int]*gomadlog.Log
}
...
var funcs = template.FuncMap{
	"Source": func(frame *gomadlog.Stackframe) (template.HTML, error) {
...
func filter(logs []*gomadlog.Log, f func(*gomadlog.Log) bool) []*gomadlog.Log {
	var filtered []*gomadlog.Log
...
func (s *Server) orRelated(f func(*gomadlog.Log) bool) func(*gomadlog.Log) bool {
	return func(l *gomadlog.Log) bool {
		if f(l) {
			return true
		}
		if l.RelatedStep != 0 {
...
func (s *Server) Index(w http.ResponseWriter, r *http.Request) {
	...
	logs = filter(logs, func(l *gomadlog.Log) bool {
	...
	if focusStr != "" {
		...
		case "goroutine":
			...
			logs = filter(logs, s.orRelated(func(l *gomadlog.Log) bool {
...
func (s *Server) Related(w http.ResponseWriter, r *http.Request) {
	...
	var logs []*gomadlog.Log
	if foundLog != nil {
		logs = filter(s.Logs, func(l *gomadlog.Log) bool {
...
func Viewer(logs string) {
	gomoddir, err := gomadtool.FindGoModDir()
	if err != nil {
	...
	s := &Server{
		...
		Base: filepath.Join(gomoddir, gomadtool.OutputDirectory, "translated", cfg.AsDirname()),

		LogsByStep: make(map[int]*gomadlog.Log),
	}
