...
// MakeGoModForTest makes a go.mod and go.sum for tests that use the `gomad` CLI
// outside of the gomad module. It takes the go.mod from the gomad module strips
// it, and adds a `replace` directive pointing to the gomad module.
//
...
func GetPathForPrecompiledTestBinary(t *testing.T, pkg string) string {
	...
	if isUncachedTestRun() {
		...
		once.Do(func() {
			...
			if err != nil {
				t.Fatalf("gomad build-tests failed: %s\n\n%s", err, string(out))
			}
		})
	}
	...
	if !didBuild {
		t.Logf("Using pre-built gomad test binary %q", path)
		...
		for dep, ts := range deps {
			...
			if !ts.Equal(info.ModTime()) {
				t.Fatalf("Pre-built gomad test binary is out of date. Update binaries with gomad build-tests or run an uncached run with -count=1. See the documentation for metatesting. Input %q changed: %s vs %s.", dep, ts, info.ModTime())
			}
		}
	}
