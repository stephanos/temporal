...
import (
	...
	"github.com/temporalio/gomad/internal/coro"
...
var (
	insideGomadRun atomic.Bool
	gs             atomicValue[*scheduler]
)

func setgs(newgs *scheduler) func() {
	if !insideGomadRun.CompareAndSwap(false, true) {
		panic("gomadruntime already running; only support one per process")
	}
	gs.set(newgs)
	...
	return func() {
		gs.set(nil)
		globalPtr.set(nil)
		if !insideGomadRun.CompareAndSwap(true, false) {
			panic("gomadruntime already stopped!!!???")
		}
	}
}
...
func newScheduler(seed int64, logger logger, checksummer *checksummer, extraenv []string) *scheduler {
	...
	s := &scheduler{
		...
		envs: append(extraenv,
			"GOMAD_LOG_LEVEL="+logger.level.String(),
		),
	}
