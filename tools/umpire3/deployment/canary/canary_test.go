...
func TestCanaryAcceptsQualifiedEvidenceWithSameSemanticDigest(t *testing.T) {
	...
	require.Equal(t, umpire3execution.ClaimConforming, result.Runtime.Claim.Kind)
...
func TestCanaryWorker(t *testing.T) {
	...
	if request.Operation == OperationExecute {
		...
		response.Result = umpire3execution.Result{
			...
			Claim: umpire3execution.Claim{
				Kind: umpire3execution.ClaimConforming, Property: request.Experiment.Property.Identifier,
			},
			Evidence: evidence.Graph{
				FormatVersion: evidence.FormatVersion, Facts: facts,
				Claims: []evidence.Claim{{
					Property: request.Experiment.Property.Identifier,
					Verdict:  string(umpire3execution.ClaimConforming),
				}},
			},
		}
...
func canaryRequest(t *testing.T, mode string) Request {
	...
	file, err := os.Open("../../testdata/generated/update-lifecycle.json")
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, file.Close()) })
	experiment, err := protocolexperiment.DecodeExperiment(file, protocolexperiment.DefaultDecodeLimit)
	require.NoError(t, err)
	config := deployment.Canary("https://temporal.example", "token", "build", "canary-namespace",
		"canary-task-queue", []string{os.Args[0], "-test.run=TestCanaryWorker", "--", mode})
	definition, err := deployment.Define(config)
	require.NoError(t, err)
