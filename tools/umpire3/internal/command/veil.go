...
func RunVeil(arguments []string) error {
	...
	view, err := readVeilFirstOrderView(*input)
	if err != nil {
		return err
	}
	...
	case "check-concrete":
		...
		binding, err := readVeilBinding(*bindingPath)
		if err != nil {
			return err
		}
		var replay []string
		...
		return writeVeilBackendResult(*output, result)
	case "check-job":
		if *jobCommand == "" {
		...
		result, err := veil.RunJob(context.Background(), []string{*jobCommand}, view, binding,
			protocolchecker.BackendJob(*job))
		if err != nil {
			return err
		}
		return writeVeilBackendResult(*output, result)
...
func writeVeilBackendResult(path string, result protocolchecker.BackendResult) error {
	encoded, err := result.CanonicalJSON()
	if err != nil {
		return err
	}
	return writeVeilOutput(path, append(encoded, '\n'))
}

func readVeilFirstOrderView(path string) (protocolchecker.FirstOrderView, error) {
	...
	if decodeErr != nil || closeErr != nil {
		return protocolchecker.FirstOrderView{}, fmt.Errorf("decode first-order view: %w", errors.Join(decodeErr, closeErr))
	}
	return view, nil
}

func readVeilBinding(path string) (veil.BindingArtifact, error) {
	if path == "" {
	...
	binding, decodeErr := veil.DecodeBindingArtifact(input, protocolexperiment.DefaultDecodeLimit)
	closeErr := input.Close()
	if decodeErr != nil || closeErr != nil {
...
func writeVeilOutput(path string, value []byte) error {
