...
namespace Umpire3.Temporal.Feature.NexusLifecycle
...
theorem terminal_stable {state action nextState}
    (terminal : Terminal state) (transition : feature.Step state action nextState) :
    state = nextState := by
  cases state <;> cases action <;> cases nextState <;> simp [Terminal, step] at terminal transition ⊢

end Umpire3.Temporal.Feature.NexusLifecycle
