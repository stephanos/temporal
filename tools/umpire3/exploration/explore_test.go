...
func TestExploreCoversGeneratedNexusLifecycleDenominator(t *testing.T) {
	...
	template := Template{
		...
		Goal: Goal{
			Kind: GoalTransitionCoverage, Target: protocolcatalog.TargetIDFeatureNexus,
		...
		Build: func(assignment Assignment) (scenario.Scenario, error) {
			...
			return scenario.Scenario{
				...
				Target:     protocolcatalog.TargetIDFeatureNexus,
				...
				Root: scenario.OnePath(
					...
					scenario.Require(protocolcatalog.PropertyIDNexusOperationClosure),
				),
			}, nil
		},
...
func TestTransitionCoverageUsesGeneratedDenominatorForEveryTarget(t *testing.T) {
	...
	template := callbackTemplate()
	template.Goal = Goal{
		Kind: GoalTransitionCoverage, Target: protocolcatalog.TargetIDProtocolAtomic,
		Property: protocolcatalog.PropertyIDCallbackResponseConsistency,
	}
...
func TestTransitionCoverageRequiresPositiveRuntimeObservation(t *testing.T) {
	...
	template := Template{
		...
		Goal: Goal{Kind: GoalTransitionCoverage, Target: protocolcatalog.TargetIDFeatureNexus,
			Property: protocolcatalog.PropertyIDNexusOperationClosure},
...
func callbackTemplate() Template {
	return Template{
		...
		Goal:       Goal{Kind: GoalChallengeSafety, Property: protocolcatalog.PropertyIDCallbackResponseConsistency},
		Holes: []Hole{{
		...
		Build: func(assignment Assignment) (scenario.Scenario, error) {
			return scenario.Scenario{
				...
				Root: scenario.OnePath(
					scenario.Action("respond-"+assignment["variant"].Text, protocolcatalog.ActionKindRecordCallbackResponse),
...
func nexusCoverageAction(action string) protocolcatalog.ActionKind {
	switch action {
	case "schedule", "reject":
		return protocolcatalog.ActionKindScheduleOperation
	case "attempt-failed":
		return protocolcatalog.ActionKindRetryTask
	case "start":
		return protocolcatalog.ActionKindDispatchTask
	case "succeed":
		return protocolcatalog.ActionKindPersistSuccess
	case "fail", "terminate":
		return protocolcatalog.ActionKindCloseNexusOperation
	case "cancel":
		return protocolcatalog.ActionKindCommitCancellation
	case "timeout":
		return protocolcatalog.ActionKindTimeoutNexusOperation
	default:
		return ""
	}
}
