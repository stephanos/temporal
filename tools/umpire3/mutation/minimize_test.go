...
func TestMinimizeActionsRemovesIrrelevantStepsAndPreservesViolation(t *testing.T) {
	...
	minimized, err := MinimizeActions(context.Background(), experiment, func(_ context.Context, candidate protocolexperiment.Experiment) (umpire3execution.Result, error) {
...
func TestMinimizeActionsRejectsDifferentPropertyViolation(t *testing.T) {
	experiment := loadMutationExperiment(t)
	_, err := MinimizeActions(context.Background(), experiment, func(context.Context, protocolexperiment.Experiment) (umpire3execution.Result, error) {
...
func TestMinimizeActionsPreservesGroundedViolationIdentity(t *testing.T) {
	...
	minimized, err := MinimizeActions(context.Background(), experiment,
		func(_ context.Context, candidate protocolexperiment.Experiment) (umpire3execution.Result, error) {
...
func TestMinimizeExperimentRemovesUnusedResourcesFaultsAndBindings(t *testing.T) {
	...
	experiment.Actions[2].Arguments = []protocolexperiment.NamedValue{{
		Name:  "reason",
		Value: protocolexperiment.Value{Type: protocolexperiment.ValueString, Text: &reason},
	}}
	experiment.Actions[0].Bindings = []protocolexperiment.Binding{{
		Symbol: "unused", Type: "identity", Projection: "operation-id",
	}}
	experiment.Actions = append(experiment.Actions, protocolexperiment.Action{
		Identifier: "unused-fault", Kind: "crash-owner",
		AllowedOutcomes:      []protocolexperiment.ActionOutcome{protocolexperiment.ActionOutcomeApplied},
		RequiredCapabilities: []string{"failover-control"},
	})
	experiment.Resources = append(experiment.Resources, protocolexperiment.Resource{Identifier: "unused", Kind: "nexus-worker"})
...
func TestMinimizeExperimentShrinksOrderPayloadPolicyAndFaultConfiguration(t *testing.T) {
	...
	experiment.Policies[0].Arguments = []protocolexperiment.NamedValue{{
		Name: "commands", Value: protocolexperiment.Value{Type: protocolexperiment.ValueList, Elements: []protocolexperiment.Value{{
			Type: protocolexperiment.ValueString, Text: &listItem,
		}}},
	}}
	experiment.Faults[0].Arguments = []protocolexperiment.NamedValue{{
		Name: "response-code", Value: protocolexperiment.Value{Type: protocolexperiment.ValueInteger, Integer: &faultArgument},
	}}
	experiment.Faults[0].Occurrence = protocolexperiment.FaultOccurrence{First: 3, Count: 4}
	...
	require.Equal(t, protocolexperiment.FaultOccurrence{First: 1, Count: 1}, minimized.Faults[0].Occurrence)
