...
import (
	...
	"go.temporal.io/server/tools/umpire3/replay"
...
type Executor func(context.Context, protocolexperiment.Experiment) (umpire3execution.Result, []CoveragePoint, error)

type Request struct {
	Candidates       []Candidate
	Traces           []protocolchecker.SemanticTrace
...
type Discovery struct {
	...
	Mutation       MutationKind                  `json:"mutation,omitempty"`
	Path           string                        `json:"path,omitempty"`
	Original       protocolexperiment.Experiment `json:"original"`
	Minimized      protocolexperiment.Experiment `json:"minimized"`
...
func Run(ctx context.Context, request Request) (Report, error) {
	...
	if request.Mutation != nil {
		...
		for _, trace := range request.Traces {
			...
			if trace.Kind == protocolchecker.SemanticTraceLive {
	...
	for _, execution := range results {
		...
		if execution.result.Claim.Kind == umpire3execution.ClaimViolating {
...
func minimizeDiscovery(ctx context.Context, execution executionResult, maxAttempts int, executor Executor) Discovery {
	...
	minimized, err := MinimizeExperiment(ctx, execution.ranked.experiment,
		func(ctx context.Context, experiment protocolexperiment.Experiment) (umpire3execution.Result, error) {
			if attempts == maxAttempts {
				return umpire3execution.Result{}, budgetErr
			}
			attempts++
...
func promotionSource(experiment protocolexperiment.Experiment) (string, error) {
	...
	source.WriteString("\t\"go.temporal.io/server/tools/umpire3/execution\"\n")
	source.WriteString("\t\"go.temporal.io/server/tools/umpire3/scenario\"\n")
	source.WriteString("\t\"go.temporal.io/server/tools/umpire3/regression\"\n")
	source.WriteString(")\n\n")
	source.WriteString("func RequirePromoted(t regression.TestingT, factory execution.Factory) {\n")
	fmt.Fprintf(&source, "\tauthored := scenario.%sScenario(%q, []scenario.Resource{\n",
	...
	source.WriteString("\tregression.RequireRegression(t, authored, regression.WithEnvironment(factory))\n")
	source.WriteString("}\n")
...
type promotionInterval struct {
	...
	fault    protocolexperiment.Fault
	children []*promotionInterval
}

func promotionRoot(experiment protocolexperiment.Experiment) (string, error) {
...
func renderPromotionAction(action protocolexperiment.Action) (string, error) {
	...
	case protocolexperiment.ResponseSynchronous:
	case protocolexperiment.ResponseAsynchronous:
		options = append(options, "scenario.Asynchronously()")
	case protocolexperiment.ResponseDeferred:
		options = append(options, "scenario.Deferred()")
	case protocolexperiment.ResponseBlocking:
		options = append(options, fmt.Sprintf("scenario.BlockingFor(%d)", action.MaxBlockNanos))
	case protocolexperiment.ResponseFailure:
	...
	for _, binding := range action.Bindings {
		if binding.Type != string(protocolcatalog.SemanticTypeIDIdentity) {
...
func renderPromotionActionArgument(action protocolexperiment.Action, argument protocolexperiment.NamedValue) (string, error) {
...
func renderPromotionValue(value protocolexperiment.Value) (string, error) {
	switch value.Type {
	case protocolexperiment.ValueString:
		return fmt.Sprintf("scenario.String(%q)", textOrEmpty(value.Text)), nil
	case protocolexperiment.ValueInteger:
	...
	case protocolexperiment.ValueDuration:
	...
	case protocolexperiment.ValueSymbol:
...
func renderPromotionFault(fault protocolexperiment.Fault, experiment protocolexperiment.Experiment) (string, error) {
	catalog, err := protocolcatalog.DefaultCatalog()
	if err != nil {
		return "", err
	}
...
func renderPromotionResources(identifiers []string, resources []protocolexperiment.Resource) ([]string, error) {
	byIdentifier := make(map[string]protocolexperiment.Resource, len(resources))
...
func promotionOutcomeName(outcome protocolexperiment.ActionOutcome) (string, error) {
	switch outcome {
	case protocolexperiment.ActionOutcomeApplied:
		return "Applied", nil
	case protocolexperiment.ActionOutcomeSuppressed:
		return "Suppressed", nil
	case protocolexperiment.ActionOutcomeRejected:
		return "Rejected", nil
	case protocolexperiment.ActionOutcomeRetried:
		return "Retried", nil
	case protocolexperiment.ActionOutcomeFaultIntercepted:
...
func modelCoverage(experiment protocolexperiment.Experiment) ([]CoveragePoint, error) {
