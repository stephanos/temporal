...
func TestCorruptedAndPartialCertificatesFailClosed(t *testing.T) {
	...
	require.NoError(t, err)
	decoded, err := DecodeCertificate(bytes.NewReader(encoded), protocolexperiment.DefaultDecodeLimit, view)
	require.NoError(t, err)
	require.Equal(t, certificate, decoded)

	corrupted := certificate
	corrupted.Nodes[1].Action = protocolcatalog.ActionKindPersistSuccess
...
func TestCounterexampleRequiresMatchingCanonicalReplayBeforeNormalization(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, protocolchecker.SemanticTraceProducerNative, trace.Producer)
	require.Len(t, trace.Steps, len(counterexample.Actions))

	receipt.Actions[0] = protocolcatalog.ActionKindRequestCancellation
	_, err = NormalizeCounterexample(view, counterexample, receipt)
	require.Error(t, err)
}

func BenchmarkParallelProducerTenReplicas(b *testing.B) {
	view, found, err := DefaultFirstOrderView(protocolcatalog.TargetIDNexusCancellation, "sound")
...
func soundView(t testing.TB) protocolchecker.FirstOrderView {
	t.Helper()
	view, found, err := DefaultFirstOrderView(protocolcatalog.TargetIDNexusCancellation, "sound")
	require.NoError(t, err)
	require.True(t, found)
