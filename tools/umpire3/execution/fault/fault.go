...
func Preflight(term Term, capabilities []protocolcatalog.CapabilityID, allowRestricted bool) error {
	catalog, err := protocolcatalog.DefaultCatalog()
	if err != nil {
		return fmt.Errorf("load fault catalog: %w", err)
	}
	var declaration *protocolcatalog.FaultDeclaration
	for index := range catalog.Faults {
		if protocolcatalog.FaultKind(catalog.Faults[index].Identifier) == term.Kind {
	...
	have := make(map[protocolcatalog.CapabilityID]struct{}, len(capabilities))
	for _, capability := range capabilities {
		have[capability] = struct{}{}
	}
	var missing []protocolcatalog.CapabilityID
