...
func TestExperimentResponseSemanticsCompileToParticipantProgram(t *testing.T) {
	...
	experiment, err := protocolexperiment.DecodeExperiment(file, protocolexperiment.DefaultDecodeLimit)
	require.NoError(t, err)
	experiment.Actions[0].ResponseMode = protocolexperiment.ResponseDeferred
...
func TestCallbackActionsCompileToDedicatedSDKOperations(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{protocolcatalog.ActionKindRegisterCallback, CommandCallbackRegister, SDKRegisterCallback},
		{protocolcatalog.ActionKindRecordCallbackResponse, CommandCallbackComplete, SDKCompleteCallback},
	} {
