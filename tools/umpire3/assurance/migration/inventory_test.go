...
func TestCheckedLedgerMatchesMechanicalRootInventory(t *testing.T) {
	...
	ledger, err := Build("../../../../tests")
	require.NoError(t, err)
	generated, err := ledger.CanonicalJSON()
	require.NoError(t, err)
	checked, err := os.ReadFile("testdata/generated/ledger.json")
	require.NoError(t, err)
...
func TestDefaultLedgerMatchesRetainedArtifact(t *testing.T) {
	retained, err := os.ReadFile("testdata/generated/ledger.json")
	require.NoError(t, err)
...
func TestLedgerRejectsUnsupportedFidelityAndEvidenceClaims(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate:     func(entry *Entry) { entry.Fidelity = protocolcatalog.FidelityInventoryOnly },
		...
		},
		{
			...
			mutate:     func(entry *Entry) { entry.EvidenceLevel = protocolcatalog.EvidenceModelProof },
...
func TestBehaviorContractsPreserveFaultSemanticsAndScopes(t *testing.T) {
	...
	require.Equal(t, []protocolcatalog.FaultKind{"hold-release"}, faultAction.Faults)
	httpFault, exists := Contract("ProbeNexusHTTPFaultSeam")
	require.True(t, exists)
	require.Equal(t, []protocolcatalog.FaultKind{"hold-release"}, httpFault.Faults)
