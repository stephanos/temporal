...
func TestIndependentLayout(t *testing.T) {
	...
	require.FileExists(t, filepath.Join(root, "..", "..", "model", "Shared", "Transition.lean"))
	require.FileExists(t, filepath.Join(root, "..", "..", "model", "Shared", "TraceReplay.lean"))
...
func TestGoPackagesDoNotImportPreviousUmpires(t *testing.T) {
	...
	require.NoError(t, err)
	for _, path := range []string{"../../tests/umpire3_test.go", "../../tests/umpire3_probe_test.go", "../../tests/umpire3_regress_test.go"} {
...
func TestRetainedUmpire2RootTestsDoNotImportUmpire3(t *testing.T) {
	...
	for _, path := range []string{"../../tests/umpire2_test.go", "../../tests/umpire2_probe_test.go", "../../tests/umpire2_regress_test.go"} {
		rootViolations, err := findImportsWithPrefixes(path, []string{"go.temporal.io/server/tools/umpire3"})
		require.NoError(t, err)
		violations = append(violations, rootViolations...)
	}
	require.Empty(t, violations)
...
import _ "go.temporal.io/server/tools/umpire2"
`), 0o600))

	violations, err := findForbiddenImports(fixture)
	require.NoError(t, err)
	require.Equal(t, []string{"bad.go imports go.temporal.io/server/tools/umpire2"}, violations)
}
...
func findUnexpectedUmpire3Imports(root string, allowed []string) ([]string, error) {
	const prefix = "go.temporal.io/server/tools/umpire3/"
