...
type qualifySetDependencies struct {
	...
	run        func(context.Context, qualificationset.Config) (qualificationset.SetReport, error)
}
...
func runQualifySetWith(arguments []string, stdout, stderr io.Writer, dependencies qualifySetDependencies) int {
	...
	if *check {
		if *format == "json" {
			encoded, encodeErr := record.CanonicalJSON(struct {
	...
	report, runErr := dependencies.run(context.Background(), qualificationset.Config{
...
func classifyQualificationSetError(report qualificationset.SetReport, runErr error) (int, string) {
...
func writeQualificationSetResult(output io.Writer, format string, report qualificationset.SetReport) error {
	if format == "json" {
