...
test_tmp=$(mktemp -d "$script_dir/.toolchain/clock-audit.XXXXXX")
cleanup() {
...
if [[ $EUID -eq 0 ]]; then
	...
	if ! sudo -n true 2>/dev/null; then
		printf 'gomadv3 clock audit requires root DTrace privileges\n' >&2
		exit 1
	fi
	dtrace_sudo=(sudo -n)
fi
...
run_trace() {
	...
	local -a environment=(/usr/bin/env -u GOMADSEED -u GOMADV3_CHILD_SEED TZ=UTC)
	if [[ -n $seed ]]; then
		environment+=(GOMADSEED="$seed")
	fi
	if ! "${dtrace_sudo[@]}" "${environment[@]}" /usr/sbin/dtrace -q -s "$probe" -c "$test_tmp/clock-audit" >"$output" 2>&1; then
		printf 'gomadv3 clock audit tracer failed for %s\n' "$name" >&2
		cat "$output" >&2
		exit 1
	fi
	if grep -Eq 'dtrace: (error|failed)|DTrace requires additional privileges' "$output"; then
		printf 'gomadv3 clock audit tracer was unavailable for %s\n' "$name" >&2
		cat "$output" >&2
		exit 1
	fi
	if ! grep -Fq 'GOMADV3_AUDIT_START' "$output"; then
		printf 'gomadv3 clock audit did not observe its activation marker for %s\n' "$name" >&2
		cat "$output" >&2
		exit 1
	fi
}
...
for function in clock_gettime mach_absolute_time; do
	if ! grep -Eq "GOMADV3_HOST_CLOCK .*${function}" "$test_tmp/positive.trace"; then
		printf 'gomadv3 clock audit positive control missed %s\n' "$function" >&2
...
if grep -Fq 'GOMADV3_HOST_CLOCK ' "$test_tmp/deterministic.trace"; then
	printf 'gomadv3 deterministic target reached an ambient host clock\n' >&2
	cat "$test_tmp/deterministic.trace" >&2
	exit 1
fi

printf 'gomadv3 host-clock escape audit passed\n'
