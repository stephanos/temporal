...
func ImplementationSHA256() evidence.SHA256 {
	return evidence.DomainHash("gomadv3-choice-frontier-controller/v1", []byte(controllerIdentity))
}
...
type Config struct {
	Execution        choice.ExecutionIdentity `json:"execution"`
...
type Candidate struct {
	...
	SourceTraceSHA256 evidence.SHA256 `json:"source_trace_sha256,omitempty"`
...
type Result struct {
	CandidateSHA256 evidence.SHA256
...
type SegmentResult struct {
	...
	TraceSHA256       evidence.SHA256 `json:"trace_sha256,omitempty"`
	TraceSourceSHA256 evidence.SHA256 `json:"trace_source_sha256,omitempty"`
...
func New(config Config) (State, error) {
	...
	if err := choice.ValidateExecutionIdentity(config.Execution); err != nil {
	...
	return State{Config: config, Queue: []Candidate{root}, Seen: []evidence.SHA256{root.SHA256}, Outcomes: []evidence.SHA256{}, FailureSignatures: []evidence.SHA256{}, PendingBytes: pendingBytes}, nil
}
...
func (candidate Candidate) PrefixReplayPlan(identity choice.ExecutionIdentity) (choice.ReplayPlan, error) {
	if candidate.ForcedDepth == 0 {
		if candidate.PrefixSHA256 != "" || len(candidate.PrefixBytes) != 0 {
			return choice.ReplayPlan{}, errors.New("root choice candidate contains a prefix")
		}
		return choice.ReplayPlan{}, nil
	}
	digest, err := candidate.PrefixSHA256.Bytes()
	if err != nil {
		return choice.ReplayPlan{}, fmt.Errorf("decode choice candidate prefix identity: %w", err)
	}
	tape := choice.ReplayPlan{Identity: identity, Bytes: append([]byte(nil), candidate.PrefixBytes...), SHA256: digest}
...
func CommitRound(state State, round Round, results []Result) (State, RoundSegment, error) {
	...
	newChildren := make(map[evidence.SHA256]Candidate)
	policyStopped := false
	for index, result := range results {
		...
		if result.Trace != nil {
			...
			segmentResult.TraceSHA256 = evidence.SHA256FromSum(trace.SHA256)
			segmentResult.TraceSourceSHA256 = evidence.SHA256FromSum(trace.SourceTraceSHA256)
...
func ReplaySegment(state State, segment RoundSegment) (State, error) {
	...
	for index, stored := range segment.Results {
		...
		if len(stored.TraceBytes) != 0 {
			...
			tape := choice.ReplayPlan{Identity: state.Config.Execution, Bytes: append([]byte(nil), stored.TraceBytes...), SHA256: digest}
			validated, validateErr := choice.ValidateReplayPlan(tape, state.Config.Execution)
			if validateErr != nil || evidence.SHA256FromSum(validated.SourceTraceSHA256) != stored.TraceSourceSHA256 {
...
func expandCandidate(state *State, parent Candidate, trace choice.ReplayPlan, children map[evidence.SHA256]Candidate) error {
	for ordinal, decision := range trace.Decisions {
		...
		for rank := uint32(0); rank < decision.Alternatives; rank++ {
			...
			prefix, err := choice.BuildRankPrefix(trace, uint64(ordinal), rank)
			if err != nil {
				return fmt.Errorf("build choice frontier child at decision %d rank %d: %w", ordinal, rank, err)
			}
			child, err := newCandidate(state.Config, &prefix, parent.SHA256, evidence.SHA256FromSum(trace.SourceTraceSHA256))
			if err != nil {
				return err
			}
...
type candidateIdentityProjection struct {
	...
	ControllerSHA256     evidence.SHA256              `json:"controller_sha256"`
	BaseSeed             evidence.Uint64String        `json:"base_seed"`
...
type decisionIdentityProjection struct {
	...
	SelectedIdentity     evidence.SHA256       `json:"selected_identity,omitempty"`
	AlternativeSetDigest evidence.SHA256       `json:"alternative_set_digest"`
}

func newCandidate(config Config, prefix *choice.ReplayPlan, parent, source evidence.SHA256) (Candidate, error) {
	decisions := []choice.Decision{}
	candidate := Candidate{ParentSHA256: parent, SourceTraceSHA256: source}
	if prefix != nil {
		validated, err := choice.ValidatePrefixReplayPlan(*prefix, config.Execution)
		if err != nil {
			return Candidate{}, err
		}
		decisions = validated.Decisions
		candidate.ForcedDepth = uint64(len(decisions))
		candidate.PrefixSHA256 = evidence.SHA256FromSum(validated.SHA256)
		candidate.PrefixBytes = append([]byte(nil), validated.Bytes...)
	}
	projection := candidateIdentityProjection{
		TargetSHA256: evidence.SHA256FromSum(config.Execution.TargetSHA256), ToolchainBuildKey: config.Execution.ToolchainBuildKey,
		GOOS: config.Execution.GOOS, GOARCH: config.Execution.GOARCH, ImplementationSHA256: evidence.SHA256FromSum(config.Execution.ImplementationSHA256),
		ControllerSHA256: config.ControllerSHA256,
		BaseSeed:         evidence.Uint64String(config.BaseSeed), Decisions: make([]decisionIdentityProjection, len(decisions)),
	}
	for index, decision := range decisions {
		selected := evidence.SHA256("")
		if decision.SelectedIdentity != ([sha256.Size]byte{}) {
			selected = evidence.SHA256FromSum(decision.SelectedIdentity)
		}
		projection.Decisions[index] = decisionIdentityProjection{
			Ordinal: evidence.Uint64String(index), Kind: uint8(decision.Kind), SiteOffset: evidence.Uint64String(decision.SiteOffset),
			SiteMissing: decision.SiteMissing, RankOverride: decision.RankOverride, Alternatives: decision.Alternatives,
			Selected: decision.Selected, Data: decision.Data, SelectedIdentity: selected, AlternativeSetDigest: evidence.SHA256FromSum(decision.AlternativeSetDigest),
		}
	}
	...
	candidate.SHA256 = evidence.DomainHash(candidateDomain, encoded)
	return candidate, nil
}

func validateCandidateTrace(candidate Candidate, trace choice.ReplayPlan, identity choice.ExecutionIdentity) error {
	if candidate.ForcedDepth == 0 {
		return nil
	}
...
func segmentIdentity(segment RoundSegment) (evidence.SHA256, error) {
	copy := segment
	copy.SHA256 = ""
	encoded, err := evidence.CanonicalJSON(copy)
	if err != nil {
		return "", err
	}
...
func cloneState(state State) State {
	...
	copy.Seen = append([]evidence.SHA256(nil), state.Seen...)
	copy.Outcomes = append([]evidence.SHA256(nil), state.Outcomes...)
	copy.FailureSignatures = append([]evidence.SHA256(nil), state.FailureSignatures...)
	return copy
}
