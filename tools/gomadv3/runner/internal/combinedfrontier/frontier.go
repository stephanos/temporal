...
const (
	candidateDomain    = "gomadv3-combined-frontier-candidate/v1"
	decisionDomain     = "gomadv3-combined-frontier-decision/v1"
...
func ImplementationSHA256() evidence.SHA256 {
	return evidence.DomainHash("gomadv3-combined-frontier-controller/v1", []byte(controllerIdentity))
}
...
const (
	...
	StopDepthComplete     StopReason = "combined_depth_complete"
...
type State struct {
	...
	Seen               []evidence.SHA256 `json:"seen"`
	Outcomes           []evidence.SHA256 `json:"outcomes"`
...
type RoundSegment struct {
	...
	SHA256       evidence.SHA256 `json:"sha256,omitempty"`
}
...
func New(config Config) (State, error) {
	...
	if pendingBytes > config.MaxFrontierBytes {
		return State{}, fmt.Errorf("combined frontier root requires %d bytes, exceeding the %d-byte frontier bound", pendingBytes, config.MaxFrontierBytes)
	}
	return State{
		Config: config, Queue: []Candidate{root}, Seen: []evidence.SHA256{root.SHA256},
		Outcomes: []evidence.SHA256{}, FailureSignatures: []evidence.SHA256{}, PendingBytes: pendingBytes,
	}, nil
}
...
func (state State) Summary() Summary {
	return Summary{
		Parallel: state.Config.Parallel, MaxRuns: state.Config.MaxRuns, MaxForcedDecisions: state.Config.MaxForcedDecisions,
		MaxFrontierBytes: state.Config.MaxFrontierBytes, MaxResultBytes: state.Config.MaxResultBytes,
		...
		DeepestOverride: state.DeepestOverride, OmittedByRunBound: state.OmittedByRunBound,
...
func StateSHA256(state State) (evidence.SHA256, error) {
...
func CommitRound(state State, round Round, results []Result) (State, RoundSegment, error) {
	...
	if !ok || round.Index != expected.Index || !sameCandidates(round.Candidates, expected.Candidates) {
		return State{}, RoundSegment{}, errors.New("combined frontier round does not match the current state")
	}
	if len(results) != len(round.Candidates) {
		return State{}, RoundSegment{}, errors.New("combined frontier result count does not match its round")
	}
	...
	children := make(map[evidence.SHA256]Candidate)
	for index, result := range results {
		candidate := round.Candidates[index]
		canonical, canonicalErr := canonicalResult(result, candidate, next.Config)
		if canonicalErr != nil {
			return State{}, RoundSegment{}, fmt.Errorf("combined frontier result %d: %w", index, canonicalErr)
		}
...
func ReplaySegment(state State, segment RoundSegment) (State, error) {
	...
	if err != nil || identity != segment.SHA256 {
		return State{}, errors.Join(errors.New("combined frontier segment identity does not match"), err)
	}
	before, err := stateIdentity(state)
	if err != nil || before != segment.BeforeSHA256 || segment.Schema != RoundSegmentSchema {
		return State{}, errors.Join(errors.New("combined frontier segment does not link to its state"), err)
	}
	round, ok := state.NextRound()
	if !ok || round.Index != segment.Index || len(round.Candidates) != len(segment.Results) {
		return State{}, errors.New("combined frontier segment round is unavailable")
	}
	...
	left, err := evidence.CanonicalJSON(segment)
	if err != nil {
		return State{}, err
	}
	right, err := evidence.CanonicalJSON(regenerated)
	if err != nil {
		return State{}, err
	}
	after, err := stateIdentity(next)
	if err != nil || !bytes.Equal(left, right) || after != segment.AfterSHA256 {
		return State{}, errors.Join(errors.New("combined frontier segment replay changed its canonical result"), err)
	}
	return next, nil
}
...
func expandCandidate(state *State, parent Candidate, decisions []Decision, children map[evidence.SHA256]Candidate) error {
...
func admitChildren(state *State, children []Candidate) error {
	...
	for index, candidate := range children {
		...
		if entryBytes > ^uint64(0)-pendingBytes {
			return errors.New("combined frontier byte accounting overflow")
		}
...
func newCandidate(config Config, overrides []ForcedDecision, parent evidence.SHA256) (Candidate, error) {
	...
	candidate := Candidate{
		SHA256: evidence.DomainHash(candidateDomain, encoded), ParentSHA256: parent, Overrides: overrides,
	}
	if parent != "" {
...
func validateConfig(config Config) error {
	if config.Parallel <= 0 || config.MaxRuns == 0 || config.MaxFrontierBytes == 0 || config.MaxResultBytes == 0 || config.FailureBudget == 0 {
		return errors.New("combined frontier requires positive parallel, run, frontier, result, and failure bounds")
	}
	for name, limit := range map[string]uint64{
		...
		if limit == 0 {
			return fmt.Errorf("combined frontier %s dimension bound is zero", name)
		}
	}
	if _, err := config.ExecutionSHA256.Bytes(); err != nil {
		return fmt.Errorf("combined frontier execution identity: %w", err)
	}
	if config.ControllerSHA256 != ImplementationSHA256() {
		return errors.New("combined frontier controller identity does not match this implementation")
	}
	return nil
}
...
func validateDecision(decision Decision) error {
	...
	seen := make(map[evidence.SHA256]struct{}, len(decision.Alternatives))
...
func validateForcedDecision(forced ForcedDecision) error {
	...
	for _, identity := range []evidence.SHA256{forced.SiteSHA256, forced.AlternativeSetSHA256, forced.SelectedSHA256, forced.Identity} {
...
func alternativeSetIdentity(dimension Dimension, ordinal uint64, site evidence.SHA256, alternatives []evidence.SHA256) (evidence.SHA256, error) {
	...
	encoded, err := evidence.CanonicalJSON(struct {
		...
		SiteSHA256   evidence.SHA256   `json:"site_sha256"`
		Alternatives []evidence.SHA256 `json:"alternatives"`
	}{dimension, ordinal, site, append([]evidence.SHA256(nil), alternatives...)})
	if err != nil {
		return "", err
	}
	return evidence.DomainHash("gomadv3-combined-frontier-alternative-set/v1", encoded), nil
}
...
func decisionIdentity(decision Decision) (evidence.SHA256, error) {
	...
	return evidence.DomainHash(decisionDomain, encoded), nil
}

func forcedDecisionIdentity(forced ForcedDecision) (evidence.SHA256, error) {
	forced.Identity = ""
	encoded, err := evidence.CanonicalJSON(forced)
	if err != nil {
		return "", err
	}
	return evidence.DomainHash(forcedDomain, encoded), nil
}
...
func stateIdentity(state State) (evidence.SHA256, error) {
	...
	encoded, err := evidence.CanonicalJSON(cloned)
	if err != nil {
		return "", err
	}
	return evidence.DomainHash(stateDomain, encoded), nil
}

func segmentIdentity(segment RoundSegment) (evidence.SHA256, error) {
	segment.SHA256 = ""
	...
	return evidence.DomainHash(roundSegmentDomain, encoded), nil
}
...
func queueBytes(queue []Candidate) (uint64, error) {
	...
	for _, candidate := range queue {
		size, err := candidateBytes(candidate)
		if err != nil {
			return 0, err
		}
		if size > ^uint64(0)-total {
			return 0, errors.New("combined frontier queue byte accounting overflow")
		}
		total += size
	}
	return total, nil
...
func insertIdentity(values []evidence.SHA256, value evidence.SHA256) []evidence.SHA256 {
...
func containsIdentity(values []evidence.SHA256, value evidence.SHA256) bool {
...
func cloneState(state State) State {
	...
	state.Seen = append([]evidence.SHA256(nil), state.Seen...)
	state.Outcomes = append([]evidence.SHA256(nil), state.Outcomes...)
	state.FailureSignatures = append([]evidence.SHA256(nil), state.FailureSignatures...)
	return state
}
