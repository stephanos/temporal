...
func TestOutputCaptureRetainsExactStreamWithinLimit(t *testing.T) {
	capture, err := NewOutputCapture(8)
	if err != nil {
		t.Fatal(err)
	}
	input := []byte("abcdefgh")
...
func TestOutputCaptureKeepsHeadMarkerAndTail(t *testing.T) {
	capture, err := NewOutputCapture(8)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOutputCaptureIsIndependentOfWriteChunking(t *testing.T) {
	input := []byte("0123456789abcdefghijklmnopqrstuvwxyz")
	oneWrite, err := NewOutputCapture(12)
	if err != nil {
		t.Fatal(err)
	}
	...
	byteWrites, err := NewOutputCapture(12)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOutputCaptureRejectsZeroLimit(t *testing.T) {
	if _, err := NewOutputCapture(0); err == nil {
		t.Fatal("NewOutputCapture(0) succeeded")
	}
}
