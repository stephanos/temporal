...
func runtimeDecisionIdentities(runtimeDecision choice.Decision) (record.SHA256, []record.SHA256, error) {
	...
	site := record.DomainHash("gomadv3-runtime-choice-site/v1", encodedSite)
	...
	for rank := range alternatives {
		...
		alternatives[rank] = record.DomainHash("gomadv3-runtime-choice-alternative/v1", encodedAlternative)
	}
...
func projectRetainedRecord(recordBytes, expectedPlan []byte) (retainedRecord, error) {
	...
	projected.outcomeSHA256 = record.DomainHash("gomadv3-simulation-exploration-outcome/v1", encodedSemantic)
	if projected.failed {
