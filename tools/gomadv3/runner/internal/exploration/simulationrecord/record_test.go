...
func TestResultForRecordProjectsValidatedDecisionsAndSemanticOutcome(t *testing.T) {
	...
	recordValue := struct {
		...
		Schema: "gomadv3.cluster-record/v7", Seed: 17, SpecSHA256: record.HashBytes([]byte("candidate-bound-spec")), Outcome: "completed",
...
func TestProjectArtifactBindsExactReplayAndNormalizedFailure(t *testing.T) {
	...
	recordBytes, err := json.Marshal(struct {
		...
		Schema: "gomadv3.cluster-record/v7", Seed: config.BaseSeed, SpecSHA256: record.HashBytes([]byte("spec")),
