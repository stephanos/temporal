...
func TestCampaignLifecycleRejectsIllegalTransitionWithoutMutation(t *testing.T) {
	...
	path := filepath.Join(journal.Path(), ".partial", "campaign", "partial.json")
...
func TestCampaignLifecycleRejectsCorruptState(t *testing.T) {
	...
	if err := atomicWrite(filepath.Join(journal.Path(), ".partial", "campaign", "partial.json"), []byte("{}")); err != nil {
		t.Fatal(err)
	}
...
func TestRecoverCampaignFinishesPublishedCleanup(t *testing.T) {
	...
	for _, path := range []string{journal.PreparedPath(), filepath.Join(journal.Path(), ".partial", "campaign")} {
...
func TestRecoverCampaignRejectsIdentitylessPlannedStateWithoutMutation(t *testing.T) {
	...
	before, err := os.ReadFile(filepath.Join(journal.Path(), ".partial", "campaign", "partial.json"))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := RecoverCampaign(context.Background(), journal.Path()); err == nil {
	...
	after, err := os.ReadFile(filepath.Join(journal.Path(), ".partial", "campaign", "partial.json"))
	if err != nil {
		t.Fatal(err)
	}
	if string(after) != string(before) {
