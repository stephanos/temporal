...
func TestCampaignJournalPublishesTheCanonicalCampaignLifecycle(t *testing.T) {
	...
	if batch.Schema != "gomadv3.campaign/v1" || batch.Journal == nil || batch.Journal.Records != 1 || batch.Journal.Segments != 1 {
...
func testBatchPlan(journal *CampaignJournal, digest record.SHA256, size uint64) CampaignPlan {
	...
	plan := CampaignPlan{
		...
		Prepared: PreparedTargetPlan{
			...
			Target: record.Target{Kind: "go-test", Source: "./pkg", SHA256: digest, Size: record.Uint64String(size), Argv: []string{"gomadv3-target"}, BuildTags: []string{"gomad_fixture"}, Adapters: []record.TargetAdapter{}, Compatibility: []record.CompatibilityPack{}, CapabilityMode: "closure", BuildInfo: record.BuildInfo{GoVersion: "go1.26.4", Path: "example.test/pkg.test"}},
		},
		IOProfile:   deterministicio.Contract{Name: "gomadv3-deterministic/v1", ImplementationSHA256: deterministicio.Digest(record.HashBytes([]byte("io"))), InventorySHA256: deterministicio.Digest(record.HashBytes([]byte("inventory")))},
...
func TestBatchJournalPreservesExplicitFailureState(t *testing.T) {
	...
	const wantLifecycleFailure = `{"campaign_id":"run-failed","detail":"build failed","last_stable_state":"planned","reason":"target_preparation","schema":"gomadv3.campaign-lifecycle/v1","schema_version":1,"state":"recoverable-failure"}`
