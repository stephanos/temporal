...
type ArtifactCapacityPlan struct {
	FailureArtifacts evidence.Uint64String  `json:"failure_artifacts"`
	FailureBytes     evidence.Uint64String  `json:"failure_bytes"`
	SuccessArtifacts evidence.Uint64String  `json:"success_artifacts"`
	SuccessBytes     evidence.Uint64String  `json:"success_bytes"`
	TotalBytes       evidence.Uint64String  `json:"total_bytes"`
	TranscriptBytes  evidence.Uint64String  `json:"transcript_bytes"`
...
func DeriveArtifactCapacityPlan(plan CampaignPlan) (ArtifactCapacityPlan, error) {
	...
	return ArtifactCapacityPlan{
		FailureArtifacts: evidence.Uint64String(maximumRuns), FailureBytes: evidence.Uint64String(failureBytes),
		SuccessArtifacts: plan.SuccessArtifactLimit, SuccessBytes: plan.SuccessBytesLimit,
		TotalBytes: evidence.Uint64String(failureBytes + successBytes), TranscriptBytes: deterministicio.MaximumTranscriptBytes,
