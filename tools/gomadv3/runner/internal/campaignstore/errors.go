...
const (
	JournalLimitRuns          JournalLimit = "runs"
	...
	JournalLimitPartialRuns   JournalLimit = "partial_runs"
...
func (err *JournalCapacityError) Error() string {
	return fmt.Sprintf("run journal %s requires %d, exceeding %d (%s)", err.Limit, err.Required, err.Maximum, err.Outcome)
}
