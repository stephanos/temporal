...
const (
	MergedCampaignSchema       = "gomadv3.merged-batch/v1"
...
type MergedShard struct {
	...
	BatchSHA256  evidence.SHA256       `json:"batch_sha256"`
	Attempted    evidence.Uint64String `json:"attempted"`
	JournalBytes evidence.Uint64String `json:"journal_bytes"`
}
...
type MergedEvidence struct {
	...
	RecordSHA256     evidence.SHA256       `json:"record_sha256"`
	StoredBytes      evidence.Uint64String `json:"stored_bytes"`
}

type OrdinalRange struct {
	Start evidence.Uint64String `json:"start"`
	End   evidence.Uint64String `json:"end"`
}
...
type MergedCampaignRecord struct {
	...
	RetainedSuccesses    evidence.Uint64String `json:"retained_successes"`
	RetainedSuccessBytes evidence.Uint64String `json:"retained_success_bytes"`
	RetainedEvidence     evidence.Uint64String `json:"retained_evidence"`
	EvidenceBytes        evidence.Uint64String `json:"evidence_bytes"`
	Journal              RunJournalReference   `json:"journal"`
...
func MergeCampaigns(ctx context.Context, spec MergeSpec) (_ MergedCampaign, retErr error) {
	...
	limits := runJournalLimitsFromPlan(spec.Journal)
	if _, err := normalizeRunJournalLimits(CampaignConfig{Journal: limits}); err != nil || limits.MaximumRuns != spec.SelectionCount {
	...
	journal, err := newSegmentedRunJournal(ctx, staging, limits)
	if err != nil {
		return MergedCampaign{}, err
	}
	...
	record := MergedCampaignRecord{
		Schema: MergedCampaignSchema, SchemaVersion: evidence.SchemaVersion, PlanSHA256: spec.PlanSHA256, Selection: spec.Selection, SelectionCount: evidence.Uint64String(spec.SelectionCount),
		Partial: len(missing) != 0, Missing: missing, Attempted: evidence.Uint64String(len(runs)), Succeeded: evidence.Uint64String(summary.succeeded), Failures: evidence.Uint64String(summary.failures), Watchdogs: evidence.Uint64String(summary.watchdogs), Cancelled: evidence.Uint64String(summary.cancelled),
		DistinctFailures: evidence.Uint64String(len(summary.failuresSeen)), RetainedSuccesses: evidence.Uint64String(summary.retainedSuccesses), RetainedSuccessBytes: evidence.Uint64String(summary.retainedSuccessBytes), RetainedEvidence: evidence.Uint64String(summary.retainedEvidence), EvidenceBytes: evidence.Uint64String(summary.evidenceBytes),
		Journal: reference, Artifacts: spec.Artifacts, Shards: shards,
	}
	manifest, err := evidence.CanonicalJSON(record)
	if err != nil {
		return MergedCampaign{}, err
	}
	...
	if err := evidence.RenameNoReplace(staging, output); err != nil {
	...
	return MergedCampaign{Path: output, Record: record, Runs: runs}, nil
}
...
type mergedSummary struct {
	...
	failuresSeen                              map[evidence.SHA256]struct{}
}

func collectMergedCampaign(spec MergeSpec) ([]MergedShard, []MergedRun, mergedSummary, error) {
	paths := append([]string(nil), spec.ShardPaths...)
	sort.Strings(paths)
	shards := make([]MergedShard, 0, len(paths))
	runs := make([]MergedRun, 0)
	retainedByIdentity := make(map[evidence.SHA256]MergedEvidence)
	assignments := make(map[uint64]struct{}, len(paths))
	ordinals := make(map[uint64]struct{})
	summary := mergedSummary{failuresSeen: make(map[evidence.SHA256]struct{})}
	...
	for _, path := range paths {
		...
		batch := opened.Record
		if batch.Schema != "gomadv3.batch/v4" || batch.PlanSHA256 != spec.PlanSHA256 || batch.Selection != spec.Selection || uint64(batch.SelectionCount) != spec.SelectionCount || batch.Shard == nil {
			return nil, nil, mergedSummary{}, fmt.Errorf("shard batch %s does not match the campaign plan", path)
		}
		...
		batchBytes, err := evidence.CanonicalJSON(batch)
		if err != nil {
			return nil, nil, mergedSummary{}, err
		}
		...
		shards = append(shards, MergedShard{Index: batch.Shard.Index, Count: batch.Shard.Count, CampaignID: batch.CampaignID, BatchSHA256: evidence.HashBytes(batchBytes), Attempted: batch.Attempted, JournalBytes: evidence.Uint64String(journalBytes)})
		for _, run := range opened.Runs {
			...
			if !found || seed != uint64(run.Seed) || ordinal%count != index {
				return nil, nil, mergedSummary{}, fmt.Errorf("shard run ordinal %d does not match the campaign plan", ordinal)
			}
			...
			merged := MergedRun{SourceCampaignID: batch.CampaignID, Run: run}
			if run.FailureSignature != nil || run.SuccessArtifact != nil {
				...
				candidate := MergedEvidence{SHA256: identity, Kind: resolved.Manifest.ArtifactKind, SourceCampaignID: batch.CampaignID, Reference: reference, RecordSHA256: resolved.Manifest.RecordHash, StoredBytes: evidence.Uint64String(resolved.StoredBytes)}
	...
	sort.Slice(runs, func(i, j int) bool { return runs[i].Run.SelectionOrdinal < runs[j].Run.SelectionOrdinal })
	...
	for _, item := range retainedByIdentity {
		...
		if item.Kind == evidence.ArtifactSuccess {
			successCount++
	...
	for index := range runs {
		...
		reference := runs[index].Run.Artifact
		if runs[index].Run.SuccessArtifact != nil {
			reference = runs[index].Run.SuccessArtifact
		}
...
func mergedEvidenceIdentity(manifest evidence.ExecutionRecord) (evidence.SHA256, error) {
	...
	return evidence.HashBytes(encoded), nil
}
...
func missingOrdinalRanges(total uint64, runs []MergedRun) []OrdinalRange {
	missing := make([]OrdinalRange, 0)
	runIndex := 0
	for ordinal := uint64(0); ordinal < total; {
		if runIndex < len(runs) && uint64(runs[runIndex].Run.SelectionOrdinal) == ordinal {
			runIndex++
			ordinal++
			continue
		}
		start := ordinal
		for ordinal < total && (runIndex >= len(runs) || uint64(runs[runIndex].Run.SelectionOrdinal) != ordinal) {
			ordinal++
		}
		missing = append(missing, OrdinalRange{Start: evidence.Uint64String(start), End: evidence.Uint64String(ordinal - 1)})
	}
	return missing
}
...
func OpenMergedCampaign(path string) (_ MergedCampaign, retErr error) {
	...
	if len(entries) != 2 || entries[0].Name() != "merge.json" && entries[1].Name() != "merge.json" || entries[0].Name() != "runs" && entries[1].Name() != "runs" {
	...
	var record MergedCampaignRecord
	if err := evidence.DecodeCanonicalJSON(manifest, &record); err != nil {
		return MergedCampaign{}, err
	}
	canonical, err := evidence.CanonicalJSON(record)
	if err != nil || !bytes.Equal(canonical, manifest) || record.Schema != MergedCampaignSchema || record.SchemaVersion != evidence.SchemaVersion || !validRecordSHA256(record.PlanSHA256) || record.Selection == "" || record.SelectionCount == 0 {
		return MergedCampaign{}, errors.Join(errors.New("merged campaign record is invalid"), err)
	}
	runs, err := readMergedRunJournal(root, record.Journal)
	if err != nil {
		return MergedCampaign{}, classifyIntegrityError(err)
	}
	if err := validateMergedCampaign(record, runs); err != nil {
		return MergedCampaign{}, newIntegrityError(err)
	}
	return MergedCampaign{Path: path, Record: record, Runs: runs}, nil
}

func validateMergedCampaign(record MergedCampaignRecord, runs []MergedRun) error {
	missing := missingOrdinalRanges(uint64(record.SelectionCount), runs)
	if uint64(len(runs)) != uint64(record.Attempted) || !reflect.DeepEqual(missing, record.Missing) || record.Partial != (len(missing) != 0) || record.Journal.Records != record.Attempted {
		return errors.New("merged campaign summary is inconsistent")
	}
	if !sort.SliceIsSorted(record.Shards, func(i, j int) bool { return record.Shards[i].Index < record.Shards[j].Index }) {
		return errors.New("merged campaign shards are not sorted")
	}
	campaigns := make(map[string]struct{}, len(record.Shards))
	assignments := make(map[uint64]struct{}, len(record.Shards))
	var shardCount, attempted uint64
	for _, shard := range record.Shards {
		if shard.Count == 0 || shard.Index >= shard.Count || shard.CampaignID == "" || !validRecordSHA256(shard.BatchSHA256) {
	...
	if attempted != uint64(record.Attempted) {
	...
	for index, run := range runs {
		if _, found := campaigns[run.SourceCampaignID]; !found {
			return errors.New("merged run source campaign is unknown")
		}
		if run.Run.FailureSignature != nil {
			failures[*run.Run.FailureSignature] = struct{}{}
			if run.EvidenceSHA256 != *run.Run.FailureSignature {
				return errors.New("merged failure evidence identity does not match its signature")
			}
		}
		hasRetained := run.Run.FailureSignature != nil || run.Run.SuccessArtifact != nil
		if hasRetained != (run.EvidenceSHA256 != "") || run.EvidenceSHA256 != "" && !validRecordSHA256(run.EvidenceSHA256) {
			return errors.New("merged run evidence identity is invalid")
		}
		executions[index] = run.Run
	}
	failureSignatures := make([]evidence.SHA256, 0, len(failures))
	...
	validationRecord := CampaignRecord{
		SchemaVersion: evidence.SchemaVersion, Schema: "gomadv3.batch/v3", CampaignID: "merged-validation", Strategy: "seed", Selection: record.Selection, SelectionCount: record.SelectionCount,
		Attempted: record.Attempted, Succeeded: record.Succeeded, Failures: record.Failures, Watchdogs: record.Watchdogs, Cancelled: record.Cancelled, DistinctFailures: record.DistinctFailures,
		RetainedSuccesses: record.RetainedSuccesses, RetainedSuccessBytes: record.RetainedSuccessBytes, StopReason: "seeds_exhausted", Journal: &record.Journal, Artifacts: &record.Artifacts, FailureSignatures: failureSignatures,
	}
	if err := validateCampaign(validationRecord, executions); err != nil {
		return fmt.Errorf("validate merged runs: %w", err)
	}
	if uint64(record.DistinctFailures) != uint64(len(failures)) {
		return errors.New("merged evidence summary is invalid")
	}
	evidenceByIdentity := make(map[evidence.SHA256]struct{}, uint64(record.RetainedEvidence))
	var failureCount, failureBytes, successCount, successBytes uint64
	for _, mergedRun := range runs {
		...
		if retained.SHA256 != mergedRun.EvidenceSHA256 || retained.SourceCampaignID != mergedRun.SourceCampaignID {
			return errors.New("merged evidence is attached to the wrong run")
		}
		reference := mergedRun.Run.Artifact
		if mergedRun.Run.SuccessArtifact != nil {
			reference = mergedRun.Run.SuccessArtifact
		}
		if reference == nil || retained.Reference != *reference {
			return errors.New("merged evidence reference does not match its run")
		}
		...
		if retained.Kind == evidence.ArtifactSuccess {
			...
			if retained.Kind != evidence.ArtifactTargetFailure && retained.Kind != evidence.ArtifactWatchdogTimeout && retained.Kind != evidence.ArtifactRunnerFailure || !validArtifactReference(retained.Reference) {
	...
	if uint64(len(evidenceByIdentity)) != uint64(record.RetainedEvidence) {
	...
	for _, run := range runs {
		if run.EvidenceSHA256 != "" {
			if _, found := evidenceByIdentity[run.EvidenceSHA256]; !found {
				return errors.New("merged run evidence is not retained")
			}
		}
	}
	if err := validateMergedArtifactCapacity(record.Artifacts, failureCount, failureBytes, successCount, successBytes); err != nil {
		return err
	}
	if successBytes > ^uint64(0)-failureBytes || failureBytes+successBytes != uint64(record.EvidenceBytes) {
		return errors.New("merged evidence byte summary is inconsistent")
	}
	return nil
}

func readMergedRunJournal(root *os.Root, reference RunJournalReference) ([]MergedRun, error) {
	if reference.Schema != runJournalSchema || reference.IndexFile != "runs/index.json" || !validRecordSHA256(reference.IndexSHA256) {
		return nil, errors.New("merged run journal reference is invalid")
	}
	indexBytes, err := readValidatedFile(root, reference.IndexFile, 0o600, maximumRunJournalIndexBytes)
	if err != nil || evidence.HashBytes(indexBytes) != reference.IndexSHA256 {
		return nil, errors.Join(errors.New("merged run journal index identity changed"), err)
	}
	...
	runs := make([]MergedRun, 0, uint64(index.Records))
	for segmentIndex, segment := range index.Segments {
		name := fmt.Sprintf("%020d.jsonl", segmentIndex)
		if segment.File != name {
			return nil, fmt.Errorf("merged run segment sequence has a gap at %s", name)
		}
		contents, err := readValidatedFile(root, filepath.Join("runs", name), 0o600, uint64(segment.Bytes))
		if err != nil || evidence.HashBytes(contents) != segment.SHA256 {
			return nil, errors.Join(fmt.Errorf("merged run segment %s identity changed", name), err)
		}
		decoded, err := decodeMergedRuns(contents)
		if err != nil || uint64(len(decoded)) != uint64(segment.Records) {
			return nil, errors.Join(fmt.Errorf("decode merged run segment %s", name), err)
		}
		runs = append(runs, decoded...)
	}
	for index := range runs {
		if index != 0 && runs[index-1].Run.SelectionOrdinal >= runs[index].Run.SelectionOrdinal {
			return nil, errors.New("merged runs are not in canonical ordinal order")
		}
	}
	return runs, nil
}

func decodeMergedRuns(contents []byte) ([]MergedRun, error) {
	if len(contents) == 0 {
		return []MergedRun{}, nil
	}
	if contents[len(contents)-1] != '\n' {
		return nil, errors.New("merged run segment is not newline terminated")
	}
	lines := bytes.Split(contents[:len(contents)-1], []byte{'\n'})
	runs := make([]MergedRun, len(lines))
	for index, line := range lines {
		if len(line) == 0 {
			return nil, fmt.Errorf("merged run segment has an empty record at line %d", index+1)
		}
