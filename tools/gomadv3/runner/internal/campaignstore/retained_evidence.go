...
func ResolveRetainedEvidence(batchPath, batchID string, run ExecutionRecord) (RetainedEvidence, error) {
	...
	if success {
		if manifest.SelectionOrdinal != run.SelectionOrdinal || manifest.Seed != run.Seed || manifest.ArtifactKind != evidence.ArtifactSuccess || manifest.Outcome.Domain != "success" || retained.StoredBytes != uint64(*run.SuccessArtifactBytes) {
			return RetainedEvidence{}, retainedMismatchError(true)
		}
	} else {
		if manifest.ArtifactKind != evidence.ArtifactTargetFailure && manifest.ArtifactKind != evidence.ArtifactWatchdogTimeout && manifest.ArtifactKind != evidence.ArtifactRunnerFailure {
...
func retainedChoiceMatches(run ExecutionRecord, manifest evidence.ExecutionRecord) bool {
