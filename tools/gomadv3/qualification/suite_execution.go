...
func retainedAnalysis(result SuiteCommandResult) (AnalysisReport, string, error) {
	if result.Err != nil {
		classification := retainedErrorClassification(result, result.Err)
		return AnalysisReport{}, classification, result.Err
	}
	if len(result.Stderr) != 0 {
		return AnalysisReport{}, "runner_failure", errors.New("JSON capability analysis wrote to stderr")
	}
	if len(result.Stdout) == 0 || result.Stdout[len(result.Stdout)-1] != '\n' || bytes.Count(result.Stdout, []byte{'\n'}) != 1 {
		return AnalysisReport{}, "runner_failure", errors.New("capability analysis output framing is invalid")
	}
	report, err := DecodeAnalysisReport(result.Stdout[:len(result.Stdout)-1])
	if err != nil {
		return AnalysisReport{}, "runner_failure", err
	}
	expectedStatus := 0
	...
	if result.ExitCode != expectedStatus {
		return AnalysisReport{}, "runner_failure", fmt.Errorf("capability analysis status is %d, want %d", result.ExitCode, expectedStatus)
	}
	return report, string(report.Classification), nil
}

func mergeAnalysisIdentity(report *SuiteReport, analysis AnalysisReport) error {
...
func matchesUnsupportedAnalysis(expected WorkloadExpectation, analysis AnalysisReport) bool {
...
func matchesSupportedExpectation(expected WorkloadExpectation, suite WorkloadReport) bool {
...
func retainedErrorClassification(result SuiteCommandResult, err error) string {
	if result.Err != nil {
...
func projectSeedReport(report QualificationReport, classification string, seed uint64, suite Workload, analysis AnalysisReport) (SeedReport, error) {
	if uint64(report.Seed) != seed {
		return SeedReport{}, fmt.Errorf("qualification seed is %d, want %d", report.Seed, seed)
	}
	result := SeedReport{
		Seed: evidence.Uint64String(seed), Classification: classification, EvidenceSHA256: report.EvidenceDigest,
...
func mergeQualificationRun(result *SeedReport, run QualificationExecutionReport, index int, seed uint64, suite Workload, analysis AnalysisReport) error {
	...
	if run.Replay != nil {
		...
		if run.Replay.ChoiceReplayStatus != ChoiceReplayExact {
			result.ChoiceReplayExact = false
		}
		if !run.Replay.Match {
			result.ReplayMatch = false
	...
	opened, err := evidence.OpenArtifact(run.ArtifactPath)
	if err != nil {
	...
	if artifactErr == nil {
		result.ElapsedNanos += evidence.Uint64String(opened.Manifest.Host.ElapsedNanos)
		result.ArtifactBytes += evidence.Uint64String(opened.StoredBytes)
...
func validateArtifactIdentity(opened evidence.Artifact, seed uint64, analysis AnalysisReport) error {
	...
	if manifest.IOProfile.Name != analysis.IOProfile.Name || manifest.IOProfile.ImplementationSHA256 != evidence.SHA256(analysis.IOProfile.ImplementationSHA256) || manifest.IOProfile.InventorySHA256 != evidence.SHA256(analysis.IOProfile.InventorySHA256) {
		return errors.New("retained artifact I/O profile does not match capability analysis")
	}
	return nil
}

func projectArtifactChoice(opened evidence.Artifact) (ChoiceCoverage, error) {
	...
	return ChoiceCoverage{
		...
		AdjacentPairsObserved:  evidence.Uint64String(projection.Features.AdjacentPairsObserved),
...
func addSeedTotals(report *SuiteReport, seed SeedReport) {
...
func suiteTimeouts(manifest SuiteManifest, suite Workload) (runTimeout time.Duration, overallTimeout time.Duration) {
...
func manifestGrace(manifest SuiteManifest) time.Duration {
