...
const ComparisonSchema = "gomadv3.support-comparison/v1"

const ComparisonBoundaryDiffDomain = "gomadv3.support-boundary-diff/v1"

type ComparisonClassification string

const (
	ComparisonClean        ComparisonClassification = "clean"
	ComparisonImproved     ComparisonClassification = "improved"
	ComparisonRegressed    ComparisonClassification = "regressed"
	ComparisonIncomparable ComparisonClassification = "incomparable"
)

type ComparisonInput struct {
	Baseline             SuiteReport
	Candidate            SuiteReport
	ApprovedBoundaryDiff evidence.SHA256
}

type Comparison struct {
	Schema              string                   `json:"schema"`
	Classification      ComparisonClassification `json:"classification"`
...
type BoundaryComparison struct {
	...
	DiffSHA256       evidence.SHA256 `json:"diff_sha256,omitempty"`
	BaselineVersion  string          `json:"baseline_version"`
	BaselineSHA256   evidence.SHA256 `json:"baseline_sha256"`
...
type BlockerIdentity struct {
	...
	Blocker AnalysisBlocker `json:"blocker"`
}

type ChoiceChange struct {
	Comparable bool             `json:"comparable"`
	Reason     string           `json:"reason,omitempty"`
	Added      []choice.Feature `json:"added"`
	Removed    []choice.Feature `json:"removed"`
}

func Compare(input ComparisonInput) (Comparison, error) {
	if input.Baseline.Schema != SuiteReportSchema || input.Candidate.Schema != SuiteReportSchema {
		return Comparison{}, errors.New("support comparison requires normalized qualification-set reports")
	}
	result := Comparison{
		Schema: ComparisonSchema, IdentityDifferences: []IdentityDifference{}, Workloads: []WorkloadChange{},
	...
	if incomparable || workloads.membershipChanged {
		result.Classification = ComparisonIncomparable
	} else if workloads.regressed {
		result.Classification = ComparisonRegressed
	} else if workloads.improved {
		result.Classification = ComparisonImproved
	} else {
		result.Classification = ComparisonClean
	}
	result.ReviewRequired = result.Classification == ComparisonRegressed || result.Boundary.Changed && !result.Boundary.Approved
	return result, nil
}
...
func compareWorkloadSets(baselineReport, candidateReport SuiteReport) workloadComparison {
...
func workloadIdentities(baseline, candidate map[string]WorkloadReport) []string {
...
func compareWorkload(id string, baseline WorkloadReport, baselineFound bool, candidate WorkloadReport, candidateFound bool, baselineChoice, candidateChoice bool) workloadChangeResult {
	result := workloadChangeResult{change: WorkloadChange{
		ID: id, BlockersAdded: []BlockerIdentity{}, BlockersRemoved: []BlockerIdentity{},
		Choice: ChoiceChange{Added: []choice.Feature{}, Removed: []choice.Feature{}},
	}}
	if baselineFound {
...
func compareIdentities(result *Comparison, baseline, candidate SuiteReport) bool {
...
func compareBoundary(baseline, candidate SuiteReport, approval evidence.SHA256) BoundaryComparison {
	...
	payload, _ := evidence.CanonicalJSON(struct {
		BaselineVersion  string          `json:"baseline_version"`
		BaselineSHA256   evidence.SHA256 `json:"baseline_sha256"`
		CandidateVersion string          `json:"candidate_version"`
		CandidateSHA256  evidence.SHA256 `json:"candidate_sha256"`
	}{result.BaselineVersion, result.BaselineSHA256, result.CandidateVersion, result.CandidateSHA256})
	result.DiffSHA256 = evidence.DomainHash(ComparisonBoundaryDiffDomain, payload)
	result.Approved = approval != "" && approval == result.DiffSHA256
	return result
}

func workloadsByID(workloads []WorkloadReport) map[string]WorkloadReport {
	result := make(map[string]WorkloadReport, len(workloads))
...
func compareBlockers(baseline, candidate []AnalysisBlocker) (added []BlockerIdentity, removed []BlockerIdentity) {
...
func blockerIdentities(blockers []AnalysisBlocker) map[evidence.SHA256]BlockerIdentity {
	result := make(map[evidence.SHA256]BlockerIdentity, len(blockers))
	for _, blocker := range blockers {
		encoded, _ := evidence.CanonicalJSON(blocker)
		digest := evidence.DomainHash("gomadv3.support-blocker/v1", encoded)
...
func compareReplay(baseline, candidate []SeedReport) (regressed bool, improved bool) {
...
func replayBySeed(seeds []SeedReport) map[evidence.Uint64String]bool {
	result := make(map[evidence.Uint64String]bool, len(seeds))
...
func compareChoice(baselineDimension, candidateDimension bool, baseline, candidate ChoiceCoverage) ChoiceChange {
	result := ChoiceChange{Added: []choice.Feature{}, Removed: []choice.Feature{}}
	...
	left := make(map[choice.Feature]struct{}, len(baseline.Features))
	right := make(map[choice.Feature]struct{}, len(candidate.Features))
...
func sortFeatures(features []choice.Feature) {
...
func sumElapsed(seeds []SeedReport) uint64 {
	var result uint64
	for _, seed := range seeds {
...
func sumArtifacts(seeds []SeedReport) uint64 {
	var result uint64
	for _, seed := range seeds {
...
func sumTrace(seeds []SeedReport) uint64 {
	var result uint64
	for _, seed := range seeds {
...
func FormatComparisonText(result Comparison) string {
