...
func TestAnalyzeReviewsTheTargetBeforeBuildingTheClaim(t *testing.T) {
	...
	report, err := analyzeWith(context.Background(), AnalysisSpec{
...
func TestBuildRedactsPathBearingArgumentsDeterministically(t *testing.T) {
	...
	requireTestNoError(t, err)
	if strings.Contains(string(encoded), absolute) || !strings.Contains(string(encoded), string(evidence.HashBytes([]byte(absolute)))) {
...
func TestBuildUsesAllSortedRootsAndRejectsUnreachableFindings(t *testing.T) {
	...
	_, err := BuildAnalysis(AnalysisInput{
...
func TestDecodeValidatesCanonicalCapabilityReport(t *testing.T) {
	...
	requireTestNoError(t, err)
	if _, err := DecodeAnalysisReport(encoded); err == nil {
		t.Fatal("DecodeAnalysisReport() accepted unsupported analysis without blockers")
	}
}
...
func TestFormatTextGroupsEquivalentBlockersWithShortestPathFirst(t *testing.T) {
	report := AnalysisReport{
		...
		Closure:        AnalysisClosure{PackageCount: 3},
	...
	formatted := FormatAnalysisText(report)
