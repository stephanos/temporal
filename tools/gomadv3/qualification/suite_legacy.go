...
type legacySuiteReport struct {
	...
	StdoutSHA256   evidence.SHA256     `json:"stdout_sha256,omitempty"`
	StderrSHA256   evidence.SHA256     `json:"stderr_sha256,omitempty"`
	ReportPath     string              `json:"report_path,omitempty"`
	ReportSHA256   evidence.SHA256     `json:"report_sha256,omitempty"`
	Report         json.RawMessage     `json:"report,omitempty"`
	Error          string              `json:"error,omitempty"`
}

func decodeLegacySetReport(data []byte) (SuiteReport, error) {
	var legacy legacySetReport
	if err := evidence.DecodeCanonicalJSON(data, &legacy); err != nil {
		return SuiteReport{}, fmt.Errorf("decode legacy qualification set report: %w", err)
	}
	if err := validateLegacySetReport(legacy); err != nil {
		return SuiteReport{}, err
	}
	report := SuiteReport{
		Schema: SuiteReportSchema, Name: legacy.Name, ManifestSHA256: legacy.ManifestSHA256,
		...
		Suites: make([]WorkloadReport, len(legacy.Suites)),
	}
	for index, suite := range legacy.Suites {
		...
		if suite.ReportPath != "" {
			...
			if err != nil {
				return SuiteReport{}, fmt.Errorf("decode legacy workload %s qualification: %w", suite.Name, err)
			}
			seed := SeedReport{
				Seed: qualification.Seed, Classification: ClassifyQualification(qualification), EvidenceSHA256: qualification.EvidenceDigest,
...
func validateLegacySetReport(report legacySetReport) error {
	if report.Schema != LegacySuiteReportSchema || report.Name == "" || report.Selected != uint64(len(report.Manifest.Suites)) || report.Completed > report.Selected || len(report.Suites) > int(report.Selected) {
	...
	manifestBytes, err := evidence.CanonicalJSON(report.Manifest)
	if err != nil || evidence.HashBytes(manifestBytes) != report.ManifestSHA256 {
...
func summarizeLegacySuites(report legacySetReport) (legacyReportCounts, error) {
	...
	for index, suite := range report.Suites {
		...
		if suite.ReportPath != "" {
			counts.completed++
			qualification, err := DecodeQualificationReport(suite.Report)
			if err != nil || suite.Classification != ClassifyQualification(qualification) {
				return legacyReportCounts{}, errors.Join(fmt.Errorf("legacy qualification set workload %s evidence is invalid", suite.Name), err)
			}
			if evidence.HashBytes(append(append([]byte(nil), suite.Report...), '\n')) != suite.ReportSHA256 {
