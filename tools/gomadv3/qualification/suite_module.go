...
type ModuleIdentity struct {
	...
	GoModSHA256 evidence.SHA256 `json:"go_mod_sha256"`
}
...
func identifyModule(workingDirectory, expected string) (ModuleIdentity, error) {
	...
	file, info, err := hostfs.OpenPath(filepath.Join(resolved, "go.mod"))
	if err != nil {
	...
	return ModuleIdentity{Path: path, GoModSHA256: evidence.HashBytes(contents)}, nil
}
