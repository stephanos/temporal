...
type UpgradeDossier struct {
	...
	Version                UpgradeVersionEvidence          `json:"version"`
	Host                   UpgradeHostEvidence             `json:"host"`
	UpstreamPatch          UpgradePatchEvidence            `json:"upstream_patch"`
	BoundaryDiff           UpgradeBoundaryDiff             `json:"boundary_manifest_diff"`
	InterceptionReport     UpgradeInterceptionEvidence     `json:"interception_report"`
	OverlayCollision       UpgradeOverlayCollisionEvidence `json:"overlay_collision_report"`
	Gates                  []UpgradeGateResult             `json:"gates"`
	RetainedCorpus         UpgradeCorpusEvidence           `json:"retained_corpus"`
	MandatoryProbePolicy   string                          `json:"mandatory_probe_policy"`
}

type UpgradeVersionEvidence struct {
...
type UpgradeHostEvidence struct {
	Platform  string `json:"platform"`
	Supported bool   `json:"supported"`
}

type UpgradePatchEvidence struct {
	Path   string `json:"path"`
	SHA256 string `json:"sha256"`
	Diff   string `json:"diff"`
}

type UpgradeBoundaryDiff struct {
...
type UpgradeInterceptionEvidence struct {
	Path   string `json:"path"`
	SHA256 string `json:"sha256"`
	Report string `json:"report"`
}

type UpgradeOverlayCollisionEvidence struct {
...
type UpgradeCorpusEvidence struct {
	Status string          `json:"status"`
...
func Upgrade(ctx context.Context, options UpgradeSpec) error {
	...
	manifestPath := filepath.Join(options.Root, "deterministicio", "boundary", "manifest.json")
	...
	dossier := UpgradeDossier{
		...
		Version: UpgradeVersionEvidence{
		...
		Host:               UpgradeHostEvidence{Platform: hostPlatform, Supported: slices.Contains(descriptor.SupportedPlatforms, hostPlatform)},
		UpstreamPatch:      UpgradePatchEvidence{Path: descriptor.Patch, SHA256: digest(patch), Diff: string(patch)},
		BoundaryDiff:       difference,
		InterceptionReport: UpgradeInterceptionEvidence{Path: reportName, SHA256: digest(report), Report: string(report)},
		OverlayCollision:   collision,
		Gates:              []UpgradeGateResult{}, RetainedCorpus: corpus,
...
func boundaryDiffEmpty(difference UpgradeBoundaryDiff) bool {
	return len(difference.Added) == 0 && len(difference.Removed) == 0 && len(difference.Changed) == 0
}

func runGate(ctx context.Context, root string, gate UpgradeGate, output io.Writer) (UpgradeGateResult, error) {
	...
	if err == nil && !executed.WatchdogTimeout && !executed.Cancelled && executed.Termination == hostexec.TerminationExit && executed.ExitCode == 0 {
		result.Status = "passed"
		return result, nil
	}
	result.Status = "failed"
	result.ExitCode = -1
	if executed.Termination == hostexec.TerminationExit {
	...
	case executed.Termination == hostexec.TerminationSignal:
...
func compareBoundaries(baseline, current []byte) (UpgradeBoundaryDiff, error) {
	currentManifest, err := decodeBoundary("current", current)
	if err != nil {
		return UpgradeBoundaryDiff{}, err
	}
	result := UpgradeBoundaryDiff{Status: "not-requested", CurrentVersion: currentManifest.ManifestVersion, Added: []string{}, Removed: []string{}, Changed: []string{}}
	if len(baseline) == 0 {
...
func finalizeBoundaryDiff(difference UpgradeBoundaryDiff, baseline, current []byte) (UpgradeBoundaryDiff, error) {
	canonicalCurrent, err := canonicalBoundaryJSON(json.RawMessage(current))
	if err != nil {
		return UpgradeBoundaryDiff{}, fmt.Errorf("canonicalize current boundary diff input: %w", err)
	}
	var canonicalBaseline json.RawMessage
	if len(baseline) != 0 {
		canonicalBaseline, err = canonicalBoundaryJSON(json.RawMessage(baseline))
		if err != nil {
			return UpgradeBoundaryDiff{}, fmt.Errorf("canonicalize baseline boundary diff input: %w", err)
		}
	}
	...
	if err != nil {
		return UpgradeBoundaryDiff{}, fmt.Errorf("encode boundary diff identity: %w", err)
	}
...
func inspectOverlayCollision(root string, descriptor gomadversion.Descriptor) (UpgradeOverlayCollisionEvidence, error) {
	archivePath := filepath.Join(root, ".toolchain", "downloads", filepath.FromSlash(descriptor.Archive.Name))
	archive, err := os.ReadFile(archivePath)
	if err != nil {
		return UpgradeOverlayCollisionEvidence{}, fmt.Errorf("read qualified Go archive: %w", err)
	}
	if actual := strings.TrimPrefix(digest(archive), "sha256:"); actual != descriptor.Archive.SHA256 {
		return UpgradeOverlayCollisionEvidence{}, fmt.Errorf("qualified Go archive digest is %s, want %s", actual, descriptor.Archive.SHA256)
	}
	zipper, err := gzip.NewReader(bytes.NewReader(archive))
	if err != nil {
		return UpgradeOverlayCollisionEvidence{}, fmt.Errorf("open qualified Go archive: %w", err)
	}
	defer zipper.Close()
	...
	for {
		...
		if nextErr != nil {
			return UpgradeOverlayCollisionEvidence{}, fmt.Errorf("read qualified Go archive: %w", nextErr)
		}
	...
	result := UpgradeOverlayCollisionEvidence{
...
func loadCorpus(root, path string) (UpgradeCorpusEvidence, error) {
	if path == "" {
		return UpgradeCorpusEvidence{Status: "not-configured"}, nil
	}
	if !filepath.IsAbs(path) {
		path = filepath.Join(root, filepath.FromSlash(path))
	}
	report, err := qualification.OpenSuiteReport(path)
	if err != nil {
		return UpgradeCorpusEvidence{}, fmt.Errorf("validate retained qualification set: %w", err)
	}
	if !report.ExpectationsMet {
		return UpgradeCorpusEvidence{}, errors.New("retained qualification set did not meet its expectations")
	}
	if report.Name != "gomadv3-core" {
		return UpgradeCorpusEvidence{}, fmt.Errorf("retained qualification set %q is not the required gomadv3-core corpus", report.Name)
	}
	contents, err := os.ReadFile(path)
	if err != nil {
		return UpgradeCorpusEvidence{}, fmt.Errorf("read retained corpus report: %w", err)
	}
	return UpgradeCorpusEvidence{Status: "checked", Path: path, SHA256: digest(contents), Report: append(json.RawMessage(nil), contents...)}, nil
}

func publish(path string, dossier UpgradeDossier) error {
