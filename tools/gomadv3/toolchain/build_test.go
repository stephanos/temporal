...
func TestBuildRejectsUnsupportedHostBeforePreparingInputs(t *testing.T) {
	...
	dependencies.validate = func(PatchSpec) error {
...
func TestBuildRejectsOverlayCollisionBeforePatching(t *testing.T) {
	...
	dependencies.materialize = func(context.Context, PatchSpec) error {
...
func writeBuildFixture(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	for _, directory := range []string{"toolchain/runtime/overlay/src/runtime", "toolchain/version", "deterministicio/boundary"} {
...
`
	for name, contents := range map[string]string{
		"toolchain/version/version.json":                 descriptor,
		"toolchain/runtime/gomad.patch":                  "fixture patch\n",
		"toolchain/runtime/overlay/src/runtime/gomad.go": "package runtime\n",
	} {
...
func testConfig(root string) BuildSpec {
	return BuildSpec{
...
func fakeDependencies(t *testing.T, builds *atomic.Int64) dependencies {
	...
	return dependencies{
		...
		validate: func(PatchSpec) error { return nil },
...
func fakeRunner(t *testing.T, hostOS, hostArch string, builds *atomic.Int64, onBuild func()) func(context.Context, hostexec.Request) (hostexec.Result, error) {
	t.Helper()
	var buildOnce sync.Once
	return func(ctx context.Context, request hostexec.Request) (hostexec.Result, error) {
		...
		default:
			return hostexec.Result{}, fmt.Errorf("unexpected command %q", request.Command)
		}
	}
}

func commandSuccess(stdout string) hostexec.Result {
	return hostexec.Result{
		Termination: hostexec.TerminationExit, ExitCode: 0, GroupGone: true,
		Stdout: hostexec.Output{Bytes: []byte(stdout), RawBytes: []byte(stdout)},
	}
}
