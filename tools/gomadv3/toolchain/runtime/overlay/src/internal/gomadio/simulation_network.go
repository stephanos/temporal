...
func (network *simulationNetwork) snapshotLocked() (simulationSnapshot, error) {
	...
	payload := append([]byte("gomadv3-simulation-network-transitions/v2\x00"), encoded...)
	...
	snapshotDigest := sha256.Sum256(append([]byte("gomadv3-simulation-network-snapshot/v2\x00"), encodedSnapshot...))
...
func simulationTransitionSHA256(transition simulationTransition) (string, error) {
	...
	digest := sha256.Sum256(append([]byte("gomadv3-simulation-network-transition/v2\x00"), encoded...))
