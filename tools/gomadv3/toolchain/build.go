...
func buildWith(ctx context.Context, config BuildSpec, dependency dependencies) (returned BuildResult, returnedErr error) {
	...
	if config.FailurePhase != "" {
		if !config.Testing {
			return BuildResult{}, errors.New("gomadv3 builder failure injection requires testing mode")
		}
		if !slices.Contains(failurePhases, config.FailurePhase) {
			return BuildResult{}, fmt.Errorf("gomadv3 builder failure phase is invalid: %s", config.FailurePhase)
		}
	}
	...
	if !slices.Contains(descriptor.SupportedPlatforms, platform) {
		return BuildResult{}, fmt.Errorf("gomadv3 complete deterministic mode requires host %s; got %s", strings.Join(descriptor.SupportedPlatforms, ","), platform)
	}
	...
	if err := os.MkdirAll(config.ToolchainRoot, 0o755); err != nil {
		return BuildResult{}, fmt.Errorf("create gomadv3 toolchain root: %w", err)
	}
	if info, err := os.Lstat(config.ToolchainRoot); err != nil || info.Mode()&os.ModeSymlink != 0 || !info.IsDir() {
		return BuildResult{}, errors.Join(errors.New("gomadv3 toolchain root is not a real directory"), err)
	}
...
func resolveConfig(config BuildSpec) (BuildSpec, gomadversion.Descriptor, error) {
	if config.Root == "" {
		return BuildSpec{}, gomadversion.Descriptor{}, errors.New("gomadv3 module root is required")
	}
	root, err := filepath.Abs(config.Root)
	if err != nil || root == string(filepath.Separator) {
		return BuildSpec{}, gomadversion.Descriptor{}, errors.Join(errors.New("gomadv3 module root must be an absolute non-root directory"), err)
	}
	config.Root = root
	...
	if err != nil || config.ToolchainRoot == string(filepath.Separator) {
		return BuildSpec{}, gomadversion.Descriptor{}, errors.Join(errors.New("gomadv3 toolchain directory must be an absolute non-root path"), err)
	}
	if config.BootstrapGo == "" || config.BuildBash == "" || config.BuildPath == "" {
		return BuildSpec{}, gomadversion.Descriptor{}, errors.New("gomadv3 bootstrap Go, build Bash, and build PATH are required")
	}
	if config.BuildTimeout == 0 {
		config.BuildTimeout = defaultBuildTimeout
	}
	if config.BuildTimeout < 0 {
		return BuildSpec{}, gomadversion.Descriptor{}, errors.New("gomadv3 build timeout must be positive")
	}
