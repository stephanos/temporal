...
const generationSchema = "gomadv3.compatibility-pack-generation/v1"
...
func renderGeneration(requests map[string]Request) (renderedGeneration, error) {
	...
	for _, id := range ids {
		...
		requestBytes, err := evidence.CanonicalJSON(request)
		if err != nil {
		...
		requestSHA256 := string(evidence.DomainHash("gomadv3.compatibility-pack-request/v1", requestBytes))
		pack, err := projectPack(request, requestSHA256, approval, false)
		if err != nil {
			return renderedGeneration{}, err
		}
		packBytes, err := evidence.CanonicalJSON(pack)
		if err != nil {
	...
	for relative, contents := range files {
		outputs = append(outputs, generationStateOutput{Path: relative, SHA256: evidence.HashBytes(contents)})
	}
	sort.Slice(outputs, func(i, j int) bool { return outputs[i].Path < outputs[j].Path })
	state, err := evidence.CanonicalJSON(generationState{Schema: generationSchema, Outputs: outputs})
	if err != nil {
