...
import ("fmt"; "os")
func main() {
    before, beforeOK := os.LookupEnv("GOMADV3_HOST_ONLY")
    seed, seedOK := os.LookupEnv("GOMADSEED")
    if err := os.Setenv("GOMADV3_HOST_ONLY", "modeled"); err != nil { panic(err) }
    modeled, modeledOK := os.LookupEnv("GOMADV3_HOST_ONLY")
    if err := os.Unsetenv("GOMADV3_HOST_ONLY"); err != nil { panic(err) }
    _, afterOK := os.LookupEnv("GOMADV3_HOST_ONLY")
    fmt.Printf("%q %t %q %t %q %t %t\n", before, beforeOK, seed, seedOK, modeled, modeledOK, afterOK)
}
`)
	command := exec.Command(target)
	command.Env = append(os.Environ(), "GOMADSEED=1", "GOMADV3_HOST_ONLY=host")
	output, err := command.CombinedOutput()
