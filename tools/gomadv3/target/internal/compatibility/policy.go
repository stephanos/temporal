...
func validatePack(candidate pack) error {
	...
	for index, candidateRule := range candidate.Rules {
		...
		if candidateRule.SourceSetSHA256 != "" {
			if _, err := evidence.ParseSHA256(candidateRule.SourceSetSHA256); err != nil {
		...
		for _, linkname := range candidateRule.Linknames {
			if _, err := evidence.ParseSHA256(linkname.SHA256); linkname.Source == "" || err != nil || len(linkname.Directives) == 0 {
