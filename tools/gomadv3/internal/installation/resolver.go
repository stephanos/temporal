...
const ManifestName = "gomadv3-install.json"

const manifestSchema = "gomadv3.installation/v1"
...
func Resolve(config Config) (Resolution, error) {
	executable, err := filepath.Abs(config.Executable)
	if err != nil {
		return Resolution{}, fmt.Errorf("resolve gomad executable path: %w", err)
	}
	...
	if config.EnvironmentToolchainRoot != "" {
		return overrideResolution(config.EnvironmentToolchainRoot, "GOMADV3_TOOLCHAIN_DIR", "environment")
	}
	executableDirectory := filepath.Dir(executable)
	for _, path := range []string{
		filepath.Join(executableDirectory, ManifestName),
		filepath.Join(filepath.Dir(executableDirectory), ManifestName),
	} {
	...
	return Resolution{
		...
		RepairInstruction: "install the Gomad toolchain at " + root + " or set GOMADV3_TOOLCHAIN_DIR",
	}, nil
}

func overrideResolution(root, name, source string) (Resolution, error) {
