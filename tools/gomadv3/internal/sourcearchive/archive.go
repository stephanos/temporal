...
func Ensure(ctx context.Context, config Config) (string, error) {
...
func Extract(ctx context.Context, archivePath, destination string) error {
	...
	file, _, err := safefile.OpenPath(archivePath)
	if err != nil {
...
func matchesDigest(filePath, want string) (bool, error) {
	file, info, err := safefile.OpenPath(filePath)
	if err != nil {
		return false, err
	}
	defer file.Close()
...
func download(ctx context.Context, client *http.Client, config Config, archivePath string) error {
