...
const manifestPath = "deterministicio/boundary/manifest.json"

const compilerTestManifestPath = "deterministicio/boundary/compiler-tests.json"
...
func InterceptionPackages(root string) ([]string, error) {
	definition, err := load(filepath.Join(root, filepath.FromSlash(manifestPath)))
	if err != nil {
		return nil, err
	}
	seen := make(map[string]struct{}, len(definition.Intercepts))
...
func GenerateCompilerTestOverlay(root, goroot, output string) error {
	...
	installedSpecPath := filepath.Join(goroot, filepath.FromSlash(strings.TrimPrefix(relativeSpecPath, "toolchain/runtime/overlay/")))
	...
	if err := hostfs.Replace(testSpecPath, testSpec, 0o644); err != nil {
	...
	if err := hostfs.Replace(filepath.Join(output, "overlay.json"), overlay, 0o644); err != nil {
...
func validateHookPolicy(policy hookPolicy) error {
	...
	prefix := "toolchain/runtime/overlay/src/" + policy.Package + "/"
...
func renderHookPolicy(policy hookPolicy, intercepts []intercept) ([]byte, error) {
	...
	source.WriteString("// Code generated by cmd/gomadtool boundary-generate. DO NOT EDIT.\n\n")
...
func compilerSpecPath(goVersion string) (string, error) {
	...
	return fmt.Sprintf("toolchain/runtime/overlay/src/cmd/compile/internal/gomadintercept/spec_go%s%s.go", version[1], version[2]), nil
}
