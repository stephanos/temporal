...
func Regenerate(ctx context.Context, config Config) error {
	...
	config.Root = root
	descriptor, _, overlayRoot, err := resolve(config)
	if err != nil {
		return err
	}
	...
	if err := sourcearchive.Extract(ctx, archivePath, extracted); err != nil {
		return err
	}
	...
	if len(changed) == 0 {
		return errors.New("gomadv3 patch candidate contains no changes")
	}
	...
	if err != nil {
		return fmt.Errorf("generate gomadv3 patch: %w", err)
	}
	if result.Stdout.Truncated {
		return fmt.Errorf("regenerated gomadv3 patch exceeds %d bytes", maximumPatchBytes)
	}
	patch := result.Stdout.RawBytes
	if len(patch) == 0 {
		return errors.New("gomadv3 patch regeneration produced no changes")
	}
...
func validateCandidateVersion(root, want string) error {
	...
	if err != nil || info.Mode()&os.ModeSymlink != 0 || !info.IsDir() {
		return errors.Join(errors.New("gomadv3 patch candidate is not a real directory"), err)
	}
	file, info, err := safefile.OpenPath(filepath.Join(root, "VERSION"))
	if err != nil {
		return fmt.Errorf("gomadv3 patch candidate must be %s: %w", want, err)
	}
	defer file.Close()
	if info.Size() <= 0 || info.Size() > 128 {
		return fmt.Errorf("gomadv3 patch candidate must be %s", want)
	}
...
func resolveArchive(ctx context.Context, config Config, name, url, digest string) (string, error) {
	if config.Archive != "" {
		actual, err := buildkey.FileDigest(config.Archive)
		if err != nil {
	...
	return sourcearchive.Ensure(ctx, sourcearchive.Config{
...
func changedFiles(ctx context.Context, pristineRoot, candidateRoot string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("inspect gomadv3 patch candidate: %w", err)
	}
	var paths []string
	...
	for name := range candidate {
		if _, found := pristine[name]; !found {
			return nil, fmt.Errorf("gomadv3 patch candidate adds a source path: %s", name)
		}
	}
	slices.Sort(paths)
	var changed []string
	for _, name := range paths {
		...
		if !found {
			return nil, fmt.Errorf("gomadv3 patch candidate deletes a source path: %s", name)
		}
		pristineDigest, err := buildkey.FileDigest(pristine[name])
		if err != nil {
			return nil, err
		}
		candidateDigest, err := buildkey.FileDigest(candidatePath)
		if err != nil {
			return nil, err
		}
...
func sourceFiles(ctx context.Context, root string) (map[string]string, error) {
	...
	err := filepath.WalkDir(root, func(filePath string, entry os.DirEntry, walkErr error) error {
		...
		if !entry.Type().IsRegular() {
			return fmt.Errorf("gomadv3 patch candidate contains a non-regular entry: %s", filePath)
		}
...
func prepareDiffTree(ctx context.Context, pristineRoot, candidateRoot, gofmt string, changed []string) error {
	if _, err := run(ctx, pristineRoot, []string{"git", "init", "-q"}, nil); err != nil {
	...
	for _, relative := range changed {
		...
		if strings.HasSuffix(relative, ".go") {
			if _, err := run(ctx, pristineRoot, []string{gofmt, "-w", relative}, nil); err != nil {
...
func publishRegeneratedPatch(ctx context.Context, root, pristineRoot, output string, patch []byte, descriptor gomadversion.Descriptor) error {
	...
	temporary, err := os.CreateTemp(directory, ".gomadv3-patch-*")
	if err != nil {
	...
	if _, err := run(ctx, pristineRoot, []string{"git", "apply", "--cached", "--check", temporaryPath}, nil); err != nil {
