...
func Validate(config Config) error {
	descriptor, patchPath, overlayRoot, err := resolve(config)
	if err != nil {
		return err
	}
...
func Materialize(ctx context.Context, config Config) error {
	if config.SourceRoot == "" {
		return errors.New("patch materialization requires a source root")
	}
	descriptor, patchPath, _, err := resolve(config)
	if err != nil {
		return err
	}
	...
	if _, err := run(ctx, sourceRoot, command, patch); err != nil {
...
func resolve(config Config) (gomadversion.Descriptor, string, string, error) {
	if config.Root == "" {
...
func validatePatch(root, path string, descriptor gomadversion.Descriptor) ([]byte, error) {
	...
	result, err := run(context.Background(), root, []string{"git", "apply", "--numstat"}, contents)
...
func run(ctx context.Context, dir string, command []string, stdin []byte) (commandrun.Result, error) {
	return runLimit(ctx, dir, command, stdin, maximumCommandOutput)
}

func runLimit(ctx context.Context, dir string, command []string, stdin []byte, outputLimit uint64) (commandrun.Result, error) {
	result, err := commandrun.Run(ctx, commandrun.Request{
		Command: command, Dir: dir, Env: os.Environ(), Stdin: bytes.NewReader(stdin), Timeout: commandTimeout,
		TerminateGrace: commandTerminationGrace, OutputLimit: outputLimit,
	})
	if err != nil {
		return result, err
	}
