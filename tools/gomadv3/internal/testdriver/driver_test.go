...
func TestRunUpstreamExecutesTypedGatesInOrder(t *testing.T) {
	...
	report, err := runWith(context.Background(), Config{Root: root, Mode: "test-upstream", Go: goCommand}, func(_ context.Context, request commandrun.Request) (commandrun.Result, error) {
		requests = append(requests, request)
		if len(request.Command) == 3 && request.Command[1] == "env" {
			result := successfulCommand()
			result.Stdout = outputcapture.Output{Bytes: []byte(goRoot + "\n"), RawBytes: []byte(goRoot + "\n")}
			return result, nil
		}
...
func TestRunBuilderExecutesTypedPackageGate(t *testing.T) {
	...
	var requests []commandrun.Request
	report, err := runWith(context.Background(), Config{Root: root, Mode: "test-builder", Go: goCommand}, func(_ context.Context, request commandrun.Request) (commandrun.Result, error) {
		requests = append(requests, request)
		if len(request.Command) == 3 && request.Command[1] == "env" {
			result := successfulCommand()
			goRoot := filepath.Join(root, "go")
	...
	if got := requests[1].Command; !slices.Equal(got, []string{goCommand, "test", "-count=1", "-tags=test_dep", "./internal/toolchainbuild"}) {
...
func TestRunAcceptsExecutableSymlink(t *testing.T) {
	...
	_, err := runWith(context.Background(), Config{Root: root, Mode: "test-builder", Go: link}, func(_ context.Context, request commandrun.Request) (commandrun.Result, error) {
		result := successfulCommand()
		if len(request.Command) == 3 && request.Command[1] == "env" {
			goRoot := filepath.Join(root, "go")
			result.Stdout = outputcapture.Output{Bytes: []byte(goRoot + "\n"), RawBytes: []byte(goRoot + "\n")}
...
func TestRunUpstreamStopsAtFailedGateWithBoundedEvidence(t *testing.T) {
	root := writeGoFixture(t)
	calls := 0
	report, err := runWith(context.Background(), Config{Root: root, Mode: "test-upstream", Go: filepath.Join(root, "go", "bin", "go")}, func(context.Context, commandrun.Request) (commandrun.Result, error) {
		calls++
		if calls == 1 {
			result := successfulCommand()
			goRoot := filepath.Join(root, "go")
		...
		return commandrun.Result{
			Termination: commandrun.TerminationExit, ExitCode: 2,
			Stderr: outputcapture.Output{Bytes: []byte("failed"), RawBytes: []byte("failed")},
		}, nil
	})
...
func TestRunReportsInfrastructureError(t *testing.T) {
	...
	_, err := runWith(context.Background(), Config{Root: root, Mode: "test-upstream", Go: filepath.Join(root, "go", "bin", "go")}, func(context.Context, commandrun.Request) (commandrun.Result, error) {
		return commandrun.Result{}, want
	})
...
func TestRunInterceptionExecutesManifestDrivenCompilerCases(t *testing.T) {
	...
	var requests []commandrun.Request
	report, err := runWith(context.Background(), Config{Root: root, Mode: "test-interception", Go: goCommand, Compiler: compiler}, func(_ context.Context, request commandrun.Request) (commandrun.Result, error) {
		requests = append(requests, request)
		...
		if strings.Contains(joined, "install -a") && strings.HasSuffix(joined, " os") && !slices.Contains(request.Env, "GOOS=linux") {
			...
			result.Stderr = outputcapture.Output{Bytes: []byte(applied.String()), RawBytes: []byte(applied.String())}
			return result, nil
		}
		for name, diagnostic := range diagnostics {
			if strings.Contains(joined, "./interceptfail/"+name) && strings.Contains(joined, "-toolexec=") {
				return commandrun.Result{
					Termination: commandrun.TerminationExit, ExitCode: 1,
					Stderr: outputcapture.Output{Bytes: []byte(diagnostic), RawBytes: []byte(diagnostic)},
				}, nil
			}
		}
		return successfulCommand(), nil
...
func successfulCommand() commandrun.Result {
	return commandrun.Result{Termination: commandrun.TerminationExit, ExitCode: 0, GroupGone: true}
}
