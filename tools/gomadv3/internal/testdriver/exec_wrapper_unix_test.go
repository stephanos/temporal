...
func TestExecWrapperRequiresSeedAndCommand(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if result.Termination != commandrun.TerminationExit || result.ExitCode != 125 || !strings.Contains(string(result.Stderr.Bytes), test.want) {
...
func TestExecWrapperPreservesTargetExitAndSignal(t *testing.T) {
	...
	if exited.Termination != commandrun.TerminationExit || exited.ExitCode != 37 {
		t.Fatalf("exit result = %#v", exited)
	}
	signaled := runCommand(t, []string{wrapper, "/bin/sh", "-c", "kill -TERM $$"}, environment)
	if signaled.Termination != commandrun.TerminationSignal || signaled.SignalNumber != 15 {
...
func runCommand(t *testing.T, command, environment []string) commandrun.Result {
	t.Helper()
