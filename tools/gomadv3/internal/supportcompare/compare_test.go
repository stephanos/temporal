...
func TestCompareClassifiesSupportTransitions(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "clean", baseline: "qualified", candidate: "qualified", classification: ClassificationClean},
		{name: "improved", baseline: "unsupported_target", candidate: "qualified", classification: ClassificationImproved},
		{name: "regressed", baseline: "qualified", candidate: "unsupported_target", classification: ClassificationRegressed},
	} {
...
func TestCompareMarksLegacyEvidenceIncomparable(t *testing.T) {
	...
	if result.Classification != ClassificationIncomparable || !strings.Contains(FormatText(result), "portable_v3") {
		t.Fatalf("comparison = %#v", result)
	}
}
...
func TestCompareRequiresReviewForAnyChangedUnsupportedBlocker(t *testing.T) {
	...
	baseline.Suites[0].Blockers = []capabilityanalysis.Blocker{first, second}
	candidate := comparisonReport("unsupported_target")
	candidate.Suites[0].Blockers = []capabilityanalysis.Blocker{second}

	result, err := Compare(Input{Baseline: baseline, Candidate: candidate})
	if err != nil {
		t.Fatal(err)
	}
	if result.Classification != ClassificationRegressed || !result.ReviewRequired || len(result.Workloads[0].BlockersRemoved) != 1 || len(result.Workloads[0].BlockersAdded) != 0 {
...
func TestCompareMarksWorkloadMembershipChangesIncomparable(t *testing.T) {
	...
	empty.Completed = 0
	empty.Supported = 0
	empty.Suites = []qualificationset.SuiteReport{}
	for _, input := range []Input{
		...
		if result.Classification != ClassificationIncomparable {
...
func TestCompareReportsUpgradeIdentitiesWithoutLosingSupportComparison(t *testing.T) {
	...
	candidate.IOProfile.ImplementationSHA256 = record.HashBytes([]byte("changed io"))
	result, err := Compare(Input{Baseline: baseline, Candidate: candidate})
	if err != nil {
		t.Fatal(err)
	}
	if result.Classification != ClassificationClean || result.ReviewRequired || len(result.IdentityDifferences) != 5 {
...
func comparisonBlocker(importPath, capability string) capabilityanalysis.Blocker {
	return capabilityanalysis.Blocker{CapabilityFinding: target.CapabilityFinding{
		...
		Capability: capability, Directives: []string{}, PolicyDisposition: compatibility.DispositionDenied,
		Remediation: compatibility.RemediationRemainUnsupported,
	}}
}

func comparisonReport(classification string) qualificationset.SetReport {
	boundary := record.HashBytes([]byte("boundary"))
	report := qualificationset.SetReport{
		...
		Suites: []qualificationset.SuiteReport{{
			ID: "fixture-case", Name: "Fixture", Tier: 1, Invariant: "fixture invariant",
			Expected: qualificationset.Expectation{Classification: "qualified"}, ExpectationMet: classification == "qualified",
			Classification: classification, Seeds: []qualificationset.SeedReport{}, Blockers: []capabilityanalysis.Blocker{},
			Choice: qualificationset.ChoiceCoverage{Features: []choicewire.Feature{}},
		}},
	}
