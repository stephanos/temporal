...
func FinalizeManifest(input Manifest) (Manifest, []byte, error) {
	...
	input.Outcome.FailureSignature = DomainHash("gomadv3-failure-signature-v1", failureProjectionBytes)
	...
	input.RecordHash = DomainHash("gomadv3-run-record-v1", recordProjectionBytes)
...
func validateManifest(manifest Manifest, requireIdentities bool) error {
	...
	if manifest.Runner.RecordContract != "gomadv3.run-record/v1" || manifest.Runner.RunnerBuild == "" || manifest.Runner.HostOS == "" || manifest.Runner.HostArch == "" {
...
func validateTarget(target Target) error {
	...
	if !sortedUniqueStrings(target.BuildTags) || !sortedBuildSettings(target.BuildInfo.Settings) {
		return fmt.Errorf("target build tags and settings must be sorted and unique")
	}
	return nil
}
...
func recordProjectionOf(manifest Manifest) recordProjection {
...
func failureProjectionOf(manifest Manifest) failureProjection {
...
func projectTarget(target Target) targetProjection {
	return targetProjection{Kind: target.Kind, SHA256: target.SHA256, Size: target.Size, Argv: target.Argv, BuildTags: target.BuildTags, BuildInfo: target.BuildInfo}
}
