...
func New(limit uint64) (*Capture, error) {
	if limit == 0 {
...
func (capture *Capture) Result() Output {
	...
	raw := make([]byte, 0, len(capture.head)+len(capture.tail))
	raw = append(raw, capture.head...)
	raw = append(raw, capture.tail...)
...
func (capture *Capture) HeadLimit() uint64 {
	return uint64(capture.headLimit)
}

func (capture *Capture) appendTail(data []byte) {
