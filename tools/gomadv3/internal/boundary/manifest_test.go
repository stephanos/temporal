...
func TestGenerateWritesDeclaredHookBoilerplate(t *testing.T) {
	...
	contents, err := os.ReadFile(filepath.Join(root, "overlay", "src", "os", "gomad_unsupported_generated.go"))
	if err != nil {
		t.Fatal(err)
	}
...
func generatedHookManifest() string {
  ...
  "hook_policies": [{
    ...
    "package": "os",
    "output": "overlay/src/os/gomad_unsupported_generated.go",
...
func TestGenerateCompilerTestOverlaySeparatesFixturesFromProduction(t *testing.T) {
	...
	productionPath := filepath.Join(root, "overlay", "src", "cmd", "compile", "internal", "gomadintercept", "spec_go126.go")
...
func writeCompilerTestManifest(t *testing.T, root, contents string) {
	t.Helper()
	path := filepath.Join(root, "boundary", "compiler-tests.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(path, []byte(contents), 0o600); err != nil {
