...
type frontierPlan struct {
	...
	Config              choicefrontier.Config `json:"config"`
...
func NewFrontierJournal(ctx context.Context, batchPath string, initial choicefrontier.State, maximumSegmentBytes uint64) (*FrontierJournal, error) {
	if ctx == nil {
	...
	expected, err := choicefrontier.New(initial.Config)
	if err != nil {
		return nil, fmt.Errorf("validate initial frontier state: %w", err)
	}
	providedIdentity, err := choicefrontier.StateSHA256(initial)
	if err != nil {
		return nil, err
	}
	expectedIdentity, err := choicefrontier.StateSHA256(expected)
	if err != nil {
		return nil, err
	}
...
func ResumeFrontierJournal(ctx context.Context, batchPath string, config choicefrontier.Config, maximumSegmentBytes uint64) (*FrontierJournal, choicefrontier.State, uint64, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	batchPath, err := filepath.Abs(batchPath)
	if err != nil {
		return nil, choicefrontier.State{}, 0, fmt.Errorf("resolve frontier batch path: %w", err)
	}
	if err := validatePrivateDirectory(batchPath, "frontier batch"); err != nil {
		return nil, choicefrontier.State{}, 0, err
	}
	planBytes, err := readFrontierFile(filepath.Join(batchPath, "frontier", "plan.json"), maximumFrontierPlanBytes)
	if err != nil {
		return nil, choicefrontier.State{}, 0, fmt.Errorf("read frontier plan: %w", err)
	}
	var plan frontierPlan
	if err := record.DecodeCanonicalJSON(planBytes, &plan); err != nil {
		return nil, choicefrontier.State{}, 0, fmt.Errorf("decode frontier plan: %w", err)
	}
	if plan.Schema != frontierPlanSchema || plan.Config != config || uint64(plan.MaximumSegmentBytes) != maximumSegmentBytes || maximumSegmentBytes == 0 {
		return nil, choicefrontier.State{}, 0, errors.New("frontier plan identity or bounds changed")
	}
	state, err := choicefrontier.New(config)
	if err != nil {
		return nil, choicefrontier.State{}, 0, err
	}
	initialIdentity, err := choicefrontier.StateSHA256(state)
	if err != nil || initialIdentity != plan.InitialStateSHA256 {
		return nil, choicefrontier.State{}, 0, errors.Join(errors.New("frontier initial state identity changed"), err)
	}
	...
	if err != nil {
		return nil, choicefrontier.State{}, 0, fmt.Errorf("read frontier rounds: %w", err)
	}
	if uint64(len(entries)) > config.MaxRuns {
		return nil, choicefrontier.State{}, 0, errors.New("frontier round count exceeds its run bound")
	}
	...
	for index, entry := range entries {
		...
		if entry.Name() != name {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("frontier round sequence has a gap at %s", name)
		}
		...
		if err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("read frontier round %d: %w", index, err)
		}
		var storedRound choicefrontier.Round
		if err := record.DecodeCanonicalJSON(roundBytes, &storedRound); err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("decode frontier round %d: %w", index, err)
		}
		expectedRound, ok := state.NextRound()
		equal, equalErr := canonicalEqual(storedRound, expectedRound)
		if equalErr != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("compare frontier round %d: %w", index, equalErr)
		}
		if !ok || !equal {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("frontier round %d does not match reconstructed state", index)
		}
		segmentBytes, err := readFrontierFile(filepath.Join(roundPath, "segment.json"), maximumSegmentBytes)
		if err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("read frontier segment %d: %w", index, err)
		}
		var segment choicefrontier.RoundSegment
		if err := record.DecodeCanonicalJSON(segmentBytes, &segment); err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("decode frontier segment %d: %w", index, err)
		}
		logicalStart := state.LogicalExecutions
		state, err = choicefrontier.ReplaySegment(state, segment)
		if err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("replay frontier segment %d: %w", index, err)
		}
		chainSHA256 = segment.SHA256
		roundRuns, err := readFrontierRoundRuns(roundPath, len(storedRound.Candidates), maximumSegmentBytes)
		if err != nil {
			return nil, choicefrontier.State{}, 0, fmt.Errorf("read frontier round %d run records: %w", index, err)
		}
		if len(roundRuns) != 0 {
			if err := validateFrontierRoundRuns(storedRound, segment, roundRuns, logicalStart, config.BaseSeed); err != nil {
				return nil, choicefrontier.State{}, 0, fmt.Errorf("validate frontier round %d run records: %w", index, err)
			}
		}
...
func (journal *FrontierJournal) State() choicefrontier.State {
	return journal.state
}

func (journal *FrontierJournal) StageRound(round choicefrontier.Round) (*FrontierRoundJournal, error) {
	if journal == nil {
	...
	encoded, err := record.CanonicalJSON(round)
	if err != nil {
		return nil, err
	}
	...
	return &FrontierRoundJournal{journal: journal, path: path, index: round.Index, runs: make([]RunRecord, len(round.Candidates)), runSet: make([]bool, len(round.Candidates))}, nil
}

func (staged *FrontierRoundJournal) RecordRun(index int, run RunRecord) error {
...
func (journal *FrontierJournal) CommitRound(staged *FrontierRoundJournal, segment choicefrontier.RoundSegment) error {
	...
	next, err := choicefrontier.ReplaySegment(journal.state, segment)
	if err != nil {
...
func (journal *FrontierJournal) CommittedRuns() []RunRecord {
	if journal == nil {
		return nil
	}
	return append([]RunRecord(nil), journal.runs...)
}

func (journal *FrontierJournal) ChainSHA256() record.SHA256 {
	if journal == nil {
		return ""
	}
...
func ValidatePublishedFrontier(batchPath string, expectedSummary choicefrontier.Summary, expectedImplementation, expectedChain record.SHA256, expectedRuns []RunRecord) error {
	planBytes, err := readFrontierFile(filepath.Join(batchPath, "frontier", "plan.json"), maximumFrontierPlanBytes)
	if err != nil {
	...
	if plan.Schema != frontierPlanSchema || plan.Config.ControllerSHA256 != expectedImplementation || expectedImplementation != choicefrontier.ImplementationSHA256() || plan.MaximumSegmentBytes == 0 {
		return errors.New("published frontier plan identity is invalid")
	}
	state, err := choicefrontier.New(plan.Config)
	if err != nil {
		return err
	}
	initialIdentity, err := choicefrontier.StateSHA256(state)
	...
	for index, entry := range entries {
		...
		var round choicefrontier.Round
		if err := record.DecodeCanonicalJSON(roundBytes, &round); err != nil {
			return err
		}
		expectedRound, ok := state.NextRound()
		...
		runs, err := readFrontierRoundRuns(roundPath, len(round.Candidates), uint64(plan.MaximumSegmentBytes))
		if err != nil || len(runs) == 0 {
			return errors.Join(errors.New("published frontier round run records are unavailable"), err)
		}
		if err := validateFrontierRoundRuns(round, segment, runs, logicalStart, plan.Config.BaseSeed); err != nil {
			return err
		}
...
func (staged *FrontierRoundJournal) BeginRun(ordinal, seed uint64) (*RunJournal, error) {
	if staged == nil || staged.journal == nil {
		return nil, errors.New("frontier staged round is required")
	}
	attempt, err := record.CanonicalJSON(frontierAttempt{Ordinal: record.Uint64String(ordinal), Seed: record.Uint64String(seed)})
	if err != nil {
		return nil, err
	}
...
func archiveIncompleteFrontier(batchPath string, state choicefrontier.State, maximumRoundBytes uint64) (uint64, error) {
...
func countFrontierAttempts(roundPath string, state choicefrontier.State, round choicefrontier.Round) (uint64, error) {
	...
	for index, entry := range entries {
		...
		if err := record.DecodeCanonicalJSON(contents, &attempt); err != nil || attempt.Ordinal != record.Uint64String(ordinal) || attempt.Seed != record.Uint64String(state.Config.BaseSeed) {
...
func readFrontierRoundRuns(roundPath string, count int, maximumBytes uint64) ([]RunRecord, error) {
	...
	for index, entry := range entries {
		...
		if err := record.DecodeCanonicalJSON(contents, &runs[index]); err != nil {
			return nil, err
		}
	}
	return runs, nil
...
func validateFrontierRoundRuns(round choicefrontier.Round, segment choicefrontier.RoundSegment, runs []RunRecord, logicalStart, baseSeed uint64) error {
