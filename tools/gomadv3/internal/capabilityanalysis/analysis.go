...
type Input struct {
	...
	IOProfile ioprofile.ProfileSpec
...
type Report struct {
	...
	Requirements   []ioprofile.Requirement      `json:"requirements"`
...
func Build(input Input) (Report, error) {
	...
	closureBytes, err := record.CanonicalJSON(input.Review.Closure)
	if err != nil {
	...
	blockers := make([]Blocker, len(input.Review.Findings))
	for index, finding := range input.Review.Findings {
		path, found := paths[finding.Package]
		if !found {
			return Report{}, fmt.Errorf("capability finding package %s is not reachable from a target root", finding.Package.ImportPath)
		}
	...
	return Report{
		...
		Target: Target{
			...
			BuildTags: append([]string{}, input.Review.BuildTags...),
		},
		Toolchain: Toolchain{
...
func FormatText(report Report) string {
	...
	fmt.Fprintf(&output, "compatibility: %s\npackages: %d\n", report.Classification, uint64(report.Closure.PackageCount))
