...
func SemanticFeatures(manifest record.Manifest, coverage ioprofile.SemanticCoverage, ioTranscript, worldTransitions []byte) ([]Feature, error) {
	...
	if len(ioTranscript)%iowire.TranscriptRecordBytes != 0 {
		return nil, fmt.Errorf("I/O transcript has invalid length %d", len(ioTranscript))
	}
	var previous string
	for offset := 0; offset < len(ioTranscript); offset += iowire.TranscriptRecordBytes {
		entry, err := iowire.DecodeTranscriptRecord(ioTranscript[offset : offset+iowire.TranscriptRecordBytes])
		if err != nil {
			return nil, fmt.Errorf("decode guided I/O transcript record %d: %w", offset/iowire.TranscriptRecordBytes, err)
		}
		if entry.Operation == "boundary.probe" {
			continue
		}
		outcome := fmt.Sprintf("%s/%d", entry.Operation, entry.Result)
...
func semanticWorldFeatures(manifest record.World, encoded []byte) ([]Feature, error) {
	...
	transitions, err := record.StrictDecodeJSONLines[world.Transition](encoded)
	if err != nil {
