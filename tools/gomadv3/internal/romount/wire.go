...
type Status = iowire.MountStatus

const (
	StatusOK        = iowire.MountStatusOK
	StatusUnmounted = iowire.MountStatusUnmounted
	StatusNotExist  = iowire.MountStatusNotExist
)
...
func ReadResponse(reader io.Reader, limits Limits) (Response, error) {
	...
	response := Response{Ordinal: decoded.Ordinal, Status: decoded.Status, Entry: Entry{Kind: Kind(decoded.Entry.Kind), Mode: os.FileMode(decoded.Entry.Mode), Data: decoded.Entry.Data, Children: make([]Child, 0, len(decoded.Entry.Children))}}
...
func writeResponse(writer io.Writer, response Response, limits Limits) error {
	encoded := iowire.MountResponse{Ordinal: response.Ordinal, Status: response.Status, Entry: iowire.MountEntry{Kind: iowire.MountKind(response.Entry.Kind), Mode: uint32(response.Entry.Mode), Data: response.Entry.Data, Children: make([]iowire.MountChild, 0, len(response.Entry.Children))}}
