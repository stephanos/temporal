...
func TestArtifactRoundTripBuildsHostIndependentReplay(t *testing.T) {
	...
	encoded, err := EncodeArtifact(mappings, limits, snapshot)
	if err != nil {
		t.Fatal(err)
	}
	decodedMappings, decodedLimits, decoded, err := DecodeArtifact(encoded.Manifest, encoded.Descriptor, func(name string, maximum uint64) ([]byte, error) {
...
func TestDecodeArtifactRejectsCorruptPayload(t *testing.T) {
	limits := DefaultLimits()
	encoded, err := EncodeArtifact([]Mapping{{Target: "/schema"}}, limits, Snapshot{
		TotalBytes: 4, Entries: []Entry{{Path: "/schema/file", Mode: 0o600, Kind: KindFile, Data: []byte("data")}},
	})
	if err != nil {
		t.Fatal(err)
	}
	_, _, _, err = DecodeArtifact(encoded.Manifest, encoded.Descriptor, func(string, uint64) ([]byte, error) {
		return []byte("evil"), nil
	})
	if err == nil {
		t.Fatal("DecodeArtifact() accepted corrupt payload")
	}
}
