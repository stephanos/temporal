...
func TestProjectDecisionTapeCopiesAndBindsCompleteTrace(t *testing.T) {
	...
	validated, err := ValidateReplayPlan(tape, identity)
	if err != nil {
		t.Fatal(err)
	}
	...
	wrong := identity
	wrong.GOARCH = "amd64"
	if _, err := ValidateReplayPlan(validated, wrong); err == nil {
		t.Fatal("ValidateReplayPlan() accepted a different execution identity")
	}
}
...
func TestBuildRankPrefixBindsCanonicalRankWithoutParentTraceIdentity(t *testing.T) {
	...
	validated, err := ValidatePrefixReplayPlan(leftPrefix, identity)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ValidateReplayPlan(leftPrefix, identity); err == nil {
		t.Fatal("ValidateReplayPlan() accepted a rank override")
	}
}
