...
type capturedInputLimits struct {
	DirectoryEntries decimal `json:"directory_entries"`
	Files            decimal `json:"files"`
	PathBytes        decimal `json:"path_bytes"`
	Requests         decimal `json:"requests"`
	SingleFileBytes  decimal `json:"single_file_bytes"`
	TotalBytes       decimal `json:"total_bytes"`
}
...
func EncodeCapturedInputs(mappings []Mapping, limits Limits, snapshot Snapshot) (CapturedInputs, error) {
	...
	descriptor := capturedInputsDescriptor{
		Schema: "gomadv3.io-read-only-mounts/v1", Mappings: targets, Limits: encodeCapturedInputLimits(limits),
		Requests: decimal(snapshot.Requests), TotalBytes: decimal(snapshot.TotalBytes),
		NotExist: notExist,
	...
	return CapturedInputs{
		Manifest: CapturedInputsManifest{
			Schema: descriptor.Schema, File: descriptorPath, SHA256: hashBytes(encoded), Bytes: uint64(len(encoded)),
...
func DecodeCapturedInputs(manifest CapturedInputsManifest, descriptorBytes []byte, readPayload PayloadReader) ([]Mapping, Limits, Snapshot, error) {
	...
	if uint64(len(descriptorBytes)) != uint64(manifest.Bytes) || hashBytes(descriptorBytes) != manifest.SHA256 {
	...
	if err := decodeCanonicalJSON(descriptorBytes, &descriptor); err != nil {
	...
	for index, encoded := range descriptor.Entries {
		...
		case KindFile:
			...
			if uint64(len(data)) != uint64(encoded.Size) || hashBytes(data) != encoded.SHA256 {
...
func encodeCapturedInputLimits(limits Limits) capturedInputLimits {
	return capturedInputLimits{
		PathBytes: decimal(limits.PathBytes), Requests: decimal(limits.Requests), Files: decimal(limits.Files),
		DirectoryEntries: decimal(limits.DirectoryEntries), SingleFileBytes: decimal(limits.SingleFileBytes), TotalBytes: decimal(limits.TotalBytes),
	}
}
