...
func (store Store) PublishArtifact(publication Publication) (_ Artifact, retErr error) {
	...
	if publication.Record.SchemaVersion != SchemaVersion || publication.Record.Runner.RecordContract != RecordContract {
	...
	manifest.Files = nil
	files := make([]File, 0, len(publication.Payloads))
	...
	manifest, manifestBytes, err := FinalizeExecutionRecord(manifest)
	if err != nil {
...
func artifactStoredBytes(manifest ExecutionRecord, manifestBytes uint64) (uint64, error) {
	total := manifestBytes
...
func storeIdentity(key StoreKey, manifest ExecutionRecord) (SHA256, error) {
	switch key {
...
func identityDirectory(identity SHA256, complete bool) string {
...
func copyPayload(ctx context.Context, source, destination, relativePath string, mode os.FileMode) (File, error) {
	if err := os.MkdirAll(filepath.Dir(destination), 0o700); err != nil {
		return File{}, fmt.Errorf("create payload parent %s: %w", relativePath, err)
	}
	if err := os.Chmod(filepath.Dir(destination), 0o700); err != nil {
		return File{}, fmt.Errorf("make payload parent private %s: %w", relativePath, err)
	}
	input, err := os.Open(source)
	if err != nil {
		return File{}, fmt.Errorf("open payload %s: %w", relativePath, err)
	}
	defer input.Close()
	info, err := input.Stat()
	if err != nil {
		return File{}, fmt.Errorf("stat payload %s: %w", relativePath, err)
	}
	if !info.Mode().IsRegular() {
		return File{}, fmt.Errorf("payload %s is not a regular file", relativePath)
	}
	output, err := os.OpenFile(destination, os.O_WRONLY|os.O_CREATE|os.O_EXCL, mode)
	if err != nil {
		return File{}, fmt.Errorf("create payload %s: %w", relativePath, err)
	}
	if err := output.Chmod(mode); err != nil {
		output.Close()
		return File{}, fmt.Errorf("set payload mode %s: %w", relativePath, err)
	}
	hasher := recordHashWriter{writer: output, hasher: sha256.New()}
	_, copyErr := copyWithContext(ctx, &hasher, input)
	if copyErr != nil {
		output.Close()
		return File{}, fmt.Errorf("copy payload %s: %w", relativePath, copyErr)
	}
	if err := syncFileContext(ctx, output); err != nil {
		output.Close()
		return File{}, fmt.Errorf("sync payload %s: %w", relativePath, err)
	}
	if err := output.Close(); err != nil {
		return File{}, fmt.Errorf("close payload %s: %w", relativePath, err)
	}
	if err := input.Close(); err != nil {
		return File{}, fmt.Errorf("close source payload %s: %w", relativePath, err)
	}
	return File{Path: relativePath, Mode: formatMode(mode), Size: Uint64String(hasher.size), SHA256: SHA256("sha256:" + hex.EncodeToString(hasher.hasher.Sum(nil)))}, nil
}
...
func writePayload(ctx context.Context, destination, relativePath string, data []byte, mode os.FileMode) (File, error) {
	if err := os.MkdirAll(filepath.Dir(destination), 0o700); err != nil {
		return File{}, fmt.Errorf("create payload parent %s: %w", relativePath, err)
	}
	if err := os.Chmod(filepath.Dir(destination), 0o700); err != nil {
	...
	if _, err := copyWithContext(ctx, file, bytes.NewReader(data)); err != nil {
		file.Close()
		return File{}, fmt.Errorf("write payload %s: %w", relativePath, err)
	}
	...
	return File{Path: relativePath, Mode: formatMode(mode), Size: Uint64String(len(data)), SHA256: HashBytes(data)}, nil
}
