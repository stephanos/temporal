...
func OpenArtifact(path string) (Artifact, error) {
	...
	manifest, err := DecodeExecutionRecord(manifestBytes)
	if err != nil {
...
func OpenPayload(opened Artifact, relativePath string, maximum uint64) (*os.File, error) {
	...
	digest := SHA256("sha256:" + hex.EncodeToString(hasher.Sum(nil)))
...
func validateDirectory(root *os.Root, directory string, expected map[string]File, seen map[string]bool) error {
...
func listedDirectory(directory string, expected map[string]File) bool {
...
func ReadPayload(opened Artifact, relativePath string, maximum uint64) ([]byte, error) {
	...
	if uint64(len(data)) != uint64(expected.Size) || HashBytes(data) != expected.SHA256 {
...
func listedFile(opened Artifact, relativePath string) *File {
...
func hashValidatedFile(root *os.Root, path string, mode os.FileMode, expectedSize uint64) (SHA256, uint64, error) {
	...
	return SHA256("sha256:" + hex.EncodeToString(hasher.Sum(nil))), uint64(size), file.Close()
}
