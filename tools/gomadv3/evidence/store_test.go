...
func artifactInput(t *testing.T) Publication {
	...
	world, worldPayloads := NoneWorld()
	exitCode := Uint64String(2)
	stdout := []byte("stdout")
	stderr := []byte("stderr")
	manifest := ExecutionRecord{
		...
		Runner:           Runner{RecordContract: RecordContract, RunnerBuild: "test", HostOS: "darwin", HostArch: "arm64"},
		Toolchain:        Toolchain{GoVersion: "go1.26.4", BuildKey: "cbeccfefbc62a2ca026d9dded0316ecedfce33bd46b5c71b6645e86b67a0713e", TargetGOOS: "darwin", TargetGOARCH: "arm64"},
		Target: Target{
			Kind: "go-run", Source: ".", SHA256: HashBytes(targetBytes), Size: Uint64String(len(targetBytes)), Argv: []string{"gomadv3-target"}, BuildTags: []string{},
			Adapters: []TargetAdapter{}, Compatibility: []CompatibilityPack{}, BuildInfo: BuildInfo{GoVersion: "go1.26.4", Path: "example.com/target"},
		},
		IOProfile:   IOProfile{Name: "gomadv3-deterministic/v1", ImplementationSHA256: HashBytes([]byte("implementation")), Inventory: "{}", InventorySHA256: HashBytes([]byte("{}"))},
		...
		Streams: Streams{
			Stdout: Stream{FullSHA256: HashBytes(stdout), TotalBytes: Uint64String(len(stdout)), RetainedBytes: Uint64String(len(stdout))},
			Stderr: Stream{FullSHA256: HashBytes(stderr), TotalBytes: Uint64String(len(stderr)), RetainedBytes: Uint64String(len(stderr))},
		},
		Host: Host{StartedAt: "2026-08-10T12:00:00Z", FinishedAt: "2026-08-10T12:00:01Z", ElapsedNanos: 1},
	}
	manifest.Target.File = "target"
	manifest.Streams.Stdout.File = "stdout"
	manifest.Streams.Stdout.RetainedSHA256 = HashBytes(stdout)
	manifest.Streams.Stderr.File = "stderr"
	manifest.Streams.Stderr.RetainedSHA256 = HashBytes(stderr)
	return Publication{Record: manifest, Payloads: []Payload{
		...
		{Path: "stdout", Mode: 0o600, Data: stdout, SHA256: HashBytes(stdout), Size: Uint64String(len(stdout))},
		{Path: "stderr", Mode: 0o600, Data: stderr, SHA256: HashBytes(stderr), Size: Uint64String(len(stderr))},
		{Path: manifest.World.Initial.File, Mode: 0o600, Data: worldPayloads.Initial, SHA256: manifest.World.Initial.RawSHA256, Size: Uint64String(len(worldPayloads.Initial))},
		{Path: manifest.World.Transitions.File, Mode: 0o600, Data: worldPayloads.Transitions, SHA256: manifest.World.Transitions.RawSHA256, Size: Uint64String(len(worldPayloads.Transitions))},
		{Path: manifest.World.Final.File, Mode: 0o600, Data: worldPayloads.Final, SHA256: manifest.World.Final.RawSHA256, Size: Uint64String(len(worldPayloads.Final))},
	}}
}
