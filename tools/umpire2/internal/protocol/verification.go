...
import (
	...
	"go.temporal.io/server/tools/umpire2/internal/model"
	regressactivity "go.temporal.io/server/tools/umpire2/regress/activity"
	regressnexus "go.temporal.io/server/tools/umpire2/regress/nexus"
)
...
func (p *Protocol) VerificationModel(options VerificationOptions) (verify.Model, error) {
	...
	for _, entityType := range p.entityOrder {
		...
		entity := verify.EntityType{
			...
			Source: verify.Provenance{Path: "tools/umpire2/model"},
		}
	...
	for _, relation := range p.relations {
		result.Relations = append(result.Relations, verify.Relation{
			...
			SourceLocation:    verify.Provenance{Path: "tools/umpire2/protocol/default_relations.go"},
		})
	}
	...
	for _, entry := range p.actionOrder {
		...
		if entry.Action == nil {
			result.Abstractions = append(result.Abstractions, verify.Abstraction{
				...
				Source: verify.Provenance{Path: "tools/umpire2/protocol/default.go"},
			})
		}
	}
		...
		if verificationAction := regressionVerificationAction(registeredAction.Realization); verificationAction != "" {
			if len(mappedRegressionActions[verificationAction]) == 0 {
				...
				mappedRegressionSources[verificationAction] = verify.Provenance{
					Path: "tools/umpire2/protocol/regress_domain.go", Symbol: registeredAction.Realization,
				}
			}
...
func lowerRegressionStartActivity(
	...
	return verify.Action{
		...
		Source:       verify.Provenance{Path: "tools/umpire2/protocol/regress_domain.go", Symbol: capability.Schema.Name},
	}, nil
}
...
func (p *Protocol) addVerificationInventory(result *verify.Model, ruleInventory []verify.InventoryItem) {
	...
	if p.regression != nil {
		...
		for _, predicate := range catalog.Predicates {
			result.Inventory = append(result.Inventory, verify.InventoryItem{
				...
				Source: verify.Provenance{Path: "tools/umpire2/protocol/regress_domain.go", Symbol: predicate.Schema.Name},
			})
		}
		for _, registeredAction := range catalog.Actions {
			...
			if !included {
				...
				result.Abstractions = append(result.Abstractions, verify.Abstraction{
					...
					Source: verify.Provenance{Path: "tools/umpire2/protocol/regress_domain.go", Symbol: registeredAction.Schema.Name},
				})
			}
			result.Inventory = append(result.Inventory, verify.InventoryItem{
		...
		for _, resource := range catalog.Resources {
			result.Inventory = append(result.Inventory, verify.InventoryItem{
				...
				Source: verify.Provenance{Path: "tools/umpire2/protocol/regress_domain.go", Symbol: resource.Realization},
			})
		}
		for _, policy := range catalog.Policies {
			result.Inventory = append(result.Inventory, verify.InventoryItem{
				...
				Source: verify.Provenance{Path: "tools/umpire2/protocol/regress_domain.go", Symbol: policy.Realization},
			})
		}
	}
	for _, footprint := range p.CausalFootprints() {
		result.Inventory = append(result.Inventory, verify.InventoryItem{
			...
			Source: verify.Provenance{Path: "tools/umpire2/protocol/causal_footprints.go", Symbol: footprint.Footprint.Action},
		})
	}
}
...
func (p *Protocol) verificationAction(
	...
	if entry.Action == nil {
		...
		return verify.Action{
			...
			Unrealized:   true,
			Source:       verify.Provenance{Path: "tools/umpire2/protocol/default.go", Symbol: entry.GapReason},
		}, nil
	}
	...
	return verify.Action{
		...
		Hosting:      key.Hosting.String(),
		Capabilities: verificationCapabilities(lifecycle.EdgeRequires(key.From, key.Event)),
		Source:       verify.Provenance{Path: "tools/umpire2/protocol/default.go", Symbol: entry.Action.Name},
	}, nil
}
