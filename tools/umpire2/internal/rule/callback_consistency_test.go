...
import (
	...
	"go.temporal.io/server/tools/umpire2/internal/fact"
	"go.temporal.io/server/tools/umpire2/internal/model"
	"go.temporal.io/server/tools/umpire2/internal/protocol"
)

func TestCallbackReferenceConsistencyAcceptsMatchingEventReference(t *testing.T) {
	state, store := callbackReferenceState(t, 0, nil)
	require.Empty(t, umpire.CheckSafetyRule(context.Background(), &CallbackReferenceConsistency{}, state, umpire.RuleConfig{Relations: store}))
}
...
func TestCallbackReferenceConsistencyRejectsStartBeforeReferencedEvent(t *testing.T) {
	...
	violations := umpire.CheckSafetyRule(context.Background(), &CallbackReferenceConsistency{}, state, umpire.RuleConfig{Relations: store})
...
func TestCallbackResponseConsistencyAcceptsLateDuplicateAndRejectsConflict(t *testing.T) {
	...
	routeFact(t, state, response)
	require.Empty(t, umpire.CheckSafetyRule(context.Background(), &CallbackResponseConsistency{}, state, umpire.RuleConfig{Relations: store}))

	conflict := callbackResponseFact("failure", "conflict", firstAt.Add(time.Second))
	routeFact(t, state, conflict)
	violations := umpire.CheckSafetyRule(context.Background(), &CallbackResponseConsistency{}, state, umpire.RuleConfig{Relations: store})
	require.Len(t, violations, 1)
