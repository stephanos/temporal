...
import (
	...
	"go.temporal.io/server/tools/umpire2/internal/rule"
)
...
// RuntimeRules returns defensive generic runtime rule declarations.
func (c *Catalog) RuntimeRules() []umpire.RuntimeRuleDeclaration {
	if c == nil {
		return nil
	}
	result := make([]umpire.RuntimeRuleDeclaration, len(c.entries))
	for index, registered := range c.entries {
		result[index] = umpire.RuntimeRuleDeclaration{Safety: registered.safety, Liveness: registered.liveness}
	}
	return result
}
...
func (c *Catalog) VerificationInventory() []verify.InventoryItem {
	...
	for index, registered := range c.entries {
		result[index] = verify.InventoryItem{
			...
			Source: verify.Provenance{Path: "tools/umpire2/rule", Symbol: registered.name},
		}
	}
	return result
}
