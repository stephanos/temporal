...
func TestRunPublishesCheckedUpgradeEvidence(t *testing.T) {
	...
	if dossier.Schema != "gomad3.upgrade-dossier/v2" || !dossier.Qualified || !dossier.BoundaryApproved || dossier.Version.GoVersion != "go1.26.4" {
...
func TestRunRejectsOverlayCollisionBeforeGates(t *testing.T) {
	...
	err := Run(context.Background(), Spec{Root: root, Output: filepath.Join(root, "dossier.json")})
...
func TestLoadCorpusRejectsUncheckedOrUnqualifiedJSON(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte("{\"qualified\":false,\"schema\":\"gomad3.qualification-set-report/v1\"}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestLoadCorpusRejectsQualifiedDownstreamSet(t *testing.T) {
	...
	if _, err := loadCorpus(root, path); err == nil || !strings.Contains(err.Error(), "gomad3-core") {
		t.Fatalf("loadCorpus() error = %v", err)
	}
}
...
func writeQualifiedCorpus(t *testing.T, root, name string) string {
	...
	_, err = qualificationset.Run(context.Background(), qualificationset.Spec{
		...
		Execute: func(_ context.Context, command qualificationset.Command) qualificationset.CommandResult {
			if command.Args[0] == "analyze" {
				...
				analysis := capabilityanalysis.Report{
					Schema: capabilityanalysis.AnalysisSchema, Classification: capabilityanalysis.ClassificationUnsupported,
					Target: capabilityanalysis.Target{
					...
					Toolchain: capabilityanalysis.Toolchain{GoVersion: "go1.26.4", BuildKey: strings.Repeat("a", 64), TargetGOOS: "darwin", TargetGOARCH: "arm64", BoundaryManifestVersion: boundaryVersion, BoundaryManifestSHA256: record.SHA256(boundaryDigest)},
					...
					IOProfile: deterministicio.Default().Identity(), Packs: []target.CompatibilityPackEvidence{}, Requirements: []deterministicio.Requirement{}, GuardedBlockers: []capabilityanalysis.Blocker{}, EliminatedBlockers: []capabilityanalysis.Blocker{},
				...
				encoded, encodeErr := canonicaljson.CanonicalJSON(analysis)
				if encodeErr != nil {
					t.Fatal(encodeErr)
				}
			...
			event, marshalErr := json.Marshal(map[string]any{
				"schema": "gomad3.qualify-event/v1", "type": "result", "classification": "unsupported_target", "report_path": path, "report": report,
			})
			if marshalErr != nil {
				t.Fatal(marshalErr)
			}
