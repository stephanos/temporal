...
func Run(ctx context.Context, options Spec) error {
	...
	dossier := Dossier{
		...
		MandatoryProbePolicy: "test-host and runtime gates must reject absent required semantic probes",
	}
...
func finalizeBoundaryDiff(difference BoundaryDiff, baseline, current []byte) (BoundaryDiff, error) {
	...
	encoded, err := json.Marshal(struct {
		...
		Schema: "gomad3.boundary-diff/v1", Status: difference.Status,
...
func loadCorpus(root, path string) (CorpusEvidence, error) {
	...
	if report.Name != "gomad3-core" {
		return CorpusEvidence{}, fmt.Errorf("retained qualification set %q is not the required gomad3-core corpus", report.Name)
	}
