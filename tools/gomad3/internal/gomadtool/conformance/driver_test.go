...
func TestRunUpstreamExecutesTypedGatesInOrder(t *testing.T) {
	...
	if filepath.Base(requests[1].Dir) != "src" || requests[1].Dir != requests[2].Dir || !strings.Contains(requests[1].Dir, "gomad3-upstream-") {
...
func TestRunRejectsUnknownModeAndMissingToolchain(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "mode", config: Config{Root: t.TempDir(), Mode: "unknown", Go: "/go"}, want: "unknown gomad3 test mode"},
...
func TestRunInterceptionExecutesManifestDrivenCompilerCases(t *testing.T) {
	...
	report, err := runWith(context.Background(), Config{Root: root, Mode: "test-interception", Go: goCommand, Compiler: compiler}, func(_ context.Context, request hostexec.Request) (hostexec.Result, error) {
		...
		if strings.Contains(joined, "install -a") && !slices.Contains(request.Env, "GOOS=linux") {
			result := successfulCommand()
			var applied strings.Builder
			packageName := request.Command[len(request.Command)-1]
	...
	if !report.Passed || len(report.Cases) != 17 {
		t.Fatalf("Run() report = %+v", report)
	}
	if len(requests) != 18 {
		t.Fatalf("compiler requests = %d, want 18", len(requests))
	}
	for _, request := range requests {
		if strings.Contains(strings.Join(request.Command, " "), "./interceptfail/") && strings.Contains(strings.Join(request.Command, " "), "-toolexec=") && !slices.Contains(request.Env, "GOMAD3_TEST_COMPILE="+compiler) {
...
func TestExecWrapperIsStrictPOSIX(t *testing.T) {
	...
	contents, err := os.ReadFile(filepath.Join(root, "internal", "gomadtool", "conformance", "scripts", "exec.sh"))
	if err != nil {
		t.Fatal(err)
	}
