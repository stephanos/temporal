...
const RequestSchema = "gomad3.compatibility-pack-request/v1"
...
func DecodeRequest(data []byte) (Request, error) {
	...
	var request Request
	if err := canonicaljson.StrictDecode(data, &request); err != nil {
...
func ValidateRequest(request Request) error {
	...
	if request.ApprovalSHA256 != "" {
		if _, err := record.ParseSHA256(request.ApprovalSHA256); err != nil {
...
func ApprovalSHA256(request Request) (string, error) {
	...
	return string(record.DomainHash("gomad3.compatibility-pack-review/v1", encoded)), nil
}
...
func validateFact(fact Fact) error {
	...
	case FactLinkname:
		...
		if _, err := record.ParseSHA256(fact.SHA256); err != nil {
