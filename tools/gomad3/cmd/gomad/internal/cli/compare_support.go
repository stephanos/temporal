...
type compareSupportDependencies struct {
	open    func(string) (qualificationset.Report, error)
	compare func(supportcomparison.Input) (supportcomparison.Result, error)
}
...
func runCompareSupport(arguments []string, stdout, stderr io.Writer) int {
	return runCompareSupportWith(arguments, stdout, stderr, compareSupportDependencies{
		open: qualificationset.OpenReport, compare: supportcomparison.Compare,
	})
}
...
func runCompareSupportWith(arguments []string, stdout, stderr io.Writer, dependencies compareSupportDependencies) int {
	...
	result, err := dependencies.compare(supportcomparison.Input{Baseline: baseline, Candidate: candidate, ApprovedBoundaryDiff: approval})
	if err != nil {
		return writeCommandError(stderr, 2, "compare support reports: %v\n", err)
	}
	if *format == "json" {
		encoded, encodeErr := canonicaljson.CanonicalJSON(result)
		if encodeErr != nil {
	...
	} else if _, err := fmt.Fprint(stdout, supportcomparison.FormatText(result)); err != nil {
		return 3
	}
	if result.Classification == supportcomparison.Incomparable {
		return 2
	}
