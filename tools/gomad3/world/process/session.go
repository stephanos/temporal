...
func Open(core *world.Model) (*Session, error) {
	...
	config := os.NewFile(configFD, "gomad3-world-config")
	output := os.NewFile(recordFD, "gomad3-world-record")
	...
	childConfig, err := readSessionSpec(config)
...
func restoreReplayWorld(config SessionSpec, seed uint64) (*world.Model, error) {
