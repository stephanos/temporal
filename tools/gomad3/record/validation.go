...
func validateManifest(manifest ExecutionRecord, requireIdentities bool) error {
	...
	if err := validateSimulationProfile(manifest.ReplayMode, manifest.SimulationProfile); err != nil {
		return err
	}
	choiceProfile := ""
	...
	if manifest.Limits.ExecutionTimeoutNanos == 0 || manifest.Limits.OverallTimeoutNanos == 0 || manifest.Limits.OutputBytes == 0 || manifest.Limits.WorldTransitionBytes == 0 {
		return fmt.Errorf("invalid zero execution limit")
	}
	if manifest.Limits.TerminateGraceNanos > manifest.Limits.ExecutionTimeoutNanos || manifest.Limits.TerminateGraceNanos > manifest.Limits.OverallTimeoutNanos {
...
func validateMinimization(manifest ExecutionRecord, requireIdentities bool) error {
	...
	if minimization.Schema != "gomad3.minimization/v1" || minimization.AttemptBudget == 0 || minimization.Attempts == 0 || minimization.Attempts > minimization.AttemptBudget || len(minimization.Accepted) == 0 || uint64(len(minimization.Accepted)) > uint64(minimization.Attempts) || minimization.OriginalForcedDecisions <= minimization.FinalForcedDecisions {
...
func validateSimulationProfile(replayMode string, profile *SimulationProfile) error {
	if profile == nil {
		return nil
	}
	...
	if profile.Name != "gomad3-simulation-exploration/v1" || profile.Plan.Schema != "gomad3.simulation-exploration-plan/v1" || profile.Plan.File != "simulation/plan.json" || profile.Record.Schema != "gomad3.cluster-record/v7" || profile.Record.File != "simulation/record.json" {
...
func validateIOProfile(profile IOProfile) error {
	...
	var decoded any
	if err := canonicaljson.DecodeCanonicalJSON([]byte(profile.Inventory), &decoded); err != nil {
		return fmt.Errorf("decode I/O profile inventory: %w", err)
	}
	if profile.Transcript != nil {
		if profile.Transcript.Schema != "gomad3.io-transcript/v1" || profile.Transcript.File != "io/transcript.bin" {
	...
	if mounts := profile.ReadOnlyMounts; mounts != nil {
		if mounts.Schema != "gomad3.io-read-only-mounts/v1" || mounts.File != "io/mounts.json" || len(mounts.Mappings) == 0 {
...
func validateTarget(target Target) error {
	...
	if len(target.Argv) == 0 || target.Argv[0] != "gomad3-target" {
		return fmt.Errorf("target argv must start with gomad3-target")
	}
	...
	return validateTargetCapability(target)
}

func validateTargetCapability(target Target) error {
	...
	case "linked":
		...
		if manifest == nil || manifest.Schema != "gomad3.live-capability-manifest/v1" && manifest.Schema != "gomad3.live-capability-manifest/v2" || manifest.File != "target-capabilities.json" || manifest.Bytes == 0 {
			return errors.New("linked target capability manifest identity is incomplete")
		}
		if manifest.Schema == "gomad3.live-capability-manifest/v2" {
	...
	case "guarded":
		manifest := target.CapabilityManifest
		if manifest == nil || manifest.Schema != "gomad3.live-capability-manifest/v2" || manifest.File != "target-capabilities.json" || manifest.Bytes == 0 {
...
func validateEnvironment(environment []Environment, seed uint64, ioProfile, choiceProfile string) error {
	...
	for index, entry := range environment {
		...
		case "GOMAD3_IO_PROFILE":
			hasIOProfile = true
			foundIOProfile = ioProfile != "" && entry.Value == ioProfile
		case "GOMAD3_CHOICE_PROFILE":
