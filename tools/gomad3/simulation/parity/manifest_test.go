...
func TestCurrentManifest(t *testing.T) {
	...
	wantPrototypes := []Prototype{
		...
		{
			ID:       "process-shared-network",
			CaseID:   "two-node-request-response",
			Package:  "./tools/gomad3sim",
			Test:     "TestProcessBackendRoutesTCPThroughSharedHostModel",
			Backend:  BackendProcess,
			Fidelity: FidelitySimulationModel,
			Status:   StatusPrototype,
		},
		{
			ID:       "process-volume-restart",
			CaseID:   "restart-durable-and-volatile",
			Package:  "./tools/gomad3sim",
			Test:     "TestProcessBackendPreservesHostVolumeAcrossRestart",
			Backend:  BackendProcess,
			Fidelity: FidelitySimulationModel,
			Status:   StatusPrototype,
		},
