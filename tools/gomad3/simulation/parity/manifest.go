...
const ManifestSchema = "gomad3.simulation-parity/v1"
const HarnessSpecSchema = "gomad3.simulation-spec/v6"
...
func validateSource(source SourceReference) error {
	if len(source.Path) == 0 || len(source.Path) > maximumTextBytes || strings.IndexByte(source.Path, 0) >= 0 || !strings.HasPrefix(source.Path, "tools/gomad2/") || path.Clean(source.Path) != source.Path || path.Ext(source.Path) != ".go" || strings.Contains(source.Path, `\`) {
...
func validatePrototype(prototype Prototype, cases map[string]Case) error {
	...
	if prototype.Package != "./tools/gomad3sim" {
		return fmt.Errorf("prototype %q package = %q, want %q", prototype.ID, prototype.Package, "./tools/gomad3sim")
	}
