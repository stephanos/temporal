...
func TestCampaignJournalPublishesTheCanonicalCampaignLifecycle(t *testing.T) {
	...
	assertFileContents(t, filepath.Join(journal.Path(), "executions", "00000000000000000000.jsonl"), wantRuns)
	...
	if batch.Schema != "gomad3.campaign/v1" || batch.Journal == nil || batch.Journal.Records != 1 || batch.Journal.Segments != 1 {
		t.Fatalf("published batch = %#v", batch)
	}
	for _, removed := range []string{journal.PreparedPath(), filepath.Join(journal.Path(), ".partial", "campaign"), run.Path()} {
...
func TestOpenBatchValidatesPublishedJournal(t *testing.T) {
	...
	if opened.Path != journal.Path() || opened.Record.CampaignID != "run-open" || len(opened.Executions) != 1 || opened.Executions[0].Seed != 7 {
...
func TestBatchJournalRecordsCanonicalResumePlan(t *testing.T) {
	...
	plan.ChoiceProfile = &ChoiceProfilePlan{Name: choice.Profile, ImplementationSHA256: record.SHA256FromSum(implementation), Limit: 8 << 20}
	artifacts, err := DeriveArtifactCapacityPlan(plan)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResumeCampaignJournalReusesVerifiedRunsAndArchivesIncompleteWork(t *testing.T) {
	journal, err := NewCampaignJournal(context.Background(), CampaignConfig{
	...
	for ordinal, seed := range []uint64{8, 9} {
		...
		if err := resumed.AppendExecution(ExecutionRecord{SelectionOrdinal: record.Uint64String(ordinal + 1), Seed: record.Uint64String(seed), Domain: "success", Reason: "success", Termination: "exit"}); err != nil {
			t.Fatal(err)
		}
		if err := run.Complete(); err != nil {
			t.Fatal(err)
...
func TestResumeCampaignJournalAcceptsRunsSharingDeduplicatedFailureEvidence(t *testing.T) {
	journal, err := NewCampaignJournal(context.Background(), CampaignConfig{
	...
	published, err := artifact.PublishArtifact(artifact.Store{Root: journal.FailuresPath()}, input)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(state.Executions) != 2 || state.Executions[0].Seed != 7 || state.Executions[1].Seed != 8 || *state.Executions[0].Artifact != *state.Executions[1].Artifact {
		t.Fatalf("resume state = %#v", state)
	}
}

func testBatchPlan(journal *CampaignJournal, digest record.SHA256, size uint64) CampaignPlan {
	var journalPlan *ExecutionJournalPlan
	parallel := record.Uint64String(2)
	if journal != nil {
		value := journal.ExecutionJournalPlan()
		journalPlan = &value
		parallel = value.MaximumPartialExecutions
	}
	plan := CampaignPlan{
		Schema: CampaignPlanSchema, Strategy: "seed", Selection: "7-9", SelectionCount: 3, Parallel: parallel,
		Journal:               journalPlan,
		ExecutionTimeoutNanos: record.Uint64String(time.Second), OverallTimeoutNanos: record.Uint64String(10 * time.Second), TerminateGraceNanos: record.Uint64String(100 * time.Millisecond),
		...
		Prepared: PreparedTargetPlan{
			...
			Target: record.Target{Kind: "go-test", Source: "./pkg", SHA256: digest, Size: record.Uint64String(size), Argv: []string{"gomad3-target"}, BuildTags: []string{"gomad_fixture"}, Adapters: []record.TargetAdapter{}, Compatibility: []record.CompatibilityPack{}, CapabilityMode: "closure", BuildInfo: record.BuildInfo{GoVersion: "go1.26.4", Path: "example.test/pkg.test"}},
		},
		IOProfile:   deterministicio.Contract{Name: "gomad3-deterministic/v1", ImplementationSHA256: deterministicio.Digest(record.HashBytes([]byte("io"))), InventorySHA256: deterministicio.Digest(record.HashBytes([]byte("inventory")))},
		Environment: []record.Environment{{Name: "GOMAD3_IO_PROFILE", Value: "gomad3-deterministic/v1"}},
...
func TestBatchJournalRoundTripsChoiceProfileAndRunSummary(t *testing.T) {
	...
	if len(opened.Executions) != 1 || opened.Executions[0].ChoiceTraceBranchingRecords == nil || *opened.Executions[0].ChoiceTraceBranchingRecords != 2 || opened.Executions[0].ChoiceTapeSHA256 == nil || *opened.Executions[0].ChoiceDecisions != 3 {
		t.Fatalf("choice run summary = %#v", opened.Executions)
	}
}

func TestChoiceProfileMatchesResumePlanExactly(t *testing.T) {
	plan := CampaignPlan{ChoiceProfile: &ChoiceProfilePlan{Name: choice.Profile, ImplementationSHA256: record.HashBytes([]byte("choice implementation")), Limit: 8 << 20}}
	manifest := record.ExecutionRecord{ChoiceProfile: &record.ChoiceProfile{
	...
	if choiceProfileMatchesPlan(CampaignPlan{}, manifest) || choiceProfileMatchesPlan(plan, record.ExecutionRecord{}) {
...
func TestOpenBatchRejectsChangedRuns(t *testing.T) {
	...
	runs := filepath.Join(journal.Path(), "executions", "00000000000000000000.jsonl")
...
func TestBatchJournalPreservesPreparedStateWhenManifestPublicationFails(t *testing.T) {
	...
	if err := os.Mkdir(filepath.Join(journal.Path(), "campaign.json"), 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestBatchJournalRejectsRunBeforeExceedingReaderCapacity(t *testing.T) {
	...
	err = journal.AppendExecution(ExecutionRecord{
		SelectionOrdinal: 0, Seed: 7, Domain: "success", Reason: strings.Repeat("x", defaultExecutionSegmentBytes), Termination: "exit",
	})
	...
	entries, err := os.ReadDir(filepath.Join(journal.Path(), ".partial", "executions"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 0 {
...
func TestBatchJournalPreservesExplicitFailureState(t *testing.T) {
	...
	const wantLifecycleFailure = `{"campaign_id":"run-failed","detail":"build failed","last_stable_state":"planned","reason":"target_preparation","schema":"gomad3.campaign-lifecycle/v1","schema_version":1,"state":"recoverable-failure"}`
	assertFileContents(t, filepath.Join(journal.Path(), ".partial", "campaign", "partial.json"), wantLifecycleFailure)
}
