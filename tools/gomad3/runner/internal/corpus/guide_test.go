...
func TestSemanticFeaturesUseStableOutcomesAndOperationPairs(t *testing.T) {
	...
	coverage, err := deterministicio.SummarizeSemanticProbes([]string{"stdlib.os.openfile"})
	if err != nil {
		t.Fatal(err)
	}
	manifest := record.ExecutionRecord{
		...
		World: record.World{
			Initial:     record.WorldPayload{Schema: "gomad3.world.snapshot/v1", SemanticDigest: record.HashBytes([]byte("initial"))},
			Transitions: record.WorldTransitions{Schema: "gomad3.world.transitions/v1"},
			Final:       record.WorldPayload{Schema: "gomad3.world.snapshot/v1", SemanticDigest: record.HashBytes([]byte("world"))},
	...
	want := []Feature{
		...
		{Kind: FeatureWorld, Value: "state/gomad3.world.snapshot/v1/changed"},
...
func TestSemanticFeaturesConsumeCanonicalChoicewireFeatureIDs(t *testing.T) {
	...
	payload := encodeChoiceRecords(t, []choice.Record{decision.Record()})
	projected, err := choice.ProjectComplete(payload, choice.CompleteMetadata{
		Limit: choice.MinimumTraceBytes, Records: 1, SHA256: sha256.Sum256(payload),
...
func encodeChoiceRecords(t *testing.T, records []choice.Record) []byte {
	t.Helper()
...
func transcriptRecord(t *testing.T, ordinal uint64, operation string, result uint32) []byte {
	t.Helper()
	encoded, err := deterministicio.EncodeOperation(deterministicio.Operation{Ordinal: ordinal, Name: operation, Result: result})
	if err != nil {
		t.Fatal(err)
	}
	return encoded
...
func semanticWorld(t *testing.T, seed uint64, key string, payload []byte, readyAt world.LogicalTime) (record.World, []byte) {
	...
	bundle, err := execution.ComposeRecording(recording, 1<<20)
	if err != nil {
		t.Fatal(err)
	}
