...
func TestEnsureDownloadsAndReusesVerifiedArchive(t *testing.T) {
	...
	config := SourceSpec{CacheDir: t.TempDir(), Name: "go1.26.4.src.tar.gz", URL: server.URL, SHA256: digest}
	first, err := EnsureSource(context.Background(), config)
	if err != nil {
		t.Fatal(err)
	}
	second, err := EnsureSource(context.Background(), config)
	if err != nil {
		t.Fatal(err)
	}
	if first != second || requests.Load() != 1 {
		t.Fatalf("EnsureSource() paths = %q, %q; requests = %d", first, second, requests.Load())
	}
...
func TestEnsureReplacesCorruptCacheOnlyAfterVerifiedDownload(t *testing.T) {
	...
	if _, err := EnsureSource(context.Background(), SourceSpec{CacheDir: cache, Name: filepath.Base(path), URL: server.URL, SHA256: digest}); err != nil {
		t.Fatal(err)
	}
...
func TestEnsureChecksumFailureDoesNotPublishPartialArchive(t *testing.T) {
	...
	_, err := EnsureSource(context.Background(), SourceSpec{
...
func TestExtractMaterializesRegularGoTree(t *testing.T) {
	...
	if err := ExtractSource(context.Background(), archivePath, destination); err != nil {
		t.Fatal(err)
	}
...
func TestExtractRejectsUnsafeOrUnsupportedEntries(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			destination := filepath.Join(t.TempDir(), "source")
			if err := ExtractSource(context.Background(), archivePath, destination); err == nil {
				t.Fatal("ExtractSource() accepted unsafe archive")
			}
		})
	}
}
...
func TestExtractRejectsDuplicatePathsAndNonemptyDestination(t *testing.T) {
	...
	if err := ExtractSource(context.Background(), archivePath, filepath.Join(t.TempDir(), "duplicate")); err == nil {
		t.Fatal("ExtractSource() accepted duplicate archive paths")
	}
	...
	if err := ExtractSource(context.Background(), archivePath, destination); err == nil {
		t.Fatal("ExtractSource() accepted a nonempty destination")
	}
}
