...
func TestProcessModelExchangeCorrelatesConcurrentResponses(t *testing.T) {
	...
	t.Setenv("GOMAD3_SIMULATION_ROLE", "node")
	t.Setenv("GOMAD3_SIMULATION_MODEL_REQUEST_FD", strconv.FormatUint(uint64(requestWrite.Fd()), 10))
	t.Setenv("GOMAD3_SIMULATION_MODEL_RESPONSE_FD", strconv.FormatUint(uint64(responseRead.Fd()), 10))
