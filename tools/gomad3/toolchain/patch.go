...
func MaterializePatch(ctx context.Context, config PatchSpec) error {
	...
	if _, err := runPatchCommand(ctx, sourceRoot, command, patch); err != nil {
		return errors.Join(errors.New("gomad3 patch does not apply with zero fuzz"), err)
	}
	command = []string{"patch", "--batch", "-V", "none", "-p1", "-F", "0"}
	if _, err := runPatchCommand(ctx, sourceRoot, command, patch); err != nil {
		return fmt.Errorf("materialize gomad3 patch: %w", err)
	}
	return nil
}
...
func validatePatch(root, path string, descriptor gomadversion.Descriptor) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open gomad3 patch: %w", err)
	}
	if info.Size() <= 0 || info.Size() > maximumPatchBytes {
		return nil, errors.Join(fmt.Errorf("gomad3 patch must be nonempty and no larger than %d bytes", maximumPatchBytes), file.Close())
	}
	contents := make([]byte, info.Size())
	if _, err := io.ReadFull(file, contents); err != nil {
		return nil, errors.Join(fmt.Errorf("read gomad3 patch: %w", err), file.Close())
	}
	if err := file.Close(); err != nil {
		return nil, fmt.Errorf("close gomad3 patch: %w", err)
	}
	...
	if err != nil || result.ExitCode != 0 {
		return nil, errors.Join(errors.New("gomad3 patch is malformed"), err)
	}
	return contents, nil
}
...
func patchPaths(contents []byte) ([]string, error) {
	...
	finish := func() error {
		if current != "" && (!oldHeader || !newHeader) {
			return errors.New("gomad3 patch must contain complete Git file headers")
		}
		return nil
	}
	for _, line := range lines {
		...
		case strings.HasPrefix(line, "diff --git "):
			...
			if len(fields) != 4 || !strings.HasPrefix(fields[2], "a/") || !strings.HasPrefix(fields[3], "b/") || fields[2][2:] != fields[3][2:] {
				return nil, errors.New("gomad3 patch has a non-canonical Git file header")
			}
			current = fields[2][2:]
		...
		case strings.HasPrefix(line, "new file mode "), strings.HasPrefix(line, "deleted file mode "), strings.HasPrefix(line, "GIT binary patch"), strings.HasPrefix(line, "Binary files "):
			return nil, errors.New("gomad3 patch may only modify existing text files")
		case strings.HasPrefix(line, "--- "):
			if current == "" || line != "--- a/"+current || oldHeader {
				return nil, errors.New("gomad3 patch has an invalid old-file header")
			}
			oldHeader = true
		case strings.HasPrefix(line, "+++ "):
			if current == "" || line != "+++ b/"+current || !oldHeader || newHeader {
				return nil, errors.New("gomad3 patch has an invalid new-file header")
			}
			newHeader = true
		case strings.HasPrefix(line, "+") && strings.Contains(line, "Code generated by ") && strings.Contains(line, " DO NOT EDIT"):
			return nil, errors.New("gomad3 patch contains generated output")
		}
	}
	if err := finish(); err != nil {
		return nil, err
	...
	if len(paths) == 0 {
		return nil, errors.New("gomad3 patch contains no file changes")
	}
	slices.Sort(paths)
	if len(slices.Compact(paths)) != len(paths) {
		return nil, errors.New("gomad3 patch contains duplicate file sections")
	}
	return paths, nil
}
...
func validateOverlay(root string, descriptor gomadversion.Descriptor) error {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if !entry.Type().IsRegular() {
			return fmt.Errorf("gomad3 overlay contains a non-regular file: %s", path)
		}
		...
		if bytes.IndexByte(contents, 0) >= 0 {
			return fmt.Errorf("gomad3 overlay contains binary output: %s", relative)
		}
		if bytes.Contains(contents, []byte("Code generated by ")) && bytes.Contains(contents, []byte(" DO NOT EDIT")) && !generatedOverlay(relative) {
			return fmt.Errorf("gomad3 overlay contains generated output: %s", relative)
		}
		paths = append(paths, relative)
		return nil
	})
	if err != nil {
		return errors.Join(errors.New("validate gomad3 overlay"), err)
	}
	slices.Sort(paths)
	if len(paths) == 0 {
		return errors.New("gomad3 overlay contains no runtime source files")
	}
	...
	if strings.HasPrefix(path, runtimePrefix) {
		...
		if !strings.Contains(name, "/") && strings.HasSuffix(name, ".go") {
			if prohibitedRuntimeArea(name) {
				return fmt.Errorf("gomad3 input touches prohibited runtime area: %s", path)
			}
			if platformRuntimeFile(name) {
				return fmt.Errorf("gomad3 input touches prohibited platform file: %s", path)
			}
		}
	}
	return fmt.Errorf("gomad3 input touches prohibited path: %s", path)
}
...
func validatePath(path string) error {
	if strings.ContainsRune(path, '\n') {
		return fmt.Errorf("gomad3 input path contains a newline: %q", path)
	}
	if path == "" || strings.ContainsRune(path, '\x00') || filepath.IsAbs(path) || filepath.Clean(path) != path || strings.HasPrefix(path, "../") || strings.Contains(path, "\\") {
		return fmt.Errorf("gomad3 input path is invalid: %q", path)
	}
	return nil
}
