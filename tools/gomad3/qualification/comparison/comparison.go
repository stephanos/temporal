...
const Schema = "gomad3.support-comparison/v1"

const ComparisonBoundaryDiffDomain = "gomad3.support-boundary-diff/v1"
...
type Input struct {
	Baseline             qualificationset.Report
	Candidate            qualificationset.Report
...
func Compare(input Input) (Result, error) {
	...
	if incomparable || workloads.membershipChanged {
		...
		result.Classification = Regressed
	} else if workloads.improved {
		result.Classification = Improved
	} else {
		result.Classification = Clean
	}
	result.ReviewRequired = result.Classification == Regressed || result.Boundary.Changed && !result.Boundary.Approved
	return result, nil
}
...
func compareWorkloadSets(baselineReport, candidateReport qualificationset.Report) workloadComparison {
	result := workloadComparison{changes: []WorkloadChange{}}
	baseline := workloadsByID(baselineReport.Workloads)
	candidate := workloadsByID(candidateReport.Workloads)
...
func workloadIdentities(baseline, candidate map[string]qualificationset.WorkloadReport) []string {
...
func compareWorkload(id string, baseline qualificationset.WorkloadReport, baselineFound bool, candidate qualificationset.WorkloadReport, candidateFound bool, baselineChoice, candidateChoice bool) workloadChangeResult {
...
func compareIdentities(result *Result, baseline, candidate qualificationset.Report) bool {
...
func compareBoundary(baseline, candidate qualificationset.Report, approval record.SHA256) BoundaryComparison {
	...
	payload, _ := canonicaljson.CanonicalJSON(struct {
		BaselineVersion  string        `json:"baseline_version"`
		BaselineSHA256   record.SHA256 `json:"baseline_sha256"`
		CandidateVersion string        `json:"candidate_version"`
		CandidateSHA256  record.SHA256 `json:"candidate_sha256"`
	}{result.BaselineVersion, result.BaselineSHA256, result.CandidateVersion, result.CandidateSHA256})
	result.DiffSHA256 = record.DomainHash(ComparisonBoundaryDiffDomain, payload)
	result.Approved = approval != "" && approval == result.DiffSHA256
	return result
}

func workloadsByID(workloads []qualificationset.WorkloadReport) map[string]qualificationset.WorkloadReport {
	result := make(map[string]qualificationset.WorkloadReport, len(workloads))
...
func blockerIdentities(blockers []capabilityanalysis.Blocker) map[record.SHA256]BlockerIdentity {
	...
	for _, blocker := range blockers {
		encoded, _ := canonicaljson.CanonicalJSON(blocker)
		digest := record.DomainHash("gomad3.support-blocker/v1", encoded)
