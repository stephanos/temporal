...
The generated files and toolchain stay under `tools/gomad3/.toolchain`; the CLI
is written to `tools/gomad3/.bin/gomad`.
...
World is an opt-in, in-memory event engine under `tools/gomad3/world`. A target
...
1. Run `make gomad3` and `gomad doctor`.
