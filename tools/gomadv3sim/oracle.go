...
func newOracleResult(name string, passed bool, evidence []OracleEvidence, maximumBytes uint64) (OracleResult, error) {
	...
	for index, item := range evidence {
		...
		identity, err := hashCanonical("gomadv3-oracle-evidence/v1", cloned[index].Value)
		if err != nil {
			return OracleResult{}, err
		}
	...
	if !passed {
		identity, err := hashCanonical("gomadv3-oracle-failure/v1", struct {
			Name     string           `json:"name"`
			Evidence []OracleEvidence `json:"evidence"`
...
func oracleResultIdentity(result OracleResult) (string, error) {
	result.Identity = ""
	return hashCanonical("gomadv3-oracle-result/v1", result)
}
