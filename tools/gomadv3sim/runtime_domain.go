...
var ErrRuntimeUnavailable = errors.New("gomadv3 simulation runtime is unavailable")
...
func gomadInterceptRuntimeDomainBegin(observationBytes, maximumDomains uint64) (uint64, error, bool) {
	...
	if token == 0 {
		return 0, errors.New("gomadv3 simulation runtime run capacity exhausted"), true
	}
...
func gomadInterceptRuntimeDomainRegister(run uint64, node NodeID, address string, incarnation uint64) (uint64, error, bool) {
	...
	if token == 0 {
		return 0, errors.New("gomadv3 simulation runtime domain capacity exhausted"), true
	}
	return token, nil, true
}
...
func gomadInterceptRuntimeDomainFinish(run uint64) ([]OutputObservation, error, bool) {
	...
	if !ok {
		return nil, errors.New("finish gomadv3 simulation runtime run"), true
	}
