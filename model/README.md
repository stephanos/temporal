...
owners of their diagnostic types and error precedence. Among production modules, only Behavior's
language implementation and `Umpire.Observation.Compiler` import the internal
`Umpire.Shared.DefinitionGraph` analysis; focused tests may import it directly. It supplies
deterministic node, edge, order, and cycle evidence, while each language decides when to consume it
and which historical cycle witness to report. Ordinary authors import the language facades, not the
graph module.
