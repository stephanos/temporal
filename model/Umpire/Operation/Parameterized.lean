...
structure ParameterTable (domain : ParameterDomain template limits)
  ...
  private mk ::
  validated : CheckedTable Setup State (ActionInstance template limits) Outcome Fact
...
def ParameterDomain.checkModel [DecidableEq Setup] [DecidableEq State]
    [DecidableEq Outcome] [DecidableEq Fact]
    (domain : ParameterDomain template limits)
    (table : FiniteTable Setup State (ActionInstance template limits) Outcome Fact)
  ...
  let _ ← domain.validateTable table |>.mapError TableAdmissionError.invalidTable
  table.checkModel { identity with actionId := fun _ => template.identity }
    { definition with definitions := {
        id := template.identity, kind := .action, source := definition.source,
        behaviorVersion := domain.canonical } :: definition.definitions }
    composition
