...
def Canonical.canonicalStrings (values : List String) : List String :=
  values.mergeSort |>.eraseDups

open Canonical

private def initialRowLe (left right : BehaviorInitialStateRow) : Bool :=
  decide (left.setup < right.setup) ||
    (left.setup == right.setup && decide (left.state ≤ right.state))

private def transitionRowLe (left right : BehaviorTransitionRow) : Bool :=
  compare left right != .gt

def Canonical.invalidBehaviorDomainEncoding?
    (domain : Vocabulary setupDomain stateDomain actionDomain outcomeDomain
...
def Machine.describeBehavior
    ...
    (domain : Vocabulary kernel.setupDomain kernel.stateDomain kernel.actionDomain
...
def Machine.behaviorTable?
    (kernel : Machine Setup State Action Outcome Observation) :
    Option BehaviorTable :=
  match kernel.vocabulary with
...
def Canonical.idLe (left right : DefinitionId) : Bool :=
...
def Canonical.definitionLe (left right : DefinitionMetadata) : Bool :=
...
def Canonical.providerLe (left right : Provider LawStatement) : Bool :=
  decide (left.id.value ≤ right.id.value)

def Canonical.connectorLe (left right : Connector LawStatement) : Bool :=
  decide (left.id.value ≤ right.id.value)

def Canonical.meaningLe (left right : Meaning) : Bool :=
  decide (left.definitionId.value < right.definitionId.value) ||
    (left.definitionId == right.definitionId &&
      decide (left.behaviorVersion ≤ right.behaviorVersion))

def Canonical.reconciliationLe (left right : Reconciliation) : Bool :=
  decide (left.definitionId.value < right.definitionId.value) ||
    (left.definitionId == right.definitionId &&
      decide (left.behaviorVersion ≤ right.behaviorVersion))

def Canonical.lawLe (left right : Law) : Bool :=
  decide (left.id.value < right.id.value) ||
    (left.id == right.id && decide (left.body ≤ right.body))

def Canonical.canonicalIds (ids : List DefinitionId) : List DefinitionId :=
  ids.mergeSort idLe |>.eraseDups

def Canonical.sourcePath (source : SourceLocation) : String :=
...
private def lawJson (law : Law) : String :=
...
private def definitionSemanticJson (declaration : DefinitionMetadata) : String :=
  "{\"id\":" ++ quote declaration.id.value ++
    ...
    ",\"behaviorVersion\":" ++ quote declaration.behaviorVersion ++ "}"
...
private def providerSemanticJson (provider : Provider LawStatement) : String :=
  ...
  "{\"id\":" ++ quote provider.id.value ++
    ...
    ",\"behaviorVersion\":" ++ quote provider.contract.behaviorVersion ++
...
private def reconciliationJson (reconciliation : Reconciliation) : String :=
  "{\"id\":" ++ quote reconciliation.definitionId.value ++
    ...
    ",\"behaviorVersion\":" ++ quote reconciliation.behaviorVersion ++ "}"

private def connectorSemanticJson (connector : Connector LawStatement) : String :=
  let laws := connector.requiredLaws.mergeSort lawLe
  "{\"id\":" ++ quote connector.id.value ++
    ",\"version\":" ++ toString connector.version ++
    ",\"behaviorVersion\":" ++ quote connector.behaviorVersion ++
    ",\"reconciliations\":" ++
...
private def initialStateRowJson (row : BehaviorInitialStateRow) : String :=
  "{\"setup\":" ++ quote row.setup ++ ",\"state\":" ++ quote row.state ++ "}"

private def transitionRowJson (row : BehaviorTransitionRow) : String :=
...
private def targetBehaviorDescriptionJson (description : BehaviorTable) : String :=
...
def Canonical.targetMetadataJson
