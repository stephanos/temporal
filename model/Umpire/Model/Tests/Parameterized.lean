...
namespace Umpire.ModelTests.Parameterized
...
private def definition : TableModelSpec := {
...
#guard (do
  ...
  pure (d.actions.map fun a =>
    ((t.machine.steps (value "example.state.phase" "idle") a.modelValue).map (·.outcome.value),
     (t.machine.steps (value "example.state.phase" "done") a.modelValue).map (·.outcome.value)))) ==
...
#guard (do
  ...
  pure (d.decode a.modelValue |>.isNone,
    t.machine.steps (value "example.state.phase" "idle") a.modelValue |>.isEmpty,
...
end Umpire.ModelTests.Parameterized
