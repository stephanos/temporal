...
| `tools/umpire/internal/artifactv2` and `tools/umpire/artifact` | The same three-field `Property` identity inside canonical ExperimentSpec v2 JSON | Validate the artifact version, exact fields, canonical order, digest syntax, checksums, and closure. These readers do not interpret Property clauses; unknown JSON fields and unsupported artifact majors fail at the existing strict artifact boundary. |
