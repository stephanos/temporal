...
private def answer (bound : Nat) (steps : List Transition)
    (endpoint : PropertyScopedEndpoint := .runtimePrefix) : Option PropertyEndpointAnswer :=
...
#guard answer 1 [step "a" request] .deliberatelyClosed == some .violated
...
#guard (do
  ...
  let property ← (property target 1 .deliberatelyClosed).toOption
...
#guard error? (evaluate 1 .runtimePrefix [{ step "a" request with priorState := quiet }]) ==
  some (.invalidTransition "a")
#guard error? (evaluate 1 .runtimePrefix [{ step "a" request with result := result true }]) ==
  some (.invalidTransition "a")
#guard error? (evaluate 1 .runtimePrefix [{ step "a" request with scope := [(id "test.run", "other")] }]) ==
  some .wrongScope
#guard error? (evaluate 1 .runtimePrefix [{ step "a" request with operationField := id "test.other" }]) ==
  some .wrongScope
#guard error? (evaluate 1 .runtimePrefix [step "a" request] { limits with transitions := 0 }) ==
  some .transitionsExhausted
#guard error? (evaluate 1 .runtimePrefix [step "a" request] { limits with obligations := 0 }) ==
  some .obligationsExhausted
#guard error? (evaluate 1 .runtimePrefix [step "a" request] { limits with work := 1 }) ==
  some .workExhausted
#guard (evaluate 1 .runtimePrefix [step "a" request] { limits with work := 49 }).isOk
#guard ([10, 100] : List Nat).all fun count =>
  (evaluate count .runtimePrefix (List.replicate count (step "a" request))
    { limits with obligations := count }).isOk &&
  error? (evaluate (count + 1) .runtimePrefix (List.replicate (count + 1) (step "a" request))
...
#guard ([
  ...
  { clause 1 with response := .atom { field := .modelOutcome, reference := id "test.missing" } },
