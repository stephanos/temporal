...
visited path before traversal, rejects aliases and containment escapes, and coalesces duplicate
case variants only when the filesystem identifies them as the same directory. Its pure
reconciliation interface checks source identities against caller-supplied module metadata and
ownership/classification predicates.
-/
...
private def uniqueSorted [BEq α] (less : α → α → Bool) (items : Array α) : Array α :=
  (items.qsort less).foldl (init := #[]) fun result item =>
    if result.back? == some item then result else result.push item

private def uniqueSortedNames (names : Array Lean.Name) : Array Lean.Name :=
  uniqueSorted nameLess names
...
private def uniqueSortedIssues (issues : Array InventoryIssue) : Array InventoryIssue :=
  uniqueSorted issueLess issues
...
/-- Validate source classification and qualified identity before loading metadata. -/
def validateSources
...
private def directoryPrefix (directory : FilePath) : String :=
  let directory := directory.normalize.toString
  if directory.endsWith FilePath.pathSeparator.toString then directory
  else directory ++ FilePath.pathSeparator.toString

private def containedBy (root path : FilePath) : Bool :=
  let rootText := root.normalize.toString
  let pathText := path.normalize.toString
  let rootPrefix := directoryPrefix root
...
private partial def scanDirectory
  ...
  let canonicalDirectory ← Lean.realPathNormalized directory
  ...
  -- Case-insensitive mounts can enumerate both spellings of one directory.
  for previous in visited do
    if previous.canonical.toString.toLower == canonicalDirectory.toString.toLower &&
      ...
      if previous.logical.toString.toLower == directory.toString.toLower then
        return (#[], visited)
      throw <| IO.userError s!"directory alias or cycle: {directory} -> {canonicalDirectory}; \
        already inventoried as {previous.logical}"
  ...
  for entry in entries do
    unless excludedDirectories.contains entry.fileName do
      ...
      let canonicalEntry ← Lean.realPathNormalized entry.path
      ...
      if metadata.type == .symlink then
        throw <| IO.userError s!"source path alias is not allowed: {entry.path} -> \
          {canonicalEntry}"
...
def scanSources
  ...
  let root ← Lean.realPathNormalized sourceRoot
