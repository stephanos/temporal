...
def table : FiniteTable Race.Setup Race.State Race.Action
    Race.Outcome Race.Fact := {
  Race.table with terminalConditions := [[.canceled, .succeeded]]
}
...
def modelSpec : TableModelSpec := {
  ...
  definitions := Race.definitions.map fun definition =>
...
def targetResult : Except TableAdmissionError (QueryModel Race.LawStatement) :=
  table.checkModel Race.identity modelSpec Race.modelProviders

/-- Cancellation evidence observes exactly the existing Race transitions and their alternatives. -/
theorem transitions_eq_race : table.transitions = Race.table.transitions := rfl
