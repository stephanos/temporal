...
`Temporal.Feature.Nexus.Operations.Planning` module owns only their shared planning import seam.
...
def incrementalKernel : IncrementalPlannerKernel target :=
  Eq.mpr (congrArg IncrementalPlannerKernel AsyncStart.query_target) AsyncStart.incrementalKernel
