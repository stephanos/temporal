...
closures cover the messages a Model names -- the workflow task the caller's commands travel on, and
the three Nexus task RPCs the handler's replies travel on. A name outside those closures rejects, and
