...
func (s *Umpire2TestSuite) newNexusProbeEnv(t *testing.T) *NexusTestEnv {
...
func (s *Umpire2TestSuite) nexusProbeDrive(t *testing.T, env *NexusTestEnv, onStart nexustest.Handler, build callerFor) probe.DriveFunc {
...
func (s *Umpire2TestSuite) nexusExec(onStart nexustest.Handler, build callerFor) probe.EnvFunc {
...
func (s *Umpire2TestSuite) newNexusStandaloneEnv(t *testing.T) *NexusTestEnv {
	return newNexusTestEnv(t, true,
		testcore.WithDynamicConfig(dynamicconfig.EnableChasm, true),
		testcore.WithDynamicConfig(dynamicconfig.EnableCHASMCallbacks, true),
		testcore.WithDynamicConfig(chasmnexus.EnableChasmWorkflowOperations, true),
...
func (s *Umpire2TestSuite) nexusGenExecPlan(plan []umpire.Action) probe.EnvFunc {
	return func(t *testing.T, _ int) (*testcore.TestEnv, probe.DriveFunc) {
...
func (s *Umpire2TestSuite) TestProbeNexusGeneratedCompletion() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusRejectedStart() {
	t := s.T()
	env := s.newNexusStandaloneEnv(t)
...
func (s *Umpire2TestSuite) TestProbeNexusReflectedVariant() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusReflectedDurationVariant() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusFaultAction() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusResilience() {
	t := s.T()
	report := probe.Umpire(t).
		Reach("NexusOperation", "succeeded").
...
func (s *Umpire2TestSuite) TestProbeNexusDegraded() {
	t := s.T()
	report := probe.Umpire(t).
		Reach("NexusOperation", "succeeded").
...
func (s *Umpire2TestSuite) TestProbeNexusFlagged() {
	t := s.T()
	report := probe.Umpire(t).
		Reach("NexusOperation", "succeeded").
...
func (s *Umpire2TestSuite) TestProbeNexusHTTPFaultSeam() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusExploration() {
	t := s.T()
	...
	lc, ok := defaultUmpire2Protocol(t).Lifecycle(umpire2.NexusOperationType)
	require.True(t, ok)
...
// cleanly.
func (s *Umpire2TestSuite) TestProbeWorkflowGenerated() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeWorkflowContinueAsNew() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeWorkflowContinueAsNewGenerated() {
	t := s.T()
	env := testcore.NewEnv(t)
...
func (s *Umpire2TestSuite) TestProbeWorkflowReset() {
	t := s.T()
	env := testcore.NewEnv(t)
...
func (s *Umpire2TestSuite) TestProbeNexusLearnedFootprint() {
	t := s.T()
	env := s.newNexusStandaloneEnv(t)
	runner := umpire2.NewActionRunner(env.TestEnv)
	endpoint := env.createRandomExternalNexusServer(env.Context(), t, runner.Handler())
...
func (s *Umpire2TestSuite) TestProbeNexusCoverageGuidedFaults() {
	t := s.T()
...
func (s *Umpire2TestSuite) TestProbeNexusRandomized() {
	t := s.T()
