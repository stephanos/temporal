...
type Umpire2TestSuite struct {
	parallelsuite.Suite[*Umpire2TestSuite]
}

func TestUmpire2TestSuite(t *testing.T) {
	parallelsuite.Run(t, &Umpire2TestSuite{})
}

func defaultUmpire2Protocol(t *testing.T) *umpire2.Protocol {
	t.Helper()
...
func (s *Umpire2TestSuite) TestPlanAndDriveWorkflowToCompletion() {
	t := s.T()
	...
	plan, err := defaultUmpire2Protocol(t).PlanTo(umpire2.WorkflowType, umpire2.WorkflowCompleted, umpirefw.Shortest, umpirefw.Constraints{})
	require.NoError(t, err)
...
func (s *Umpire2TestSuite) TestPlanAndDriveKitchenSinkWorkflow() {
	t := s.T()
	env := testcore.NewEnv(t, testcore.WithUmpireMonitorFactory(umpire2.NewMonitor))
...
// TracerProvider.
func (s *Umpire2TestSuite) TestPlanAndDriveNexusOperationCHASM() {
	t := s.T()
	...
	plan, err := defaultUmpire2Protocol(t).PlanTo(umpire2.NexusOperationType, umpire2.NexusSucceeded, umpirefw.Shortest, umpirefw.Constraints{})
	require.NoError(t, err)
...
func (s *Umpire2TestSuite) TestPlanAndDriveKitchenSinkNexusOperation() {
	t := s.T()
