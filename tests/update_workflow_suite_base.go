...
import (
	...
	"go.temporal.io/server/tests/testenv"
)
...
func sendUpdateNoError(s *testenv.Scenario, tv *testvars.TestVars) <-chan *workflowservice.UpdateWorkflowExecutionResponse {
	s.T().Helper()
...
func sendUpdateNoErrorInternal(s *testenv.Scenario, tv *testvars.TestVars, waitPolicy *updatepb.WaitPolicy) <-chan *workflowservice.UpdateWorkflowExecutionResponse {
	s.T().Helper()
	retCh := make(chan *workflowservice.UpdateWorkflowExecutionResponse)
	syncCh := make(chan struct{})
	go func() {
...
func waitUpdateAdmitted(s *testenv.Scenario, tv *testvars.TestVars) {
	s.T().Helper()
	s.EventuallyWithTf(func(collect *assert.CollectT) {
		pollResp, pollErr := s.FrontendClient().PollWorkflowExecutionUpdate(testcore.NewContext(), &workflowservice.PollWorkflowExecutionUpdateRequest{
			Namespace:  s.Namespace().String(),
...
func (s *WorkflowUpdateBaseSuite) startWorkflow(tv *testvars.TestVars) string {
	s.T().Helper()
	startResp, err := s.FrontendClient().StartWorkflowExecution(testcore.NewContext(), startWorkflowRequest(s.Namespace(), tv))
	s.NoError(err)

	return startResp.GetRunId()
}

func startWorkflow(s *testenv.Scenario, tv *testvars.TestVars) string {
	s.T().Helper()
	startResp, err := s.FrontendClient().StartWorkflowExecution(testcore.NewContext(), startWorkflowRequest(s.Namespace(), tv))
	s.NoError(err)
...
func startWorkflowRequest(ns namespace.Name, tv *testvars.TestVars) *workflowservice.StartWorkflowExecutionRequest {
	return &workflowservice.StartWorkflowExecutionRequest{
		RequestId:    tv.Any().String(),
		Namespace:    ns.String(),
		WorkflowId:   tv.WorkflowID(),
