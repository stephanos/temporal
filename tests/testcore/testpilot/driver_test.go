...
func (*recordingControllerSession) CompleteNexusOperation(context.Context, testpilot.Coordinate, testpilot.OpaqueCapability, *testpilotpb.Value) (testpilot.EffectHandle, error) {
	return recordingEffect{}, nil
}
...
func (*recordingControllerSession) Diagnose(context.Context, string, *testpilotpb.RunDiagnostic) error {
	return nil
}
...
func (*recordingWorkerSession) Diagnose(context.Context, string, *testpilotpb.RunDiagnostic) error {
	return nil
}
...
func TestCarrierEffectDrainFinalizesCompletedResult(t *testing.T) {
	...
	effect := &carrierEffect{
		EffectHandle: &terminalEffect{done: done, result: testpilot.EffectResult{
			Outcome: &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_PROTOCOL_NON_SUCCESS},
		}},
