...
func (l *Ledger) CreateBundle(ctx context.Context, origin testpilot.Coordinate, plan testpilot.ReservationCarrierPlan, workflowBinding WorkflowBinding, handles []testpilot.ReservationHandle) (Bundle, error) {
	...
	for _, proxy := range proxies {
		...
		if entrypointContext == testpilotpb.ENTRYPOINT_KIND_WORKFLOW {
...
func validateBundle(runID string, origin testpilot.Coordinate, plan testpilot.ReservationCarrierPlan, workflowBinding binding, handles []testpilot.ReservationHandle, limits Limits) (map[reservationKey]testpilotpb.EntrypointKind, []testpilot.ReservationHandle, error) {
...
func validateTopology(plan testpilot.ReservationCarrierPlan, limits Limits) (map[reservationKey]testpilotpb.EntrypointKind, int, error) {
	expected := make(map[reservationKey]testpilotpb.EntrypointKind)
	workflowCount := 0
	handlerCount := 0
	for _, topology := range plan.Reservations {
		...
		if topology.Context != testpilotpb.ENTRYPOINT_KIND_WORKFLOW && topology.Context != testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER {
			return nil, 0, ErrInvalid
		}
		...
		if topology.Context == testpilotpb.ENTRYPOINT_KIND_WORKFLOW {
...
func validateHandles(origin testpilot.Coordinate, expected map[reservationKey]testpilotpb.EntrypointKind, handles []testpilot.ReservationHandle) ([]testpilot.ReservationHandle, error) {
...
func validateRoutes(plan testpilot.ReservationCarrierPlan, expected map[reservationKey]testpilotpb.EntrypointKind, handlerCount int) error {
	...
	for _, planned := range plan.Routes {
		...
		if expected[reservationKey{entrypoint: planned.WorkflowEntrypointID, ordinal: planned.WorkflowOrdinal}] != testpilotpb.ENTRYPOINT_KIND_WORKFLOW || expected[handler] != testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER || !validRouteText(planned.SourceInstructionID) || seenSources[source] || seenHandlers[handler] {
