...
func New(options Options) (*Driver, error) {
	...
	for _, role := range p.Roles {
		if role.Kind != testpilotpb.ROLE_KIND_ENDPOINT || len(role.Methods) == 0 {
			continue
		}
...
func (h *Driver) Identity(ctx context.Context) (testpilot.DriverIdentity, error) {
	...
	return testpilot.DriverIdentity{Profile: h.profile.Identity, Catalog: h.profile.Catalog.Identity()}, nil
}
...
func (h *Driver) open(ctx context.Context, runID string, program *testpilotpb.Program) (*Session, error) {
	if err := contextError(ctx); err != nil {
		return nil, err
	}
	...
	if cleanup := program.Cleanup; cleanup != nil {
		s.entries[cleanup.EntrypointId] = testpilotpb.ENTRYPOINT_KIND_CONTROLLER
...
func entrypointKind(entry *testpilotpb.EntrypointDefinition) testpilotpb.EntrypointKind {
	...
	case *testpilotpb.EntrypointDefinition_Activity:
		return testpilotpb.ENTRYPOINT_KIND_ACTIVITY
