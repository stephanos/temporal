...
func nexusSession(t *testing.T, h *Driver, source *testpilotpb.Case, run string) (*Session, testpilot.Coordinate) {
	t.Helper()
	program := proto.CloneOf(source.Program)
	node := rpcNode("complete", "")
	node.Instruction = &testpilotpb.Instruction{Instruction: &testpilotpb.Instruction_CompleteNexusOperation{CompleteNexusOperation: &testpilotpb.CompleteNexusOperation{CapabilitySlotId: "capability"}}}
	program.Entrypoints[0].Instructions = append(program.Entrypoints[0].Instructions, node)
	program.Entrypoints = append(program.Entrypoints, &testpilotpb.EntrypointDefinition{EntrypointId: "handler", Activation: &testpilotpb.EntrypointDefinition_NexusHandler{NexusHandler: &testpilotpb.NexusHandlerActivation{}}})
	for _, id := range []string{"capability", "other"} {
		program.Slots = append(program.Slots, &testpilotpb.SlotDefinition{SlotId: id, Content: &testpilotpb.SlotDefinition_OpaqueCapability{OpaqueCapability: &testpilotpb.OpaqueCapabilityType{}}})
	}
	s, err := h.open(t.Context(), run, program)
	require.NoError(t, err)
	return s, testpilot.Coordinate{RunID: run, EntrypointID: "handler", ActivationID: "activation", InstructionID: "respond", Attempt: 1}
}
func completionValue() *testpilotpb.Value {
	return &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "result"}}
}
...
func TestNexusCompletionPayloadAndOpaqueOwnership(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED, result.Outcome.Status)
...
func TestNexusFailureTimeoutSizeAndRedirect(t *testing.T) {
	for _, mode := range []string{"failure", "timeout", "oversized", "redirect", "truncated"} {
		t.Run(mode, func(t *testing.T) {
			...
			s, origin := nexusSession(t, h, source, "run")
			if mode == "timeout" {
				s.nodes[nodeKey{"controller", "complete"}].Limits.TimeoutMilliseconds = 20
			}
...
func TestCapabilityClosureAndLimits(t *testing.T) {
	...
	require.NoError(t, s.Diagnose(t.Context(), "run", &testpilotpb.RunDiagnostic{}))
...
func TestNexusQuarantineRetainsCapacityUntilTransportReturns(t *testing.T) {
	...
	result, err := handle.Wait(t.Context())
	require.NoError(t, err)
	require.Equal(t, testpilotpb.INSTRUCTION_OUTCOME_STATUS_CANCELED, result.Outcome.Status)
	h.mu.Lock()
