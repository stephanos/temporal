...
// CompletionInfo is supplied only by trusted Driver wiring at worker activation. It must never
...
// NewCompletionCapability is the injection seam for composite Driver wiring. The worker can
// receive this bound function without importing the server package. Minting does no target I/O.
func (s *Session) NewCompletionCapability(ctx context.Context, origin testpilot.Coordinate, info CompletionInfo) (testpilot.OpaqueCapability, error) {
	if err := contextError(ctx); err != nil {
		return nil, err
	}
	if origin.RunID != s.runID || origin.ActivationID == "" || len(origin.ActivationID) > 256 || s.entries[origin.EntrypointID] != testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER {
...
func (s *Session) CompleteNexusOperation(ctx context.Context, c testpilot.Coordinate, opaque testpilot.OpaqueCapability, value *testpilotpb.Value) (testpilot.EffectHandle, error) {
	...
	handle, err := s.startLocked(ctx, c, n.Limits, func(ctx context.Context) testpilot.EffectResult {
		return s.complete(ctx, info, data, min(n.Limits.MaxResponseBytes, s.host.profile.ProgramLimits.MaxResponseBytes))
	})
	if err != nil {
		return nil, err
	}
...
func (s *Session) complete(ctx context.Context, info CompletionInfo, data []byte, maxResponseBytes int64) testpilot.EffectResult {
	...
	outcome := &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED, ProtocolCode: "ok"}
	if err != nil {
		outcome.Status = testpilotpb.INSTRUCTION_OUTCOME_STATUS_PROTOCOL_NON_SUCCESS
