// Package testpilot composes controller transports and SDK workers behind one Testpilot Driver.
package testpilot
...
import (
	...
	"go.temporal.io/server/tests/testcore/testpilot/server"
	workerhost "go.temporal.io/server/tests/testcore/testpilot/worker"
...
func hasWorkerEntrypoint(program testpilot.PreparedProgram) bool {
	for _, entrypoint := range program.Entrypoints() {
		if entrypoint.Context() == testpilotpb.ENTRYPOINT_KIND_WORKFLOW ||
			entrypoint.Context() == testpilotpb.ENTRYPOINT_KIND_ACTIVITY ||
			entrypoint.Context() == testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER {
			return true
		}
	}
	return false
...
func (s *compositeSession) CompleteNexusOperation(ctx context.Context, coordinate testpilot.Coordinate, capability testpilot.OpaqueCapability, value *testpilotpb.Value) (testpilot.EffectHandle, error) {
...
func (s *compositeSession) Diagnose(ctx context.Context, runID string, diagnostic *testpilotpb.RunDiagnostic) error {
