...
func validWorkerProfile(profile testpilot.ProfileSpec) bool {
	...
	ceiling := &testpilotpb.ProgramLimits{
...
func (h *Driver) Identity(ctx context.Context) (testpilot.DriverIdentity, error) {
	...
	return testpilot.DriverIdentity{Profile: h.options.profile.Identity, Catalog: h.options.profile.Catalog.Identity()}, nil
}
...
func (h *Driver) prepareDefinitionPlans(snapshot *testpilotpb.Program, plans []testpilot.EntrypointPlan) (programDefinition, error) {
...
func (h *Driver) boundEntry(plan testpilot.EntrypointPlan) (entryDefinition, bool, error) {
...
func (h *Driver) addInstructionBindings(definition *programDefinition, plan testpilot.EntrypointPlan) error {
	for _, instruction := range plan.Instructions() {
		...
		if response := source.GetRespondNexus(); response != nil && response.GetKind() == testpilotpb.NEXUS_RESPONSE_KIND_ASYNCHRONOUS {
