...
func (s *Session) Reserve(ctx context.Context, request testpilot.ReservationRequest) ([]testpilot.ReservationHandle, error) {
	...
	if !exists || entry.plan.Context() != testpilotpb.ENTRYPOINT_KIND_WORKFLOW && entry.plan.Context() != testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER {
		return nil, ErrInvalid
	}
...
func (*Session) CompleteNexusOperation(context.Context, testpilot.Coordinate, testpilot.OpaqueCapability, *testpilotpb.Value) (testpilot.EffectHandle, error) {
	return nil, ErrUnsupportedOperation
}
...
func (s *Session) Diagnose(ctx context.Context, runID string, diagnostic *testpilotpb.RunDiagnostic) error {
...
func (s *Session) finishActivation(activation delivery.Activation, outcome *testpilotpb.InstructionOutcome, err error) {
