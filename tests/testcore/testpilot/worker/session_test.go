...
func TestConcurrentRunsRouteReorderedWorkflowAndNexusExactly(t *testing.T) {
	...
	headerB, valueB, err := sessionB.preparedNexusDispatch(workflowB.activation, "start", nil, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "b"}})
	require.NoError(t, err)
	headerA, valueA, err := sessionA.preparedNexusDispatch(workflowA.activation, "start", nil, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "a"}})
	require.NoError(t, err)
...
func TestAdmittedWorkflowUsesImmutableNexusDispatchAfterStop(t *testing.T) {
	...
	header, value, err := session.preparedNexusDispatch(routed.activation, "start", nexus.Header{"user": "value"}, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}})
	require.NoError(t, err)
...
func TestNewRejectsProfileLimitsBeforeRetainedStateAllocation(t *testing.T) {
	...
	require.NoError(t, err)
	limits := proto.CloneOf(preparedRuntimeFixture(t, testpilotpb.NEXUS_RESPONSE_KIND_SYNCHRONOUS).Snapshot().GetLimits())
...
func TestAsyncCompletionAuthorityIsOpaqueReplaySafeAndLateBounded(t *testing.T) {
	prepared := preparedRuntimeFixture(t, testpilotpb.NEXUS_RESPONSE_KIND_ASYNCHRONOUS)
	host, definition := runtimeTestDriver(t, prepared)
	...
	require.NoError(t, err)
	dispatchHeader, dispatchValue, err := session.preparedNexusDispatch(workflowRoute.activation, "start", nil, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	session.finishActivation(workflowRoute.activation, &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED}, nil)
	session.finishActivation(nexusRoute.activation, &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED}, nil)
	require.NoError(t, session.Close(t.Context()))
