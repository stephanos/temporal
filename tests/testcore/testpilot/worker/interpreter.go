...
type nexusResult struct {
	...
	kind  testpilotpb.NexusResponseKind
...
func (s *Session) executeWorkflow(ctx workflow.Context, activation delivery.Activation) (*testpilotpb.Value, error) {
	entry, exists := s.definition.entries[activation.Coordinate().EntrypointID]
	if !exists || entry.plan.Context() != testpilotpb.ENTRYPOINT_KIND_WORKFLOW {
...
func (i *workflowInterpreter) execute(instruction testpilot.InstructionPlan, input *testpilotpb.Value) (*testpilotpb.Value, bool, error) {
...
func (i *workflowInterpreter) startNexus(instruction testpilot.InstructionPlan, input *testpilotpb.Value) error {
...
func terminalOutcome(instruction testpilot.InstructionPlan, input *testpilotpb.Value) *testpilotpb.InstructionOutcome {
	...
	for _, field := range instruction.Source().GetOutcome().GetFields() {
		if field.GetField() == testpilotpb.INSTRUCTION_OUTCOME_FIELD_VALUE {
...
func outcomeForError(err error) *testpilotpb.InstructionOutcome {
	if err != nil {
		return sdkFailureOutcome(err)
	}
	return &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED}
}

func (s *Session) executeNexus(ctx context.Context, activation delivery.Activation, _ *testpilotpb.Value, options nexus.StartOperationOptions) (nexus.HandlerStartOperationResult[*testpilotpb.Value], error) {
...
func (s *Session) interpretNexus(ctx context.Context, activation delivery.Activation, options nexus.StartOperationOptions) (testpilotpb.NexusResponseKind, *testpilotpb.Value, string, error) {
	entry, exists := s.definition.entries[activation.Coordinate().EntrypointID]
	if !exists || entry.plan.Context() != testpilotpb.ENTRYPOINT_KIND_NEXUS_HANDLER {
...
func (s *Session) respondNexus(ctx context.Context, activation delivery.Activation, response *testpilotpb.RespondNexus, input *testpilotpb.Value, options nexus.StartOperationOptions) (testpilotpb.NexusResponseKind, *testpilotpb.Value, string, error) {
	switch response.GetKind() {
	case testpilotpb.NEXUS_RESPONSE_KIND_SYNCHRONOUS:
		if input == nil {
			return 0, nil, "", ErrInvalid
		}
		return response.GetKind(), proto.CloneOf(input), "", nil
	case testpilotpb.NEXUS_RESPONSE_KIND_ASYNCHRONOUS:
		return s.respondNexusAsync(ctx, activation, response, input, options)
	case testpilotpb.NEXUS_RESPONSE_KIND_ERROR:
...
func (s *Session) respondNexusAsync(ctx context.Context, activation delivery.Activation, response *testpilotpb.RespondNexus, input *testpilotpb.Value, options nexus.StartOperationOptions) (testpilotpb.NexusResponseKind, *testpilotpb.Value, string, error) {
...
func (r *nexusResult) response() (nexus.HandlerStartOperationResult[*testpilotpb.Value], error) {
	if r.err != nil {
		return nil, r.err
	}
	switch r.kind {
	case testpilotpb.NEXUS_RESPONSE_KIND_SYNCHRONOUS:
		return &nexus.HandlerStartOperationResultSync[*testpilotpb.Value]{Value: proto.CloneOf(r.value)}, nil
	case testpilotpb.NEXUS_RESPONSE_KIND_ASYNCHRONOUS:
...
func evaluateInstruction(ctx context.Context, values *activationValues, instruction testpilot.InstructionPlan) (*testpilotpb.Value, bool, error) {
...
func validateAndStore(ctx context.Context, values *activationValues, instruction testpilot.InstructionPlan, outcome *testpilotpb.InstructionOutcome) error {
...
func sdkFailureOutcome(err error) *testpilotpb.InstructionOutcome {
	outcome := &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SDK_FAILURE, SdkFailureCode: "sdk_failure", Detail: boundedText(err.Error())}
	if temporal.IsCanceledError(err) || errors.Is(err, context.Canceled) {
		outcome.Status = testpilotpb.INSTRUCTION_OUTCOME_STATUS_CANCELED
		outcome.SdkFailureCode = "canceled"
	} else if temporal.IsTimeoutError(err) || errors.Is(err, context.DeadlineExceeded) {
		outcome.Status = testpilotpb.INSTRUCTION_OUTCOME_STATUS_TIMED_OUT
