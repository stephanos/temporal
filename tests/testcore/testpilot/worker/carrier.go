...
func (s *Session) validCarrierBinding(plan testpilot.ReservationCarrierPlan, binding WorkflowBinding) bool {
	...
	for _, reservation := range plan.Reservations {
		if reservation.Context != testpilotpb.ENTRYPOINT_KIND_WORKFLOW {
			continue
		}
	...
	return exists && entry.plan.Context() == testpilotpb.ENTRYPOINT_KIND_WORKFLOW && entry.workflowType == binding.WorkflowType && entry.queue == binding.TaskQueue
}
