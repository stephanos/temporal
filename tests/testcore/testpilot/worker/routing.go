...
func (s *Session) preparedNexusDispatch(activation delivery.Activation, sourceID string, header nexus.Header, value *testpilotpb.Value) (nexus.Header, *testpilotpb.Value, error) {
...
func (s *Session) lateDiagnostic(ctx context.Context, code string) {
	...
	defer cancel()
	_ = s.Diagnose(ctx, s.runID, &testpilotpb.RunDiagnostic{Kind: testpilotpb.RUN_DIAGNOSTIC_KIND_POST_CLOSE_EVENT, Code: code, Detail: "reserved worker delivery rejected after Run closure"})
}
