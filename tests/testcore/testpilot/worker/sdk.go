...
func (i *workflowOutboundInterceptor) ExecuteNexusOperation(ctx workflow.Context, input interceptor.ExecuteNexusOperationInput) workflow.NexusOperationFuture {
	...
	value, valueOK := input.Input.(*testpilotpb.Value)
...
type genericNexusOperation struct {
	nexus.UnimplementedOperation[*testpilotpb.Value, *testpilotpb.Value]
	queue, service, operation string
}

func (o *genericNexusOperation) Name() string { return o.operation }

func (o *genericNexusOperation) Start(ctx context.Context, input *testpilotpb.Value, options nexus.StartOperationOptions) (nexus.HandlerStartOperationResult[*testpilotpb.Value], error) {
...
func (h *Driver) dynamicWorkflow(ctx workflow.Context, _ converter.EncodedValues) (*testpilotpb.Value, error) {
	routed, ok := ctx.Value(workflowRouteKey{}).(routedWorkflow)
	if !ok {
