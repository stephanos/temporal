...
import (
	...
	"go.temporal.io/server/tests/testcore/testpilot/internal/delivery"
)
...
func TestSDKWorkflowInterpretsStartAwaitAndFinishWithArbitraryArguments(t *testing.T) {
	for _, valueOutcome := range []bool{false, true} {
		t.Run(fmt.Sprintf("value=%t", valueOutcome), func(t *testing.T) {
			prepared := preparedRuntimeFixture(t, testpilotpb.NEXUS_RESPONSE_KIND_SYNCHRONOUS, func(program *testpilotpb.Program) {
				if valueOutcome {
			...
			operation := nexus.NewOperationReference[*testpilotpb.Value, *testpilotpb.Value]("operation")
			environment.OnNexusOperation(
				"service",
				operation,
				&testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}},
				mock.Anything,
			).Return(&nexus.HandlerStartOperationResultSync[*testpilotpb.Value]{Value: &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}}}, nil)
			...
			environment.AssertNexusOperationCalled(t, "service", "operation", &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}}, mock.Anything)
			workflowReservation := reservationForEntrypoint(t, session, "workflow")
			workflowResult, err := workflowReservation.Wait(t.Context())
			require.NoError(t, err)
			require.Equal(t, testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED, workflowResult.Outcome.GetStatus())
			handlerResult, err := reservationForEntrypoint(t, session, "handler").Wait(t.Context())
			require.NoError(t, err)
			require.Equal(t, testpilotpb.INSTRUCTION_OUTCOME_STATUS_CANCELED, handlerResult.Outcome.GetStatus())
...
func TestSDKWorkflowReplayerCompletesAnUnfinishedAdmission(t *testing.T) {
	prepared := preparedRuntimeFixture(t, testpilotpb.NEXUS_RESPONSE_KIND_SYNCHRONOUS)
	host, definition := runtimeTestDriver(t, prepared)
	...
	require.NoError(t, err)
	header, _, err := session.preparedNexusDispatch(routed.activation, "start", nil, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}})
	require.NoError(t, err)
...
func TestSDKNexusInboundRoutesRedeliveryThroughLedger(t *testing.T) {
	for _, valueOutcome := range []bool{false, true} {
		t.Run(fmt.Sprintf("value=%t", valueOutcome), func(t *testing.T) {
			...
			require.NoError(t, err)
			header, value, err := session.preparedNexusDispatch(workflowRoute.activation, "start", nil, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			require.Equal(t, "accepted", result.(*nexus.HandlerStartOperationResultSync[*testpilotpb.Value]).Value.GetText())
			_, err = reservationForEntrypoint(t, session, "handler").Wait(t.Context())
			require.NoError(t, err)
			replay, err := inbound.StartOperation(t.Context(), input)
			require.NoError(t, err)
			require.Equal(t, "accepted", replay.(*nexus.HandlerStartOperationResultSync[*testpilotpb.Value]).Value.GetText())
...
func TestSDKAdmittedWorkflowUsesCachedDispatchWhenStopRacesNextCommand(t *testing.T) {
	...
	environment.SetHeader(request.GetHeader())
	operation := nexus.NewOperationReference[*testpilotpb.Value, *testpilotpb.Value]("operation")
	environment.OnNexusOperation("service", operation, mock.Anything, mock.Anything).Return(&nexus.HandlerStartOperationResultSync[*testpilotpb.Value]{Value: &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}}}, nil)
	environment.RegisterDynamicWorkflow(func(ctx workflow.Context, arguments converter.EncodedValues) (*testpilotpb.Value, error) {
		close(entered)
		<-proceed
	...
	replayEnvironment.OnNexusOperation("service", operation, mock.Anything, mock.Anything).Return(&nexus.HandlerStartOperationResultSync[*testpilotpb.Value]{Value: &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}}}, nil)
...
func TestSDKConcurrentRunsAdmitReorderedWorkflowDelivery(t *testing.T) {
	...
	newEnvironment := func(binding WorkflowBinding, request *commonpb.Header, dynamic func(workflow.Context, converter.EncodedValues) (*testpilotpb.Value, error)) *testsuite.TestWorkflowEnvironment {
		var suite testsuite.WorkflowTestSuite
		environment := suite.NewTestWorkflowEnvironment()
		environment.SetWorkerOptions(sdkworker.Options{Interceptors: []interceptor.WorkerInterceptor{&sdkWorkerInterceptor{host: host, queue: "task-queue", registration: definition.registrations[0]}}})
		environment.SetStartWorkflowOptions(client.StartWorkflowOptions{ID: binding.WorkflowID, TaskQueue: binding.TaskQueue})
	...
	environmentB := newEnvironment(bindingB, requestB.GetHeader(), func(ctx workflow.Context, arguments converter.EncodedValues) (*testpilotpb.Value, error) {
		close(enteredB)
		<-proceedB
		return host.dynamicWorkflow(ctx, arguments)
	})
...
func workflowReplayHistory(t *testing.T, binding WorkflowBinding, header *commonpb.Header, nexusHeader nexus.Header) []*historypb.HistoryEvent {
	...
	require.NoError(t, err)
	nexusInput, err := dataConverter.ToPayload(&testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}})
	require.NoError(t, err)
	nexusResult, err := dataConverter.ToPayload(&testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}})
	require.NoError(t, err)
	workflowResult, err := dataConverter.ToPayloads(&testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}})
	require.NoError(t, err)
...
func TestSDKAwaitUsesItsOwnTimeout(t *testing.T) {
	for _, tc := range []struct {
		...
		{"await expires first", 10 * time.Second, time.Second, 5 * time.Second, testpilotpb.INSTRUCTION_OUTCOME_STATUS_TIMED_OUT},
		{"start expires first", time.Second, 10 * time.Second, 5 * time.Second, testpilotpb.INSTRUCTION_OUTCOME_STATUS_TIMED_OUT},
		{"completion before await", 10 * time.Second, 5 * time.Second, time.Second, testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, environment.RegisterNexusAsyncOperationCompletion("service", "operation", "token", &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "done"}}, nil, tc.completion))
			environment.ExecuteWorkflow(func(ctx workflow.Context) (int32, error) {
				...
				if err := i.startNexus(start, &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "request"}}); err != nil {
					return 0, err
				}
				...
				return i.values.lookup(testpilot.ValueReference{Kind: testpilot.OutcomeReference, Entrypoint: "workflow", ID: "await", Field: int32(testpilotpb.INSTRUCTION_OUTCOME_FIELD_STATUS)}).GetEnumValue().GetNumber(), nil
			})
			require.NoError(t, environment.GetWorkflowError())
			var status int32
