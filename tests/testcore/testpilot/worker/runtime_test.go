...
func TestReservationCancelBeforeAdmissionRetiresWithoutTargetCall(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, testpilotpb.INSTRUCTION_OUTCOME_STATUS_CANCELED, result.Outcome.GetStatus())
}

func TestActivationValuesOwnValidatedOutcome(t *testing.T) {
	values := newActivationValues("workflow", 8)
	original := &testpilotpb.Value{Value: &testpilotpb.Value_Text{Text: "result"}}
	values.store("await", &testpilot.OutcomeSnapshot{Outcome: &testpilotpb.InstructionOutcome{Status: testpilotpb.INSTRUCTION_OUTCOME_STATUS_SUCCEEDED, Value: original}, Fields: map[testpilotpb.InstructionOutcomeField]*testpilotpb.Value{testpilotpb.INSTRUCTION_OUTCOME_FIELD_VALUE: original}})
	original.Value = &testpilotpb.Value_Text{Text: "mutated"}
	reference := testpilot.ValueReference{Kind: testpilot.OutcomeReference, Entrypoint: "workflow", ID: "await", Field: int32(testpilotpb.INSTRUCTION_OUTCOME_FIELD_VALUE)}
