...
type TerminalDisposition = protocol.TerminalDisposition

const (
	ResponseSynchronous         = protocol.ResponseSynchronous
	ResponseAsynchronous        = protocol.ResponseAsynchronous
	ResponseDeferred            = protocol.ResponseDeferred
	ResponseBlocking            = protocol.ResponseBlocking
	ResponseFailure             = protocol.ResponseFailure
	TerminalDispositionSuccess  = protocol.TerminalDispositionSuccess
	TerminalDispositionFailure  = protocol.TerminalDispositionFailure
	TerminalDispositionUntagged = protocol.TerminalDispositionUntagged
)
...
func CompileExperiment(experiment protocol.Experiment) (Program, Plan, error) {
	...
	for index, action := range experiment.Actions {
		kind, known := actionCommandKind(protocol.ActionKind(action.Kind))
		if !known {
...
func ValidateActionMappings() error {
	...
	for _, action := range catalog.Actions {
		if _, known := actionCommandKind(protocol.ActionKind(action.Identifier)); !known {
...
func actionCommandKind(kind protocol.ActionKind) (CommandKind, bool) {
	switch kind {
	case protocol.ActionKindScheduleOperation, protocol.ActionKindDispatchTask,
		protocol.ActionKindWorkerReturnsSuccess, protocol.ActionKindPersistSuccess,
		protocol.ActionKindAckTask, protocol.ActionKindCloseNexusOperation,
		protocol.ActionKindLinkNexusActivity, protocol.ActionKindTimeoutNexusOperation:
		return CommandNexus, true
	case protocol.ActionKindRequestCancellation, protocol.ActionKindCommitCancellation:
		return CommandCancellation, true
	case protocol.ActionKindRetryTask, protocol.ActionKindAcquireOwnership,
		protocol.ActionKindRecoverOwner:
		return CommandRetry, true
	case protocol.ActionKindCrashOwner:
		return CommandFailure, true
	case protocol.ActionKindStartUpdate, protocol.ActionKindAcceptUpdate,
		protocol.ActionKindCompleteUpdate:
		return CommandUpdate, true
	case protocol.ActionKindRecordUpdateHistory, protocol.ActionKindDispatchWorkflowTask,
		protocol.ActionKindCompleteWorkflowTask, protocol.ActionKindEnqueueWorkflowTask,
		protocol.ActionKindDeliverWorkflowTask, protocol.ActionKindAcknowledgeWorkflowTask,
		protocol.ActionKindCreateSpeculativeWorkflowTask, protocol.ActionKindCommitSpeculativeWorkflowTask,
		protocol.ActionKindDispatchAssuranceWorkflowTask, protocol.ActionKindProgressEntity:
		return CommandWorkflow, true
	case protocol.ActionKindRegisterCallback:
		return CommandCallbackRegister, true
	case protocol.ActionKindRecordCallbackResponse:
		return CommandCallbackComplete, true
	case protocol.ActionKindContinueWorkflow:
		return CommandContinuation, true
	case protocol.ActionKindResetWorkflow:
		return CommandReset, true
	case protocol.ActionKindRouteWorkflowTask:
		return CommandRouting, true
	case protocol.ActionKindFenceWorkflowOwner:
...
func compileCommand(command Command) (Operation, string, error) {
	...
	return Operation{
		...
		Arguments: append([]protocol.NamedValue(nil), command.Arguments...), MaxBlockNanos: command.MaxBlockNanos,
	}, capability, nil
}
