...
func Run(promotionSource string) (Report, error) {
	...
	report := Report{
		...
		EntryPoint:    "umpire3test.RequireRegression",
...
func (r Report) Validate() error {
	if r.FormatVersion != FormatVersion || r.EntryPoint != "umpire3test.RequireRegression" {
...
func authoringCases() []authoredCase {
	first := scenario.ProtocolAtomicRegression("ux-first-regression",
	...
	partial := scenario.ProtocolAtomicRegression("ux-partial-order",
		[]scenario.Resource{scenario.Callback("callback")},
		scenario.AllPaths(
			scenario.AnyOrder(
	...
	identity := workflow.Regression("ux-runtime-identity", update,
		scenario.OnePath(update.Lifecycle(), update.CompletionThroughHistory()))
	operation := nexus.Operation("operation")
	fault := nexus.Regression("ux-typed-fault", operation, scenario.OnePath(
		operation.Schedule(),
		operation.Dispatch(),
...
func compileCase(capability Capability, authored scenario.Scenario) (Case, error) {
	...
	explained, err := umpire3test.Explain(authored, limits)
	if err != nil {
...
func inspectPromotion(source string) (Promotion, error) {
	...
	ast.Inspect(parsed, func(node ast.Node) bool {
		...
		promotion.RequireRegression = ok && packageName.Name == "umpire3test"
		return true
	})
	if !promotion.RequireRegression {
		return Promotion{}, errors.New("campaign promotion does not call umpire3test.RequireRegression")
	}
...
func isArtifactPlumbing(path string) bool {
	for _, forbidden := range []string{
		"/campaign", "/cmd/", "/internal/", "/model", "/model-checkers", "/protocol", "/replay",
	} {
