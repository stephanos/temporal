...
//go:embed results/cross-layer-mutation.audit.json
var defaultApprovedMutationAuditJSON []byte

func DefaultApprovedMutationAudit(source protocol.Experiment) (MutationGateReport, error) {
...
func (r MutationGateReport) Validate(source protocol.Experiment) error {
...
func (r MutationGateReport) ValidateBinding(experimentID string, property string, sourceDigest string) error {
	if r.FormatVersion != MutationGateReportFormatVersion ||
		...
		r.TrustBadge != protocol.TrustBadgeTestedInstance || r.Seed == 0 {
	...
	if !r.Replay.Reproduced || len(r.Replay.Drift) != 0 ||
		r.Replay.Baseline.Kind != umpire3runtime.ClaimViolating ||
		r.Replay.Current.Kind != umpire3runtime.ClaimViolating ||
...
func (r MutationGateReport) CanonicalJSON(source protocol.Experiment) ([]byte, error) {
...
func sealMutationGateReport(
	...
	report.ResultClass = protocol.ResultClassTraceWitness
	report.TrustBadge = protocol.TrustBadgeTestedInstance
	var err error
