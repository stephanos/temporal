...
type MutationRequest struct {
	...
	TopologyKinds []protocol.EntityKind
}
...
func Mutate(request MutationRequest) (MutationReport, error) {
	...
	clone := func() (protocol.Experiment, error) {
		var result protocol.Experiment
		if err := json.Unmarshal(encodedSource, &result); err != nil {
			return protocol.Experiment{}, fmt.Errorf("clone mutation source: %w", err)
		}
		return result, nil
	}
	report := MutationReport{Seed: request.Seed, Complete: true}
	var candidates []Mutation
	add := func(kind MutationKind, path string, experiment protocol.Experiment) {
	...
	for actionIndex, action := range request.Experiment.Actions {
		for argumentIndex, argument := range action.Arguments {
			for _, replacement := range request.Values {
				if replacement.Type != argument.Value.Type || replacement.Type == protocol.ValueSymbol {
					continue
				}
				candidate, err := clone()
				if err != nil {
	...
	for faultIndex := range request.Experiment.Faults {
		...
		for _, occurrence := range []protocol.FaultOccurrence{{First: 1, Count: 1}, {First: 2, Count: 1}} {
			candidate, err := clone()
			if err != nil {
				return MutationReport{}, err
			}
	...
	for _, kind := range request.TopologyKinds {
		...
		candidate.Resources = append(candidate.Resources, protocol.Resource{
...
func DefaultCoverageCatalog() ([]CoveragePoint, error) {
	...
	protobuf, err := protocol.DefaultProtobufInventory()
	if err != nil {
		return nil, err
	}
	...
	for _, schedule := range []protocol.OrderRelation{
		protocol.OrderUser, protocol.OrderSemantic, protocol.OrderSameSource, protocol.OrderRuntimeCausal,
	} {
