...
func PlanFor(graph protocol.FamilyDependencyGraph, target protocol.TargetID, repositoryRoot string) (Plan, error) {
	...
	if target == protocol.TargetIDNexusCancellation || target == protocol.TargetIDWorkflowUpdateLifecycle {
		makeTargets = append(makeTargets, "umpire3-check-proof")
	}
	goPackages := []string{
		"./tests/umpire3/protocol",
		"./tests/umpire3/scenario/...",
		"./tests/umpire3/explore",
	}
	for _, checker := range family.Checkers {
		...
		case "native":
			...
			goPackages = append(goPackages, "./tests/umpire3/model-checkers/native")
		case "veil":
			makeTargets = append(makeTargets, "umpire3-check-veil-results")
			goPackages = append(goPackages, "./tests/umpire3/model-checkers/veil")
		default:
...
func Run(ctx context.Context, plan Plan, runner Runner) error {
	if plan.RepositoryRoot == "" || len(plan.LeanModules) == 0 || len(plan.MakeTargets) == 0 ||
		...
		return fmt.Errorf("complete family check plan and runner are required")
	}
	leanArguments := []string{"exec", "--", "lake", "build"}
	leanArguments = append(leanArguments, plan.LeanModules...)
	if err := runner.Run(ctx, Command{
		Directory: filepath.Join(plan.RepositoryRoot, "tests", "umpire3", "model"),
