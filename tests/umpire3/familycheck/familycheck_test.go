...
func TestPlanSelectsOnlyTheFamilyCheckerPortfolio(t *testing.T) {
	...
	nexus, err := PlanFor(graph, protocol.TargetIDNexusCancellation, "/repo")
	require.NoError(t, err)
	...
	require.Contains(t, nexus.GoPackages, "./tests/umpire3/model-checkers/veil")
	require.NotEmpty(t, nexus.LeanModules)

	delivery, err := PlanFor(graph, protocol.TargetIDFoundationDeliverySafety, "/repo")
	require.NoError(t, err)
...
func TestRunStopsAtTheFirstFailedGate(t *testing.T) {
	...
	err := Run(context.Background(), Plan{
		RepositoryRoot: "/repo", LeanModules: []string{"Temporal.Product.Nexus"},
