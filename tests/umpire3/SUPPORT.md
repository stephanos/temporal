...
`testdata/umpire3-1.2.json` are attached by deployment owners for local in-process, CI test cluster,
...
The supported operator surface is `cmd/umpire3` with `explain`, `run`, `replay`, `campaign`,
`qualify`, and `promote` subcommands. Existing single-purpose commands remain buildable compatibility
entry points. Replay bundles use `umpire3/replay-bundle/v3`; qualification receipts use
