...
func normalize(root Node) (*normalizedPlan, error) {
	plan := &normalizedPlan{}
...
func normalizeNode(node Node, plan *normalizedPlan, suffix string) (normalizedGroup, error) {
	switch node.kind {
	case nodeAction:
	...
	case nodeRepeat:
		...
		children := make([]Node, node.repeatCount)
...
func normalizeSequence(children []Node, plan *normalizedPlan, suffix string) (normalizedGroup, error) {
	var result normalizedGroup
