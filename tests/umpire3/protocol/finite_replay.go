...
//go:embed generated/finite-replay-catalog.json
...
func DefaultFiniteReplayCatalog() (FiniteReplayCatalog, error) {
	return DecodeFiniteReplayCatalog(defaultFiniteReplayCatalogJSON)
}
...
func (t FiniteReplayTarget) Outcomes(action ActionKind) ([]ActionOutcome, bool) {
	attempt, found := t.attempt(action)
	if !found {
		return nil, false
	}
	return append([]ActionOutcome(nil), attempt.Outcomes...), true
}

func (t FiniteReplayTarget) Replay(requests []AttemptRequest) (AttemptReplay, error) {
	if len(requests) == 0 {
		return AttemptReplay{}, errors.New("finite replay requires actions")
	}
	states := append([]int(nil), t.InitialStates...)
	var liveOnly []ActionKind
	for _, request := range requests {
		attempt, found := t.attempt(request.Action)
		if !found {
		...
		for _, state := range states {
			for _, outcome := range outcomes {
				if outcome != ActionOutcomeApplied {
					next[state] = struct{}{}
					continue
				}
				for _, path := range attempt.AppliedPaths {
					for _, target := range t.follow([]int{state}, path) {
						next[target] = struct{}{}
					}
					if len(path) == 0 {
...
func (t FiniteReplayTarget) ReplayObserved(attempts []ObservedAttempt) (AttemptReplay, error) {
...
func (t FiniteReplayTarget) attempt(action ActionKind) (FiniteReplayAttempt, bool) {
	for _, attempt := range t.Attempts {
		if attempt.Action == action {
			return attempt, true
		}
	}
	return FiniteReplayAttempt{}, false
}

func validateFiniteReplayOutcomes(attempt FiniteReplayAttempt, outcomes []ActionOutcome) error {
	...
	for _, outcome := range outcomes {
		...
		if !slices.Contains(attempt.Outcomes, outcome) {
			return fmt.Errorf("action %q outcome %q is not declared by its Lean-derived finite mapping", attempt.Action, outcome)
		}
	}
	return nil
}
