...
func sdkObservationTruth(
	...
	}).Normalize(protocolexperiment.Checkpoint{Identifier: observationID, Observation: observationID}, completeSDKHistory(history))
	...
	require.NoError(t, err)
	program, ok := catalog.Program(protocolcatalog.ObservationID(observationID))
	require.True(t, ok)
...
func TestSDKFactNormalizerFeedsGeneratedObservationProgram(t *testing.T) {
	...
	normalizer := sdkFactNormalizer{
		experiment: protocolexperiment.Experiment{ExperimentID: "experiment", Actions: []protocolexperiment.Action{{
	...
	facts, err := normalizer.Normalize(protocolexperiment.Checkpoint{
	...
	catalog, err := observation.DefaultCatalog()
	require.NoError(t, err)
	program, ok := catalog.Program(protocolcatalog.ObservationIDUpdateAccepted)
	require.True(t, ok)
...
func TestSDKFactNormalizerSupportsEverySDKObservation(t *testing.T) {
	...
	session := &sdkSession{
		experiment: protocolexperiment.Experiment{Actions: []protocolexperiment.Action{
	...
	linkFacts, err := normalizer.Normalize(protocolexperiment.Checkpoint{
...
func TestSDKObservationFailsClosedOnMissingTargetEventOrFailedCommand(t *testing.T) {
	...
	}).Normalize(protocolexperiment.Checkpoint{Observation: "unknown-observation"}, completeSDKHistory(history))
...
func TestSDKNexusClosureRequiresOperationTerminalBeforeCallerClose(t *testing.T) {
	...
	session := &sdkSession{
		experiment: protocolexperiment.Experiment{Actions: []protocolexperiment.Action{{Identifier: "action", Kind: "complete-operation"}}},
		results:    map[string]participant.Result{"action": testSDKResult("completed", "temporal-sdk-participant")},
	}
	tests := []struct {
		name   string
...
func TestSDKNexusTimeoutObservationRejectsWrongMetadata(t *testing.T) {
	...
	session := &sdkSession{
		experiment: protocolexperiment.Experiment{Actions: []protocolexperiment.Action{{Identifier: "action", Kind: "timeout-operation"}}},
		results:    map[string]participant.Result{"action": testSDKResult("completed", "temporal-sdk-participant")},
	}
	history := historyPosition{
		events: map[enumspb.EventType]historyEventPosition{
...
func TestSDKNexusActivityObservationRequiresReciprocalLinks(t *testing.T) {
	...
	session := &sdkSession{
		experiment: protocolexperiment.Experiment{Actions: []protocolexperiment.Action{{Identifier: "link", Kind: "link-nexus-activity"}}},
...
func TestSDKCapabilitiesAreConservativeOrExplicitlyAttested(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []protocolcatalog.CapabilityID{
	...
	require.Equal(t, []protocolcatalog.CapabilityID{
		...
		protocolcatalog.CapabilityIDUpdate, protocolcatalog.CapabilityIDWorkflowTaskControl,
	}, capabilities)
...
func TestSDKSessionEmitsIndependentlyNormalizableCorroboratingFacts(t *testing.T) {
	...
	factSets, err := session.CorroborateFacts(context.Background(), protocolexperiment.Checkpoint{
	...
	catalog, err := observation.DefaultCatalog()
	require.NoError(t, err)
	program, ok := catalog.Program(protocolcatalog.ObservationIDEntityProgressed)
	require.True(t, ok)
...
func TestSDKSessionFailsClosedWhenIndependentHistorySourceFails(t *testing.T) {
	...
	session := &corroboratingSDKSession{
		sdkSession: &sdkSession{
			experiment: protocolexperiment.Experiment{ExperimentID: "experiment", Actions: []protocolexperiment.Action{{Identifier: "action"}}},
	...
	_, err := session.CorroborateFacts(context.Background(), protocolexperiment.Checkpoint{}, nil)
