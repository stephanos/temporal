...
func Execute(parent context.Context, experiment protocol.Experiment, options Options) (execution.Result, error) {
	deployment, err := Validate(options)
	if err != nil {
	...
	factory, err := NewSDKFactory(SDKFactoryOptions{
		Client: client, Registry: sdkWorker, Deployment: deployment, Namespace: options.Namespace,
	...
	bound, err := profile.Bind(deployment, factory)
	if err != nil {
		return execution.Result{}, err
	}
...
func (f *workerEnvironment) Capabilities() []protocol.CapabilityID {
...
func Validate(options Options) (profile.Profile, error) {
	if options.Address == "" || options.Namespace == "" || options.TaskQueue == "" ||
		options.BuildID == "" || options.Timeout <= 0 {
		return profile.Profile{}, errors.New("address, namespace, task queue, build, and positive timeout are required")
	}
	nexusValues := 0
	...
	if nexusValues != 0 && nexusValues != 3 {
		return profile.Profile{}, errors.New("nexus endpoint, service, and operation must be supplied together")
	}
	var spec profile.Spec
	switch options.Profile {
	case "local-in-process":
		spec = profile.Local(options.BuildID, options.Namespace, options.TaskQueue)
	case "ci-test-cluster":
		spec = profile.CI(options.BuildID, options.Namespace, options.TaskQueue)
	case "remote-deployment", "grpc-only-black-box":
		...
		if options.Profile == "remote-deployment" {
			spec = profile.Remote(endpoint, options.APIKey, options.BuildID, options.Namespace, options.TaskQueue)
		} else {
			spec = profile.BlackBox(endpoint, options.APIKey, options.BuildID, options.Namespace, options.TaskQueue)
		}
	default:
		return profile.Profile{}, fmt.Errorf("unsupported execution profile %q", options.Profile)
	}
	return profile.Define(spec)
}
