...
type Spec struct {
	...
	Capabilities        []protocol.CapabilityID `json:"capabilities,omitempty"`
}
...
func configurationDigest(config Spec) string {
	capabilities := append([]protocol.CapabilityID(nil), config.Capabilities...)
...
func catalogCapabilities() []protocol.CapabilityID {
	catalog, err := protocol.DefaultCatalog()
	if err != nil {
		return nil
	}
	result := make([]protocol.CapabilityID, len(catalog.Capabilities))
...
func withoutCapabilities(values []protocol.CapabilityID, removed ...protocol.CapabilityID) []protocol.CapabilityID {
	blocked := make(map[protocol.CapabilityID]struct{}, len(removed))
	for _, capability := range removed {
		blocked[capability] = struct{}{}
	}
	result := make([]protocol.CapabilityID, 0, len(values))
...
func (f *boundFactory) Capabilities() []protocol.CapabilityID {
	return append([]protocol.CapabilityID(nil), f.capabilities...)
}

func (f *boundFactory) Prepare(ctx context.Context, experiment protocol.Experiment) (environment.PreparedEnvironment, error) {
	...
	identity.Capabilities = append([]protocol.CapabilityID(nil), f.capabilities...)
...
func intersectCapabilities(left, right []protocol.CapabilityID) []protocol.CapabilityID {
	rightSet := make(map[protocol.CapabilityID]struct{}, len(right))
	for _, capability := range right {
		rightSet[capability] = struct{}{}
	}
	var result []protocol.CapabilityID
...
func hasFaultCapability(capabilities []protocol.CapabilityID) bool {
	for _, capability := range capabilities {
		if strings.HasPrefix(string(capability), "fault-") || capability == protocol.CapabilityIDFailoverControl {
			return true
		}
	}
	return false
...
func capabilityStrings(capabilities []protocol.CapabilityID) []string {
