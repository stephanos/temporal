...
booleans, use sleeps, or turn unsupported/inconclusive into a skip. Promoted mutation source uses
this same API and must compile without mutation-only support.

Regressions normally require conformance. A deliberate negative control uses
`regression.ExpectViolation()` with the same `RequireRegression` entry point; it still requires a
...
Adding a protobuf-backed behavior starts in `model/Temporal/API/testdata/fixtures/selection.json`. Generation imports
