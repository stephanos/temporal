...
def proofManifest : SemanticProofManifest where
  ...
  proof := resolved_simulation% Refinement.UpdateLifecycle.soundSimulation
  assumptions := [{
...
theorem experimentWellFormed : experiment.WellFormed := by
  simp [SemanticExperiment.WellFormed, experiment, Mechanisms.TaskDelivery.guarantee]
