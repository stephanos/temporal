...
def Projects (systemState : State)
    (productState : Umpire3.Temporal.Product.NexusTimeout.State) : Prop :=
  systemState.visible = productState

theorem initialProjects (systemState : State) (isInitial : system.Initial systemState) :
    ∃ productState, Umpire3.Temporal.Product.NexusTimeout.model.Initial productState ∧
      Projects systemState productState := by
  subst systemState
  exact ⟨Umpire3.Temporal.Product.NexusTimeout.initial, rfl, rfl⟩

theorem stepSimulates (systemState : State)
    (productState : Umpire3.Temporal.Product.NexusTimeout.State)
    ...
    ∃ nextProductState, Umpire3.Temporal.Product.NexusTimeout.model.StepStar
...
def nexusTimeoutRefinesProduct :
    Refinement system Umpire3.Temporal.Product.NexusTimeout.model where
  Relates := Projects
