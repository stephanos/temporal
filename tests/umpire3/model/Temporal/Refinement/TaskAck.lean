...
abbrev State := Product.TaskAck.State
abbrev Action := Product.TaskAck.Command
abbrev behavior : Behavior Unit := Behavior.ofTransitionSystem Product.TaskAck.product
...
theorem mappedRun {state action nextState} (step : nextState ∈ System.next state action) :
    Runs Product.TaskAck.product (project state) (actionMap action).actions
...
theorem mutationBreaksDeclaredSimulation :
  ...
  have finalState : nextFeatureState = Product.TaskAck.State.acknowledgedWithBacklog := projects.symm
  subst nextFeatureState
  rcases run.uncons with ⟨middle, invalidStep, tail⟩
  have middleIsFinal : middle = Product.TaskAck.State.acknowledgedWithBacklog :=
    Runs.empty tail
  subst middle
  change Product.TaskAck.product.Step .delivered .acknowledge .acknowledgedWithBacklog
    at invalidStep
  simp [Product.TaskAck.step] at invalidStep
