...
def Projects (systemState : State)
    (productState : Umpire3.Temporal.Product.WorkflowLineage.State) : Prop :=
  systemState.visible = productState

theorem initialProjects (systemState : State) (isInitial : system.Initial systemState) :
    ∃ productState, Umpire3.Temporal.Product.WorkflowLineage.model.Initial productState ∧
      Projects systemState productState := by
  subst systemState
  exact ⟨Umpire3.Temporal.Product.WorkflowLineage.initial, rfl, rfl⟩

theorem stepSimulates (systemState : State)
    (productState : Umpire3.Temporal.Product.WorkflowLineage.State)
    ...
    ∃ nextProductState, Umpire3.Temporal.Product.WorkflowLineage.model.StepStar
...
def workflowLineageRefinesProduct :
    Refinement system Umpire3.Temporal.Product.WorkflowLineage.model where
  Relates := Projects
