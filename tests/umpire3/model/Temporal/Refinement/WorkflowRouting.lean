...
def Projects (systemState : State)
    (productState : Umpire3.Temporal.Product.WorkflowRouting.State) : Prop :=
  systemState.visible = productState

theorem initialProjects (systemState : State) (isInitial : system.Initial systemState) :
    ∃ productState, Umpire3.Temporal.Product.WorkflowRouting.model.Initial productState ∧
      Projects systemState productState := by
  subst systemState
  exact ⟨Umpire3.Temporal.Product.WorkflowRouting.initial, rfl, rfl⟩

theorem stepSimulates (systemState : State)
    (productState : Umpire3.Temporal.Product.WorkflowRouting.State)
    ...
    ∃ nextProductState, Umpire3.Temporal.Product.WorkflowRouting.model.StepStar
...
def workflowRoutingRefinesProduct :
    Refinement system Umpire3.Temporal.Product.WorkflowRouting.model where
  Relates := Projects
