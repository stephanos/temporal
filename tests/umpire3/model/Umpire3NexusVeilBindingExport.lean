...
def main : IO Unit := do
  ...
  let some exported := Umpire3.Temporal.Veil.NexusCancellationFencing.soundBindingExport
    | throw (IO.userError "sound Veil binding does not match its checked first-order export")
