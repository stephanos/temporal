...
def main : IO Unit := do
  ...
  let some exported := Umpire3.Temporal.Targets.NexusCancellationFencing.soundFirstOrderExport
    | throw (IO.userError "sound first-order search did not produce a checked exhaustive certificate")
