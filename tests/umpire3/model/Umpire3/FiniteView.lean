...
structure FiniteView {World : Type u} (model : Behavior.{u, v} World) (world : World) where
...
def FiniteView.withFingerprint {World : Type u} {model : Behavior.{u, v} World} {world : World}
...
def FiniteView.initials {World : Type u} {model : Behavior.{u, v} World} {world : World}
    (view : FiniteView model world) : List (model.State world) :=
  view.executable.initials world

def FiniteView.successors {World : Type u} {model : Behavior.{u, v} World} {world : World}
