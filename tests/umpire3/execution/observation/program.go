...
type Program struct {
	...
	Observation protocolcatalog.ObservationID `json:"observation"`
...
//go:embed testdata/generated/programs.json
...
func (c Catalog) Validate() error {
	...
	knownObservations := make(map[protocolcatalog.ObservationID]struct{}, len(semanticCatalog.Observations))
	for _, declaration := range semanticCatalog.Observations {
		knownObservations[protocolcatalog.ObservationID(declaration.Identifier)] = struct{}{}
	}
	programs := make(map[protocolcatalog.ObservationID]Program, len(c.Programs))
	...
	for _, declaration := range semanticCatalog.Observations {
		observation := protocolcatalog.ObservationID(declaration.Identifier)
	...
	coveredPrograms := make(map[protocolcatalog.ObservationID]struct{}, len(c.Programs))
...
func (c Catalog) Program(observation protocolcatalog.ObservationID) (Program, bool) {
	for _, program := range c.Programs {
