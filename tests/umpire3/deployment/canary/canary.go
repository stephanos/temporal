...
import (
	...
	"go.temporal.io/server/tests/umpire3/internal/subprocess"
...
type Result struct {
	FormatVersion  string                  `json:"formatVersion"`
	ApprovalDigest string                  `json:"approvalDigest"`
	Runtime        umpire3execution.Result `json:"runtime"`
...
func (c Controller) Run(ctx context.Context, request Request) (result Result, retErr error) {
	...
	workerResponse, executeErr := runWorker(ctx, request, OperationExecute, result.Recovery,
		request.Approval.MaxDuration, request.Approval.MaxOutputBytes, subprocess.Limits{
	...
	if executeErr != nil {
		...
		if err := c.Store.Save(ctx, result.Recovery); err != nil {
			...
			if err := result.Runtime.BindEvidenceDigest(); err != nil {
				...
				result.StopReason = err.Error()
			} else if workerResponse.Result.Claim.Kind == umpire3execution.ClaimViolating {
	...
	cleanupResponse, cleanupErr := runWorker(cleanupCtx, request, OperationCleanup, result.Recovery,
		request.Approval.CleanupTimeout, request.Approval.MaxOutputBytes, subprocess.Limits{
			CPUSeconds: request.Approval.MaxCPUSeconds, MemoryBytes: request.Approval.MaxMemoryBytes,
		})
	cancelCleanup()
	if cleanupErr != nil || !cleanupResponse.CleanupComplete {
		...
		result.Runtime.Cleanup = umpire3execution.CleanupResult{Complete: true}
...
func (c Controller) ResumeCleanup(ctx context.Context, definition deployment.Profile, approval Approval, environment []string) (Result, error) {
	if c.Store == nil {
	...
	if recovery.FormatVersion != FormatVersion || recovery.ApprovalID != approval.Identifier ||
		...
		definition.Kind != deployment.KindCanary || definition.Namespace != approval.Namespace ||
	...
	response, cleanupErr := runWorker(cleanupCtx, request, OperationCleanup, recovery,
		approval.CleanupTimeout, approval.MaxOutputBytes, subprocess.Limits{
...
func preflight(request Request, store Store, authority ApprovalAuthority) (string, error) {
	...
	if request.Profile.Kind != deployment.KindCanary || !request.Profile.Environment.HardExecutionBudget ||
...
func runWorker(
	...
	worker, err := subprocess.Run(ctx, subprocess.Request{
...
func validateWorkerResult(request Request, digest string, result umpire3execution.Result) error {
	...
	if result.Claim.Kind != umpire3execution.ClaimConforming && result.Claim.Kind != umpire3execution.ClaimViolating {
...
func errorClass(err error) string {
	...
	switch {
	case errors.Is(err, subprocess.ErrDeadline):
		return "deadline"
	case errors.Is(err, subprocess.ErrOutputLimit):
		return "output-limit"
	default:
