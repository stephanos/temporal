...
func TestBenchmarkRecordsTenXDeterminismAndRecovery(t *testing.T) {
	...
	report, certificate, receipt, err := runBenchmark(
		...
		func(_ context.Context, _ []string, _ protocol.FirstOrderView, certificate Certificate) (
	...
	require.NoError(t, err)
	decoded, err := DecodeBenchmarkReport(bytes.NewReader(encoded), protocol.DefaultDecodeLimit,
