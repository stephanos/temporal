...
type CompactNode struct {
	State  protocol.FirstOrderState `json:"state"`
...
func DecodeCertificate(input io.Reader, limit int64, view protocol.FirstOrderView) (Certificate, error) {
...
func (c Certificate) CanonicalJSON(view protocol.FirstOrderView) ([]byte, error) {
...
func (c Certificate) Validate(view protocol.FirstOrderView) error {
	...
	for index, node := range c.Nodes {
		...
		if node.Parent == -1 {
			...
			if !slices.ContainsFunc(steps, func(step protocol.FirstOrderStep) bool {
...
func (r Receipt) Validate(certificate Certificate) error {
	if r.FormatVersion != ReceiptFormatVersion || r.CertificateDigest != certificate.Digest ||
		...
		r.ResultClass != protocol.ResultClassFiniteExhaustive ||
		r.TrustBadge != protocol.TrustBadgeCheckedCertificate ||
...
func firstOrderViewDigest(view protocol.FirstOrderView) (string, error) {
