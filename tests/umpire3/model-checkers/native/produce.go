...
type CounterexampleError struct {
	...
	State   protocol.FirstOrderState
}
...
func checkpointSeen(machine protocol.FirstOrderMachine, checkpoint Checkpoint) (map[string]struct{}, error) {
	seen := make(map[string]struct{}, len(checkpoint.Nodes))
...
func actionsFor(nodes []ExpandedNode, index int) []protocol.ActionKind {
	var reversed []protocol.ActionKind
