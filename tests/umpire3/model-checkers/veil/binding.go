...
type CompiledBinding struct {
	View               protocol.FirstOrderView `json:"view"`
	...
	ActionLabels       []protocol.TraceSource  `json:"actionLabels"`
...
func (a BindingArtifact) ValidateAgainst(view protocol.FirstOrderView) error {
	if err := a.Validate(); err != nil {
		return err
	}
...
func (a BindingArtifact) validateIdentity() error {
	...
	if a.BackendRevision != protocol.VeilBackendRevision {
...
func (b CompiledBinding) Validate() error {
	...
	for index, label := range b.ActionLabels {
		expected := protocol.ActionKind(b.View.Actions[index].Identifier)
	...
	enumSorts := make([]protocol.FirstOrderSort, 0, len(b.View.Sorts))
...
func (b SemanticBinding) Validate() error {
	...
	expectedTrust := protocol.TrustBadgeKernel
	if len(b.Axioms) != 0 {
		expectedTrust = protocol.TrustBadgeKernelWithDeclaredAxioms
	}
