...
//go:embed testdata/generated/control-plane.audit.json
...
func DefaultAudit() (Report, error) {
	return DecodeAudit(defaultAuditJSON, protocolexperiment.DefaultDecodeLimit)
}
...
func hostileDecodeRejected() bool {
	_, err := protocolrelease.DecodeReleaseManifest([]byte(`{"unknown":true}`))
...
func cardinalityLimitRejected() bool {
	...
	view.Bounds.ConcreteStateLimit = protocolchecker.MaxFirstOrderConcreteStateLimit + 1
	return view.Validate() != nil
}

func secretRedacted() bool {
	const secret = "umpire3-resilience-secret"
	definition, err := deployment.Define(deployment.Remote(
...
func deadlineEnforced(ctx context.Context) (bool, error) {
	...
	if !errors.Is(err, subprocess.ErrDeadline) {
...
func outputBoundEnforced(ctx context.Context) (bool, error) {
	...
	if !errors.Is(err, subprocess.ErrOutputLimit) {
...
func resourceLimitsApplied(ctx context.Context) (bool, error) {
	result, err := subprocess.Run(ctx, subprocess.Request{
		...
		Limits: subprocess.Limits{CPUSeconds: 1, MemoryBytes: 64 << 20},
	})
	if err != nil {
...
func transactionalPublication() (bool, error) {
	...
	if err := artifactio.Publish(path, []byte("first")); err != nil {
		return false, err
	}
	if err := artifactio.Publish(path, []byte("second")); err != nil {
		return false, err
	}
...
func workerCrashRecovered(ctx context.Context) (bool, error) {
	supervisor, err := subprocess.NewSupervisor(subprocess.Request{
	...
	return first.Generation == 1 && crashed.Termination == subprocess.TerminationCrash &&
		second.Generation == 2 && first.PID != second.PID && stopped.Termination == subprocess.TerminationStop, nil
}

func waitForOutput(ctx context.Context, supervisor *subprocess.Supervisor, expected string) error {
