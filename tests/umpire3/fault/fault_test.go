...
func TestPreflightRejectsEscapingUnsupportedAndRestrictedFaults(t *testing.T) {
	...
	require.NoError(t, Preflight(term, []protocol.CapabilityID{protocol.CapabilityIDFaultRpc}, false))

	term.Scope.Namespaces = []string{"one", "two"}
	require.ErrorContains(t, Preflight(term, []protocol.CapabilityID{protocol.CapabilityIDFaultRpc}, false), "one isolated namespace")

	restricted := term
	restricted.Kind = protocol.FaultKindClockSkew
	restricted.Scope = Scope{Namespaces: []string{"namespace"}, Participants: []string{"worker"}}
	require.ErrorContains(t, Preflight(restricted, []protocol.CapabilityID{protocol.CapabilityIDFaultClock}, false), "restricted")
}
