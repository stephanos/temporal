...
// NewUpdateTestEnv creates a new update test environment.
func NewUpdateTestEnv(t *testing.T, dynamicConfigOverrides ...map[dynamicconfig.Key]any) *UpdateTestEnv {
	env := testcore.NewTestEnv(t, dynamicConfigOverrides...)
...
func (s *UpdateTestEnv) T() *testing.T {
	return s.t
}

// TODO: extract sendUpdate* methods to separate package.

func (s *UpdateTestEnv) sendUpdate(ctx context.Context, tv *testvars.TestVars) <-chan updateResponseErr {
	s.T().Helper()
	return s.sendUpdateInternal(ctx, tv, nil, false)
}

func (s *UpdateTestEnv) sendUpdateNoError(tv *testvars.TestVars) <-chan *workflowservice.UpdateWorkflowExecutionResponse {
	s.T().Helper()
	return s.sendUpdateNoErrorInternal(tv, nil)
}

func (s *UpdateTestEnv) sendUpdateNoErrorWaitPolicyAccepted(tv *testvars.TestVars) <-chan *workflowservice.UpdateWorkflowExecutionResponse {
	s.T().Helper()
...
func (s *UpdateTestEnv) pollUpdate(tv *testvars.TestVars, waitPolicy *updatepb.WaitPolicy) (*workflowservice.PollWorkflowExecutionUpdateResponse, error) {
	s.T().Helper()
...
// updateWorkflowRequest creates an update workflow request
func (s *UpdateTestEnv) updateWorkflowRequest(
...
// sendUpdateNoErrorInternal is the internal implementation
func (s *UpdateTestEnv) sendUpdateNoErrorInternal(tv *testvars.TestVars, waitPolicy *updatepb.WaitPolicy) <-chan *workflowservice.UpdateWorkflowExecutionResponse {
	s.T().Helper()
...
// sendUpdateInternal is the core update sending logic
func (s *UpdateTestEnv) sendUpdateInternal(
	ctx context.Context,
	tv *testvars.TestVars,
	waitPolicy *updatepb.WaitPolicy,
	requireNoError bool,
...
// waitUpdateAdmitted waits for an update to be admitted
func (s *UpdateTestEnv) waitUpdateAdmitted(tv *testvars.TestVars) {
	s.T().Helper()
	for i := 0; i < 500; i++ {
		...
		if pollErr == nil && pollResp.GetStage() >= enumspb.UPDATE_WORKFLOW_EXECUTION_LIFECYCLE_STAGE_ADMITTED {
			return
		}
	}
	s.T().Fatalf("update %s did not reach Admitted stage", tv.UpdateID())
}

// startWorkflow starts a workflow and returns its run ID
func (s *UpdateTestEnv) startWorkflow(tv *testvars.TestVars) string {
	s.T().Helper()
...
func (s *UpdateTestEnv) clearUpdateRegistryAndAbortPendingUpdates(tv *testvars.TestVars) {
...
func (s *UpdateTestEnv) loseUpdateRegistryAndAbandonPendingUpdates(tv *testvars.TestVars) {
	cleanup := s.OverrideDynamicConfig(dynamicconfig.ShardFinalizerTimeout, 0)
	defer cleanup()
	s.closeShard(tv.WorkflowID())
}

func (s *UpdateTestEnv) speculativeWorkflowTaskOutcomes(
  ...
  7 WorkflowTaskStarted`, task.History)
				...
				msgHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*protocolpb.Message, error) {
					...
					case 1:
						return nil, nil
					case 2:
						s.Require().Positive(len(task.Messages), "Task has no messages", task)
						updRequestMsg := task.Messages[0]
						updRequest := protoutils.UnmarshalAny[*updatepb.Request](s.T(), updRequestMsg.GetBody())
...
`, events)
	...
	t.Run("CompleteWorkflow_AbortUpdates", func(t *testing.T) {
		type testCase struct {
			...
			commands      func(s *UpdateTestEnv, tv *testvars.TestVars) []*commandpb.Command
			messages      func(s *UpdateTestEnv, tv *testvars.TestVars, updRequestMsg *protocolpb.Message) []*protocolpb.Message
		}
		...
		testCases := []testCase{
			{
				...
				updateFailure: "",
				commands:      func(s *UpdateTestEnv, _ *testvars.TestVars) []*commandpb.Command { return nil },
				messages:      func(s *UpdateTestEnv, _ *testvars.TestVars, _ *protocolpb.Message) []*protocolpb.Message { return nil },
			},
			{
				...
				commands:      func(s *UpdateTestEnv, tv *testvars.TestVars) []*commandpb.Command { return s.UpdateAcceptCommands(tv) },
				messages: func(s *UpdateTestEnv, tv *testvars.TestVars, updRequestMsg *protocolpb.Message) []*protocolpb.Message {
			...
			},
			{
				...
				commands:      func(s *UpdateTestEnv, tv *testvars.TestVars) []*commandpb.Command { return nil },
				messages: func(s *UpdateTestEnv, tv *testvars.TestVars, updRequestMsg *protocolpb.Message) []*protocolpb.Message {
				...
				t.Run(tc.name+" "+wfCC.name, func(t *testing.T) {
					...
					wtHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*commandpb.Command, error) {
						...
						case 1:
							...
							return nil, nil
						case 2:
							return append(tc.commands(s, tv), wfCC.command(tv)), nil
						default:
							s.Failf("wtHandler called too many times", "wtHandler shouldn't be called %d times", wtHandlerCalls)
							return nil, nil
						}
					...
					msgHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*protocolpb.Message, error) {
						...
						case 2:
							updRequestMsg := task.Messages[0]
							return tc.messages(s, tv, updRequestMsg), nil
						default:
							s.Failf("msgHandler called too many times", "msgHandler shouldn't be called %d times", msgHandlerCalls)
							return nil, nil
						}
...
`
  ...
  4 WorkflowTaskCompleted`, s.GetHistory(s.Namespace().String(), tv.WorkflowExecution()))
	...
	t.Run("UpdateWithStart", func(t *testing.T) {
		...
		sendUpdateWithStart := func(s *UpdateTestEnv, ctx context.Context, startReq *workflowservice.StartWorkflowExecutionRequest, updateReq *workflowservice.UpdateWorkflowExecutionRequest) chan multiopsResponseErr {
			capture := s.GetTestCluster().Host().CaptureMetricsHandler().StartCapture()
			defer s.GetTestCluster().Host().CaptureMetricsHandler().StopCapture(capture)
		...
		startWorkflowReq := func(s *UpdateTestEnv, tv *testvars.TestVars) *workflowservice.StartWorkflowExecutionRequest {
...
func (s *UpdateTestEnv) closeShard(wid string) {
	s.T().Helper()
...
func (s *UpdateTestEnv) useRunID(tv *testvars.TestVars, useRunID bool, runID string) *testvars.TestVars {
	if useRunID {
...
// requireNotStartedButRunning checks that a workflow was not started but is running
