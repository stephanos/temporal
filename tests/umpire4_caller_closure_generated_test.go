...
var callerClosureInputBinding = runner.InputBinding{
...
func admitCallerClosure(t *testing.T) (artifact.AdmittedSet, error) {
	t.Helper()
	return loadUmpireCallerClosureInputSet(t, "caller-closure-input-set"), nil
}
...
func newCallerClosurePath(
	...
	t.Helper()
	experiment := loadUmpireCallerClosureArtifact(t, "caller-closure-input-set", "artifacts/experiment.json")
	return callerClosurePath{
		checkSubject: func() error {
			return runevaluation.CheckSubject(experiment, callerClosureSubject)
		},
		admit: func() (artifact.AdmittedSet, error) { return admitCallerClosure(t) },
		run: func(ctx context.Context, input artifact.AdmittedSet) (umpireruntime.Output, error) {
			return runner.Run(
				ctx, input, callerClosureInputBinding, runIdentity, adapter,
			)
		},
...
func runCallerClosureEvaluation(
	...
	if _, err := artifact.DecodeResultV2(resultBytes); err != nil {
		return callerClosureEvaluation{}, err
	}
	var result callerClosureResult
...
// TestUmpireCallerClosurePortability runs the exact
// generated two-member input through the bounded runner and shared Run Evaluation path.
func TestUmpireCallerClosurePortability(t *testing.T) {
	env, factory := newUmpireTestEnvironment(t)
	binding := newUmpireNexusBinding(t, factory)
	ctx, cancel := context.WithTimeout(env.Context(), 315*time.Second)
	defer cancel()
	localPath := newCallerClosurePath(
		t, "umpire.local.caller-closure.portability-reference-1", binding,
	)
	ciPath := newCallerClosurePath(
		t, "umpire.ci.caller-closure.portability-proof-1", binding,
	)
	...
	requireCallerClosureBoundedProofMatrix(
		t, localResult.StableMeaning, callerClosureBoundedProofOutcomes(ctx, t, binding, localOutcome),
	)
	limitOutcome := callerClosureLimitNPlusOneOutcome(ctx, t, binding)
