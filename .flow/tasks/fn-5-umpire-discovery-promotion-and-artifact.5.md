...
- Register exactly `temporal.nexus.caller-closure.promotion.cancel-unique-regression` against two
  explicit lineages: the unchanged `exactActionQuery`, target, kernel, PlanResult, and base
  `Plan` that supply the expected count-one trace; and the selected duplicate-delivery
  Space-point `Plan` whose distinct identity/checksum is reproduced and minimized by fn-22.
...
- [ ] The unchanged base Query/PlanResult/base Plan supplies the target-owned expected count-one trace, while the selected fault-bearing Plan retains its separate identity/checksum; neither is conflated with the observed count-two result.
- [ ] The two promoted identities are fixed, fresh, and collision-checked against every retained Nexus discovery identity.
- [ ] Changed candidate, base Query/plan/Plan, fault-bearing Plan, target/kernel/import/promoted identity, or source digest fails before a sealed source is returned.
...
- Tests: baseline: red (cd model && mise exec -- lake build Temporal.Tool.NexusDiscoveryTests Temporal.Tool.PromotionTests TemporalExperimentalTests umpire-inspect temporal-model-promote failed pre-edit: future task .6 target temporal-model-promote is absent), baseline: red (make umpire-check-regression failed pre-edit: known model/Umpire/SemanticInventory/KnownGaps.lean:296 Temporal-owned vocabulary finding), cd model && mise exec -- lake build Temporal.Feature.Nexus.Experimental.CallerClosurePromotionTests Temporal.Tool.PromotionBindingTests, make lint-model, git diff --check, Codex impl-review SHIP: /tmp/impl-review-receipt-fn-5-umpire-discovery-promotion-and-artifact.5.json
- PRs: