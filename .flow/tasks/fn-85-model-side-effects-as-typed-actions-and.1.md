...
- [ ] `Umpire.Case.Producer` assembles Program and Contract from a path and a realization binding action classes to instructions, RPCs and entrypoints; no Nexus-specific branch exists in it (a grep for `nexus` in `Umpire/` is empty)
