...
- Parse the fixed protected `ProtectedCanaryAuthority/v2` under the 1-MiB closed-field contract, retain secrets only in memory, and accept no target/credential from arguments or artifacts.
...
- Construct only secret-free checked digests and closed trust/Known Gap values; explicitly represent the protected assertion as operational provenance rather than independently audited fact.
...
### Key context
The adapter can verify exact dedicated routing and run-owned identities, but it cannot prove global production inactivity. Preserve that limitation in trust/Known Gap data and never weaken the pre-mutation gate.
