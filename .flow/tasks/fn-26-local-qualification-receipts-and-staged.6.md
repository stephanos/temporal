...
## Description
Close the Profile, subject, receipt, reason, multiplicity, cap, Known Gap, publication and output matrices: a second, test-only Profile (a JSON fixture in the Go tests, never declared in Lean or embedded) assesses the same subject to a different receipt without touching the first; the same subject and Profile publish byte-identical receipts twice; every crossed (a correlated-only Case included), stale, open, inconsistent, incompatible and oversized subject rejects before assessment, at N and N+1 of each cap; an inconclusive Verdict is `incomplete` and a violated one `rejected`. Live proof in the Testpilot suite: record the caller Model's `asyncCompletion` Case with `umpire-run --record` against the test cluster, assess it with `umpire-assess run` twice under `local-ephemeral` (accepted, one receipt, the second `already-published`), and assess the negative control's pinned record (`rejected`, its Verdict violated) -- no Driver or Run is created by either assessment. Document the exact environment-scoped claim, the lack of self-authentication, the separation between the Case Runtime's Verdict and offline Claim Assessment, and the retained exclusions in `model/README.md`, `.plans/UMPIRE4_COMPONENTS.md` and `.plans/UMPIRE4_SPEC.md`.
