...
## Done summary
Extracted deterministic DefinitionGraph analysis behind focused imports and migrated Behavior and Observation adapters while preserving validation staging, diagnostics, fingerprints, and their divergent historical cycle witnesses. Added empty/disconnected/acyclic, mixed-fault, multiple-graph-fault, divergent-witness, and dense-graph regressions; the review-driven worklist fix eliminates exponential sibling re-expansion.

stage: impl-review - ran [2026-09-03T02:13:39Z..2026-09-03T02:30:37Z]
stage: plan-sync - skipped(config: planSync.enabled != true)

## Evidence
- Commits: 6203a2924701c539acca14a429c3abd80acc49b1, 7e5e222ab3d4c7484200f7a33cb17137405fe440
- Tests: baseline: green, GATE_SKIPPED:unittest:green-receipt da89554d - baseline reused from prior post-gate pass, GATE_SKIPPED:smoke:green-receipt da89554d - baseline reused from prior post-gate pass, TDD_RED: cd model && mise exec -- lake build Umpire.Behavior.Tests.Validation Umpire.Observation.Tests.Compilation (failed only because Umpire.Shared.DefinitionGraph was missing), TDD_RED: gtimeout 5s sh -c 'cd model && mise exec -- lake build Umpire.Behavior.Tests.Validation' (exit 124 before reachability fix), gtimeout 5s sh -c 'cd model && mise exec -- lake build Umpire.Behavior.Tests.Validation', cd model && mise exec -- lake build Umpire.Behavior.Tests.Validation Umpire.Observation.Tests.Compilation, cd model && mise exec -- lake build Umpire.Behavior.Tests Umpire.Observation.Tests, cd model && mise exec -- lake build Umpire.Property.Tests Umpire.Behavior.Tests Umpire.Query.Tests Umpire.Observation.Tests Umpire.ImplementationLink.Tests UmpireTests, make umpire-build-model, make lint-model, cd model && mise exec -- lake env lean ../.flow/tmp/Fn43Task4Trust.lean, GREEN_RECEIPT: .flow/tmp/green-receipts/7e5e222a-unittest.json, GREEN_RECEIPT: .flow/tmp/green-receipts/7e5e222a-smoke.json
- PRs:
