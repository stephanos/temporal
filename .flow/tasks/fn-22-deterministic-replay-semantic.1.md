...
## Acceptance
- [ ] Crossed, stale, noncanonical, incomplete, unsupported (a supporting sequence naming no event), non-violated and duplicate inputs fail before `binding.Open`, each with its own reason; the subject is prepared under the recorded Profile name and `stale` is a catalog or bindings fingerprint other than the recorded one; `binding.Prepare` opens no connection and provisions nothing.
...
- [ ] `PreparedCase.Evaluate` reproduces a recorded Run's Verdict offline and names each violated rule's violating evidence; it is the only semantic-replay path; `umpire-run --record` and `umpire-fuzz --record-root` write the recorded Run shape; no bundle, digest, trust store or reader is introduced.
## Done summary
TBD
