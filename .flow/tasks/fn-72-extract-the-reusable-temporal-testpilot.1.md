...
- Rename only the composite package to `temporaltestpilot`; keep the `server`, `worker`, and private `internal/delivery` package roles intact.
...
Review: SHIP after correcting the moved root package comment to name `temporaltestpilot`; receipt `/tmp/impl-review-receipt-fn-72.1-synthetic-correct.json`.
