...
- Outage Model: states `pending`, `completed`; Action `awaitCompletion`; Fact `completed`; Property requiring `state completed` and `fact completed`; the Scenario from the spec; `limits`; a `find` Query; `case ... fixture "worker-outage" as workflow "umpire-worker-outage-workflow" evidence awaitCompletion ← history workflowExecutionCompleted`.
