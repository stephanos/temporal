...
- Let the Lean bridge validate each full admitted Result closure, compute one opaque checked admission identity plus reproduction-tuple digest, and construct fn-17's domain-neutral `ExplorationObservation/v2`; Go-visible metadata stays limited to admission, bindings, leases, and transport limits and never supplies semantic/evidence vocabulary, coverage, corpus, priority, or mutation feedback.
- Add strict `umpire-campaign-checkpoint/v2` codecs and ArtifactSet relationships that bind—but do not alter—the fn-18 coverage checkpoint, environment input set, leases/attempts/results, parent/generation, versions, Limits, Known Gaps, and provenance.
...
- [ ] Campaign-checkpoint closure binds coverage state/report, input set, artifacts, leases/attempts, parent/generation, Limits, versions, Known Gaps, and provenance with one-at-a-time relationship mutations.
