...
- Strictly extract the supported format, query identity, semantic identity, provenance sources, property identities, and observation requirements from canonical JSON; derive the lowercase `sha256:` fingerprint from decoded semantic-identity bytes.
- Render a thin generated Go test that calls the task-2 verifier and a generated Markdown index from the same projection record. Normalize Go with `go/format`, sort every rendered collection, and use exact UTF-8/LF bytes with no environmental data.
- Use injected/synthetic inputs for malformed and collision cases, plus the production fixture for the early proof. Do not implement a general JSON canonicalizer or decode procedural semantics beyond the metadata the projections display.
...
### Key context
`ExperimentSpec.semanticIdentity` is a large canonical value, not a short digest. The projection fingerprint is SHA-256 of that decoded string, never of raw JSON or rendered documentation. The exact Go generated marker must appear before the package clause.
...
- [ ] Supported production input yields one validated projection with exact ID, format, sorted sources, properties, requirements, and the specified `sha256:` fingerprint.
- [ ] Generated Go parses/formats, has the standard marker and one collision-checked `TestXxx`, and delegates only to the shared verifier; Markdown carries matching metadata and a no-runtime notice.
...
- [ ] Focused tests reject malformed/empty JSON, unsupported version, identity mismatch, missing/duplicate/unsafe provenance, empty identity, unsafe manifest paths, invalid/colliding Go names, and Go/Markdown metadata divergence.
