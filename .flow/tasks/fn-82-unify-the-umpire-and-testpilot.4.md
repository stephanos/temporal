...
- `model/Umpire/Property/Authoring.lean:93-137,269-289` — `bounded_response%`, `Property`, `property%`
- `model/Umpire/Behavior/Language.lean:134-215,669-783,803-900` — declaration, JSON keys, entry points
- `model/Umpire/Behavior/Authoring.lean:9-100,292-314` — the two sugar records and `scenario%`

**Optional** (reference as needed):
- `model/Temporal/Feature/Nexus/Operations/SuccessfulCompletion.lean` — production consumer of `Property`/`Scenario`
...
- Retire `Scenario`, `CheckedScenario`, `Scenario`, `Scenario`, `Umpire.Behavior`, `scenario%`, `Property`, `Property`, `bounded_response%`, `sameStepCases`, `quiescentWithin`; never the bare word `Behavior`.
...
- The Scenario record merge is the expensive part. `Scenario`, `Scenario` and
  `Scenario` must collapse into one `Scenario` record whose only construction operations
  ...
  `Umpire/Behavior/Tests/Authoring.lean`), so each `def x : Scenario := { ... }` becomes a
  named-argument call to `Scenario.exactly` (or its constraint counterpart for `Scenario`).
- `model/Umpire/Behavior/Language.lean` splits cleanly at its first `private def quote` line:
  everything above is the `Umpire/Scenario.lean` types module (through `CheckedScenario`, which has
  no private constructor), everything below is `Umpire/Scenario/Check.lean`.
- `Scenario.Step`/`Scenario.Count`/`Scenario.Order`/`Scenario.Role` cannot take the bare names
  `Step`/`Count`/`Order`/`Role`: `Umpire.Step` is the model step since `.2`. Namespace them under
  `Scenario`.
- `Scenario.Trace` should become `Scenario.Trace`, not the bare `Trace` the R2 table reserves:
