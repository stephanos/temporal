...
- Rows (all consumers already found by research; verify each with the stated command):
  ...
  - gomad family: nested modules `tools/gomad`, `tools/gomad2` (module path `github.com/temporalio/gomad`, root `go.mod:62` require + `:263` replace), `tools/gomad3` plus `qualification/corpus/go.mod`; root-module trees gomad1, gomad3sim, gomad3integration (build tag `gomad3_integration`, `Makefile:344`); `gomad3.yml`; `.gitattributes:3-4`; `.github/.yamlfmt:13`; prune clauses `Makefile:240,242,1396,1477` for a `.toolchain` dir that does not exist on disk.
  ...
  - `tests/testcore/monitor`, legacy `tests/umpire[23]_*.go`, `tests/probe`, `tests/gomadfunctional`.
...
- Search surfaces to run and cite: `git grep -n -E 'server/(tools/(gomad|umpire[123]|agentworkflow)|cmd/umpire-genmodels|common/testing/umpire)' -- '*.go'`, the same over `Makefile`, `.github`, `*.sh`, `*.toml`, `lakefile*`, `*.proto`, `model/**/*.lean`, `*.json`, `*.md`.
