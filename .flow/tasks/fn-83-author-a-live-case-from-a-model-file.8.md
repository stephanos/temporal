...
### Approach
- Tutorial: empty file to green test in order: `enum` (or `inductive`) declarations, `model`, `property`, `scenario`, `limits`, `query`, `case`, `lake build`, `make umpire-gen-case-runtime-conformance`, the Go test, `make umpire-check-regression`; then `umpire-run` against a dev server; then a section listing every located diagnostic from .3 and .5 with its fix; then the fault lines with the outage Model as the example.
