...
- Recompute `artifactOfSelection originalQuery trace reason originalRun.result.metadata.explored` from the found result and require whole-value equality with the sole `originalRun.artifact` before compiling anything; this binds Query, witness, reason, explored counts, plan, semantic identities, properties, requirements, and provenance without treating `PlannerRun` as carrying checked Query semantics.
...
- Compare the replanned witness/reason, properties, target, query form, policy, bounds, and semantic digests against independent expected inputs. Return newly derived promoted declaration IDs/digests and plan/artifact identities with ordinary recomputed promoted provenance plus original Query/artifact identity and provenance lineage; never require promoted identity or provenance equality with the original artifact.
- Keep deterministic Lean source and qualified Temporal constants outside this reusable module.
...
- `model/Umpire/Artifact.lean:193-241` — canonical semantic identity.
...
## Acceptance
- [ ] The canonical checked catalog, explicit fresh promoted IDs, an independently produced Switch checked Query plus exactly bound `PlannerRun.found`, and matching target-indexed kernel compile into one checked exact proposal with the same trace/reason and new checked Behavior/Query/plan/artifact identities.
...
- [ ] Original Query/artifact lineage identities and provenance are retained exactly and are provably distinct from the promoted Query/artifact identities and ordinarily recomputed promoted provenance.
- [ ] Any whole-value mismatch between the stored original artifact and canonical recomputation from Query, found trace/reason, and explored counts returns no partial proposal, as do missing IDs, collisions with canonical identities or aliases, catalog/reference/digest mismatch, loose or non-`.found` selections, form/reason/property mismatch, unresolved roles, failed target equality, incompatible bounds, and trace/reason drift.
- [ ] Properties, target, query form, policy, bounds, witness, selection reason, and source provenance are retained exactly.
