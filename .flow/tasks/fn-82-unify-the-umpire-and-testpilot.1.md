...
- Add a `breaking.ignore` entry for `temporal/server/api/testpilot/v1` in `proto/internal/buf.yaml` so `develop/buf-breaking.sh:63` stays green through the R4 rename; run `make lint-protos` to confirm.
- Extend `retired_vocabulary_test.go` (three tests at `:13`, `:86`, `:102`) with: missing listed path fails with the path in the message; bare-word rule rejected; stale spec id fails.
...
- [ ] `proto/internal/buf.yaml` ignores the testpilot package for breaking checks and `make lint-protos` passes
