...
**Size:** M
**Files:** `.github/workflows/umpire-production-canary-claim-assessment.yml`, `tools/umpire/temporal/canary/**`, `tools/umpire/remoteassessment/**`, `tools/umpire/canaryassessment/**`, `tools/umpire/cmd/umpire-assess-production-canary/**`
**Touches:** [.github/workflows/umpire-production-canary-claim-assessment.yml, tools/umpire/temporal/canary/**, tools/umpire/remoteassessment/**, tools/umpire/canaryassessment/**, tools/umpire/cmd/umpire-assess-production-canary/**]
...
- Test controller RPC partitions at N/N+1, prove the 24-call cleanup/reconcile reserve cannot be borrowed or reset through RemoteRecoveryRecord v3, and prove staging v2 stays unchanged. Separately test idle polling, response retry, redelivery, cancellation, and timeout using one-slot `WorkflowTaskPollerBehavior`/`NexusTaskPollerBehavior`, a one-slot WorkerTuner, `LocalActivityWorkerOnly:true`, zero legacy concurrency fields, and no activity registrations.
...
- [ ] Actual controller RPC accounting and worker transport Limits are tested under idle, redelivery, cancellation, ambiguity, cleanup, and N/N+1 cases.
