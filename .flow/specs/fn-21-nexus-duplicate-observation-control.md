...
flowchart LR
  ...
  A --> Q[Existing conformance checker]
  Q --> V[Qualified uniqueness violation]
```
...
- Existing direct and root commands accept the faulted immutable sets without new flags. The local execution command publishes an operational-success four-member set with status 0. The conformance command publishes a qualified semantic-violation six-member set with its existing status 2. The existing normal set continues to publish a satisfied six-member set with status 0.
...
- A synthetic marker without one completed real cancellation lifecycle receipt is a runtime invariant failure, not evidence. A real receipt without the selected marker/contribution is operationally inspectable but cannot become the intended qualified violation.
...
- The extra fact/field is charged against existing phase, source, record, and byte ceilings before append. N+1 records the existing explicit capacity omission/gap and cannot report operational success or silently drop the marker.
- Baseline and faulted runs use distinct immutable input/output identities and run identities. Republishing the exact already-constructed four- or six-member set and rechecking the same immutable four-member input are idempotent under fn-18/fn-20. Separate live executions use fresh run identities and may differ in timestamps, artifact bytes, manifests, and destinations; only their declared stable semantic and qualified-outcome identities must agree. Facts from separate runs cannot cross-correlate.
...
| Completed real lifecycle/callback present but marker, synthetic count, receipt link, or required causal/order discriminator absent | Fn-20 qualification `unknown`, semantic `incomplete`, conformance status 2. |
| Multiple distinct markers/contributions, callback count not one, synthetic count not one, or incompatible correlation/order facts | Fn-20 qualification `conflict`, semantic `incomplete`, conformance status 2. |
| Recognized required value available only under an unusable redact/hash disposition, or a recognized source-schema field cannot express the selected mapping declaration | Fn-20 qualification `unsupported`, semantic `incomplete`, conformance status 2. |
| Missing/duplicate/unexpected Property result, derivation/disposition bijection failure, or checker response identity/status drift | Fn-20 checker/output-invariant tooling error, conformance status 1, no SemanticEvidence/Result publication. |
| Exact request, one requested/completed lifecycle, callback count one, synthetic count one, one marker, complete correlation/closure/dispositions | Operational `succeeded`, qualification `qualified`, semantic `violated` for uniqueness alone; execution status 0 and conformance status 2. |
...
make umpire-check-local-conformance SET=/tmp/umpire-local-runs/caller-closure-duplicate-delivery OUTPUT_ROOT=/tmp/umpire-local-results
...
- **R5:** A checked fault-specific evidence profile/program/mapping is compiled before RuntimeConfiguration binding; the existing conformance controller/checker consumes it to derive semantic cancellation count two from callback count one plus synthetic-contribution count one. The unchanged pure caller-closure Property reports semantic `violated` with delivery/ownership satisfied and only its at-most-one cancellation clause responsible. Errors: every request/receipt/marker/count/mapping/configuration/causality/closure/disposition/Property-partition mutation yields exactly the tooling, unknown, conflict, unsupported, or incomplete result assigned by the mutation table and never a guessed violation.
- **R6:** Paired bounded live controls prove the existing no-fault path remains operationally succeeded, qualified, and semantically satisfied, while the faulted path is operationally succeeded, qualified, and semantically violated. Rechecking and republishing the same immutable constructed set are idempotent; separate live executions may differ in transport bytes/destinations but preserve declared stable semantic and qualified-outcome identities. Errors: status/exit mismatch, stable-identity drift, cross-run binding, non-deterministic semantic projection, partial publication, or a changed baseline byte fails the system proof.
- **R7:** Existing direct/root commands, focused and aggregate tests, developer documentation, and component status explain the negative-control lifecycle and the independence of requested attempt, realization receipt, operational status, qualification, and semantic verdict. Errors: a new CLI flag/authority surface, generic fault framework, retry/campaign control, second mapper/evaluator/artifact family, changed Property/production target semantics, model-local Makefile, CI/non-local qualification, replay/minimization/promotion, prohibited legacy dependency, or an unlabeled Temporal-defect claim is a verification failure.
- **R8:** The faulted participant program and evidence mapping are System-owned, and the qualified System trace reaches the unchanged Feature caller-closure Property only through the checked Nexus refinement. Errors: fault intent counted as realization, accepted transport counted as semantic observation, synthetic System evidence inserted into Feature declarations, missing/stale refinement, or a refinement non-success reported as a uniqueness violation fails the negative control.
...
- No replay, minimization, promotion, exploration, coverage scoring, formal checking, remote/existing-cluster/CI/canary execution, or release qualification.
...
- Flow spec fn-18 — exact RuntimeConfiguration, ExperimentRun, RawEvidence, Result, closure, bounds, admission, and immutable publication contracts.
...
- Flow spec fn-20 — exact Nexus evidence mapping, pure Property evaluation, qualified Result identity, and status/CLI contracts.
- Temporal Go SDK v1.44.0 workflow Nexus cancellation modes and Nexus Go SDK v0.6.0 idempotent asynchronous cancellation contract.
