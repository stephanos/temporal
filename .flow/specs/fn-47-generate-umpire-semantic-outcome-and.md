...
Each owning Lean stage exposes a typed ordered catalog of its own status values plus an exhaustiveness
proof. A small reusable `Umpire.SemanticInventory` module defines documentation descriptors and the
closed Known Gap lineage/scope vocabulary; it does not define a replacement status enum. A Temporal
leaf aggregator combines the stage catalogs and current gap sources into deterministic Markdown.
...
Known Gap inventory entries have a stable catalog ID, owning stage, lineage, scope, source shape, and
description. Lineage is exactly `authored`, `synthesized`, or `carried`; scope is exactly `production`
or `test-only`. Source shape is typed as an exact existing `KnownGap`, a namespaced generated family,
or a reference to the earlier catalog entry being carried. The existing `KnownGapKind` remains
unchanged.

## API Contracts

- Every stage-owned status catalog returns values of that stage's existing status type in canonical
  display order and carries a total proof that every constructor belongs to it.
...
- `cd model && mise exec -- lake exe temporal-model-semantic-inventory` emits exactly the complete
  Markdown bytes plus terminal LF on stdout; errors produce non-zero status, diagnostics on stderr,
  and no partial Markdown.
...
1. Define the small descriptor types and exhaustive Planning/runtime outcome catalogs next to their
   owners, with compile-time and executable tests.
...
- A new status constructor must make an owning catalog proof or test fail until classified; catalog
  completeness is not inferred from source text.
...
- Fixed planner gaps, request/raw-evidence gaps, Observation-derived gaps, Implementation Link authored
  gaps, Result carriers, and test fixtures retain their actual origin and propagation role.
- Unknown/dynamic request gap codes are represented by the carried source that admitted them, not by
  ...
  namespaced prefix.
- Rendering buffers and validates the complete document before stdout or replacement. Failed or
  interrupted generation preserves the prior checked document; check mode never writes.
...
Typed owner-local catalogs make additions fail near the source and avoid a second handwritten
vocabulary. One leaf aggregator is a deep, testable module: it knows how to validate and render all
inventory rows while callers see only a complete document. Rejected source scanning because syntax is
not semantic authority. Rejected one umbrella outcome enum because stage distinctions are part of the
...
- **R1:** Every current Planning, execution phase/control/source-closure/cleanup/operational,
  ...
  exactly once in its owning typed catalog with its current rendered name and an exhaustiveness proof.
  Errors: missing/duplicate constructors, duplicate family-local names, unclassified additions, or
  noncanonical ordering fail compilation or focused tests.
...
- **R3:** The Known Gap catalog records every fixed production gap, generated family, authored source,
  carry boundary, and test-only fixture with typed `authored|synthesized|carried` lineage and
  `production|test-only` scope, while actual fixed/synthesized producers reuse named declarations.
  Errors: duplicate IDs/codes, invalid namespaces, unresolved carries, wildcard invented semantics,
  missing scope/lineage, or a `KnownGapKind` expansion fail atomically.
- **R4:** `model/SEMANTIC_INVENTORY.md` is a deterministic complete projection with stable sections,
  canonical ordering, no timestamps or machine paths, exact source owners, and terminal LF. Errors:
  render/validation failure emits no partial stdout; interrupted generation leaves the prior file;
  missing/stale/extra document content makes the read-only check fail with a deterministic diff.
...
Task fn-47.1 proves owner-local typed catalogs can be exhaustive without introducing a shared status
type or changing existing rendered values. If that proof requires semantic duplication or invasive
type changes, reconsider the catalog interface before touching the remaining stages.
