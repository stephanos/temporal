...
- Replaying the candidate does not rerun the planner and does not accept an `ExperimentSpec`; it reconstructs only the exact `Scenario.Trace` needed by the checked Query.
...
- No `ExperimentSpec`, DrivePlan, runtime/evidence/result artifact, replay-minimization, promotion, or Claim Assessment schema change.
