...
The reusable module is `Umpire.Verify.Native`, not a generic `Umpire.Formal` authoring surface. Temporal declares model-owned verification profiles containing exact per-commit, nightly, and named checks with Limits, assumptions, trust, Known Gaps, and semantic bindings. The thin public command is `umpire-check-model`; the per-commit profile is default, and `list`/`explain` expose profiles and named checks without letting the CLI assemble or broaden them.

Canonical counterexample replay includes the referenced checked Target, Behavior, Implementation Link where applicable, and Property. Verification receipts use fn-18's versioned artifact envelope and keep kernel proof, reconstructed proof, trusted solver, bounded search, testing, and concrete replay as distinct trust classes.
...
This slice is deliberately independent of Veil. It provides the native receipt/replay authority that remains useful if fn-23 defers Veil and that a later optional family-owned Veil adapter must consume if fn-23 adopts it. It does not extend `Plan`, interpret semantics in Go, promote regressions, or touch runtime/production paths.
...
Model engineers should be able to run one opt-in root command against a statically registered checked target and save canonical receipt JSON. The receipt must say exactly what was checked, within which Limits, with which completeness evidence and trust class. A counterexample remains only a candidate until the same Lean process proves its setup, initial state, every ordered step, behavior, and violated Property against the canonical checked query.
...
flowchart LR
  ...
  N -->|complete, no violation| E[Established-within-Limits receipt]
...
```lean
nativeCheck : (query : CheckedQuery law) → SearchView query.target →
...
- `FormalClaim`: `established-within-Limits|violating-trace|null`.
...
- `ReplayStatus`: `matched|target-mismatch|query-mismatch|property-mismatch|kernel-mismatch|Limits-exceeded|setup-rejected|initial-rejected|step-rejected|behavior-rejected|property-satisfied|reason-mismatch|malformed`.

`CounterexampleCandidate` is exactly `{sourceChecker,targetIdentity,targetBehaviorFingerprint,kernelIdentity,kernelContractDigest,queryIdentity,queryBehaviorFingerprint,propertyIdentity,propertyBehaviorFingerprint,Limits,setup,trace,selectionReason,candidateIdentity}`. Setup bindings are sorted by role/value; the trace retains exact initial state and ordered `{selectedAction,outcome,state,facts}` steps. `candidateIdentity` is `umpire-counterexample-candidate/v2:` plus SHA-256 of all preceding fields in that order. No backend-private state, string log, Artifact Checksum, or `Plan` is accepted.

Canonical replay proceeds in a fixed fail-fast order: validate shape and recompute candidate identity; compare target, kernel, query, Property, and Limits Definition IDs/Behavior Fingerprints; require the Query form to be verify/counterexample and the selection reason to be `violatingCounterexample`; admit the sorted setup through `target.resolvedSetups`; admit the initial state through `target.kernel.initialStates setup`; enforce transition and selected-action typed Limits; for each ordered step require the exact `Step` in `target.kernel.steps currentState selectedAction`; require `query.behavior.admits` the reconstructed `Scenario.Trace`; evaluate the Query's exact single checked Property through `evaluateProperty`; and accept only `satisfied = false`. Kernel list membership carries the existing initial/step soundness proofs; no second transition relation is defined.

`ReplayResult` retains the candidate, exact replay status, canonical `PropertyEvaluation` including clause IDs/results/spans/Limits/provenance when evaluation was reached, and `replayIdentity`. `replayIdentity` is `umpire-counterexample-replay/v2:` plus SHA-256 over canonical `{candidateIdentity,request,status,propertyEvaluation}` where `request` is the receipt request preimage derived from the replaying Query, and `propertyEvaluation` is null before evaluation or the exact canonical evaluation afterward. Only a private matched constructor contains the admitted trace and false Property result required by `receiptOfNativeReplay`; changing a serialized status string cannot manufacture a violated receipt.

## Receipt Contract

The terminal envelope is `umpire-verification-receipt/v2` with exact field order `{formatVersion,request,checker,result,evidence,counterexample,diagnostics,Known Gaps,receiptIdentity}`.

- `request` is `{targetIdentity,targetBehaviorFingerprint,kernelIdentity,kernelContractDigest,queryIdentity,queryBehaviorFingerprint,propertyIdentity,propertyBehaviorFingerprint,view,Limits}`. `view` is exactly `{kind:"canonical-transition-kernel",identity,kernelContractDigest}` in this slice. Limits reuse canonical typed Query Limits.
- `checker` is `{kind,checkerIdentity,implementationIdentity,toolchain}`. Native values are `lean-native`, `umpire.checker.lean-native`, `umpire-lean-native-planner/v2`, and the exact trimmed `model/lean-toolchain` value.
...
- `counterexample` is null unless outcome is `violated`, otherwise `{candidateIdentity,trace,propertyEvaluation,replayStatus,replayIdentity}` with replay status necessarily `matched`. Property evaluation uses canonical clause order and exact typed spans/Limits/provenance.
- `diagnostics` is `{explored,instrumentation}` using the existing planner structures. It is inspectable but not claim-bearing.
- `Known Gaps` is a sorted unique closed list. Native values begin with `deployment-claim-assessment|external-checker|external-solver|liveness-beyond-Limits|promotion|runtime-evidence`; `search-incomplete` is added only for unknown budget exhaustion.

`receiptIdentity` is `umpire-verification-receipt/v2:` plus SHA-256 over canonical `{formatVersion,request,checker,result,evidence,counterexample,Known Gaps}`. Diagnostics are excluded so planner instrumentation may improve without changing the checked claim. The whole stdout JSON remains deterministic for the same implementation and inputs; source paths are repository-relative and all arrays have the stated canonical order. Every semantic/source/toolchain mutation changes or invalidates the identity.

## Native Outcome Mapping

| Checked input/result | Receipt outcome | Claim | Trust | Required evidence |
| --- | --- | --- | --- | --- |
| verify + `.verified` + completeness established | established | established-within-Limits | bounded-symbolic-search | complete finite domains + target/kernel/query/property |
| counterexample + `.noSuchTraceWithinCompleteBounds` + completeness established | established | established-within-Limits | bounded-symbolic-search | same |
...
| `.budgetExhausted` | unknown | null | bounded-symbolic-search | Limits + explored counts; `search-incomplete` Known Gap |
...
`Temporal.Feature.Nexus.CallerClosureFormal` is the first family adapter. It statically registers only `workflow-nexus.target.caller-closure` and calls `nativeCheck` with its existing `verifyQuery` plus the dependent specialization of `incrementalKernel`; the existing `verifyRun` remains comparison evidence, not caller-supplied lineage. The adapter binds the caller-closure Property, finite completeness evidence, target Behavior Fingerprint, and kernel contract digest into the generic native receipt path.

The family also defines one explicitly non-production negative-control Property and Query beside the owning feature: it requires the existing canonical `nexus.observation.pending-cancellation-count` after force-close to be at most zero. It reuses the unchanged caller-closure target, exact-action Behavior, exhaustive Limits, and incremental kernel. The existing canonical force-close trace therefore becomes a planner-selected violating counterexample that replay admits and the pure evaluator rejects. It is available to tests only, is absent from catalog/Generated View/inspector/public CLI registration, and never changes the production caller-closure Property or target.
...
- A verified planner value without `metadata.completeness.established`, exact Limits, and the Query's finite evidence digests is invalid, never established.
- Complete absence of a counterexample establishes only the exact Property within the recorded finite Limits. No unbounded liveness or global Temporal correctness wording appears in receipt fields or docs.
- Replaying the candidate does not rerun the planner and does not accept an `Plan`; it reconstructs only the exact `Scenario.Trace` needed by the checked Query.
...
- Empty traces are legal only when admitted by the Behavior and Property; the same setup/initial/Limits checks apply.
...
- Existing comments are preserved. No generated Lean, Go semantics, remote checker, Veil import/dependency, checker-neutral IR, receipt decoder/migration, output file writer, runtime/server path, promotion admission, or release Claim Assessment is introduced.
...
- **R6:** Focused canonical fixtures, outcome/replay/mutation/anti-forgery matrices, aggregate Lean builds, import-direction guards, before/after Plan fixture checks, developer/architecture/component documentation, and default-regression checks prove the seam is reusable and isolated. Errors: Temporal/Nexus vocabulary under `model/Umpire`, Plan/schema/fixture drift, hand-edited fn-5 glossary/catalog projections, generated source, Veil/dependency/runtime/promotion/release coupling, prohibited legacy dependency/use, or missing C11 roadmap status blocks completion.
- **R7:** `Umpire.Verify.Native` and Temporal-owned checked profiles expose exact per-commit, nightly, and named verification checks with Limits, assumptions, Known Gaps, provenance, Behavior Fingerprints, and distinct trust classes. Errors: a caller/CLI-assembled check list, broadened profile Limit, target/property/Implementation Link mismatch, collapsed trust class, or an unprofiled receipt cannot establish or violate a claim.
...
- No `Plan`, Plan.Steps, runtime/evidence/result artifact, replay-minimization, promotion, or Claim Assessment schema change.
