...
- **R7:** Authors can optionally declare checked model-owned Known Gaps for unsupported capabilities, inputs, interpretations, and claims; an explicit empty set represents no authored gaps, and every authored gap composes deterministically with phase-owned gaps through checked Queries, planning, portable lowering, Run Evaluation, artifacts, and results without affecting model behavior. Errors: malformed codes or subjects, duplicate rows, conflicting details for the same kind/code/subject, and noncanonical external order retain existing `KnownGapError` failures before runtime I/O; unknown wire categories remain a decoder error; a declared gap can never establish success or silently disappear, while truthful omission is an authoring responsibility rather than an inferable checker error.
