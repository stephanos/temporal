...
The commands always run. A pre-existing repository-wide integration or lint failure is acceptable
only when it matches the recorded pre-change failure set exactly and all task-scoped tests and lint
checks are clean; any new, changed, or unclassified failure blocks completion.
