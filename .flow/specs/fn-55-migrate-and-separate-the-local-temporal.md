...
The commands always run. A pre-existing repository-wide integration failure is acceptable only
when every failing identity belongs to the inherited Umpire2/Umpire3 suites and no Umpire4 or
unclassified test fails. A pre-existing lint failure is acceptable only when it matches the recorded
pre-change failure set exactly and all task-scoped tests and lint checks are clean.
...
- **R7:** Runtime, Run Evaluation, architecture, generator, Makefile, and CI documentation/commands describe the TestEnv-attached ownership model and execute the relocated integration tests. Focused unit, race, generator determinism, aggregate regression, and formatting gates pass. The complete TestEnv integration and lint commands always run. Integration failures are acceptable only for inherited Umpire2/Umpire3 identities; every Umpire4 or unclassified failure blocks completion. Lint failures are acceptable only when they match the exact recorded pre-change failure set and task-scoped lint has zero findings.
