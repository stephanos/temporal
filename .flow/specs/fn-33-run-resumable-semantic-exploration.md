...
Add the Go `campaign` deep module and the `umpire-fuzz` command around Lean-owned `Umpire.Exploration`. Lean owns candidate generation, mutation meaning, semantic coverage, corpus/priorities, and opaque state through `initialize`, `nextBatch`, and `observe`; Go leases and executes admitted batches concurrently through the shared runner/conformance path, publishes exact checkpoints, and resumes after bounded failures without acquiring semantic authority.
...
A checked Temporal-owned `RunnableExplorationBinding` is keyed to the existing fn-5 catalog identity and binds one checked Exploration identity/digest, the complete ExperimentSpec/RuntimeConfiguration input-set identity, environment profile identity/digest, allowed seed set, campaign time range, parallelism ceiling, protocol batch ceiling, pinned-set digest, and fixed runner/conformance identities. Fn-5 supplies the coherent catalog/list/explain projection; this binding adds operational admissibility without creating a second catalog. The first binding is `temporal.nexus.caller-closure.runtime-smoke`: exhaustive, selection budget one, seed zero, no faults or pins, the exact fn-19 caller-closure ExperimentSpec and ephemeral-local RuntimeConfiguration, parallelism exactly one, protocol batch size one, and campaign time 1 second through 5 minutes. Fake campaigns—not this one-point live proof—cover N-way leases and pinned-regression independence.

The fixed Lean sibling executable is `umpire-exploration-bridge`. It is resolved from the installed tool manifest/sibling directory, never from PATH or a flag. Each invocation exchanges exactly one request and one response: a four-byte unsigned big-endian length followed by canonical compact JSON, then EOF. Request and response are each at most 16 MiB; one batch contains at most eight ExperimentSpecs or Results, each already admitted under fn-18's 1 MiB member ceiling. Envelopes bind `umpire-exploration-bridge/v1`, operation `initialize|next-batch|observe`, invocation identity, runnable-binding identity/digest, model/checker/protocol identities and versions, prior state identity when applicable, and payload digest. A response is `ok` with canonical state/batch/report bindings or `rejected` with one typed Lean error; both exit 0. Malformed/trailing/oversized frames, handshake drift, timeout, empty response, nonzero exit, or stderr overflow are tooling failures and produce no state. Each call has a 30-second timeout; cancellation sends termination, waits two seconds, kills if needed, and always reaps. Stderr is capped at 64 KiB, sanitized, and never canonical.

`umpire-campaign-checkpoint/v1` is a distinct fn-18 artifact. It binds—without altering `umpire-coverage-checkpoint/v1`—campaign/runnable-binding identity, the coverage-checkpoint binding, environment input-set/profile binding, generation, optional parent campaign-checkpoint binding, sorted leases and attempts, accepted/rejected/stale result lineage, completed Run/Evidence/Result artifact-set bindings, campaign/protocol/model/checker versions, time/parallelism/batch/seed bounds, termination, omissions, and provenance. Fn-18 set closure gains explicit campaign-to-coverage, campaign-to-input-set, and campaign-to-result relationships plus strict cross-binding/mutation fixtures.
...
Progress is canonical NDJSON on stderr with schema `umpire-fuzz-progress/v1`, at most 4 KiB per line and one periodic event per second plus state-transition events. Events are `started|leased|result|checkpoint|stopping|cleanup|finished` and include campaign identity, generation, leased/active/completed/rejected/retry counts, remaining time, checkpoint identity when published, cleanup status, and final termination reason. Progress is bounded diagnostic output: it never enters semantic/campaign identities or deterministic stdout comparisons. Final stdout remains one canonical summary.
...
- Execute only through fn-19 runner/participant adapters and fn-20 conformance; preserve operational, semantic, cleanup, and tooling outcomes independently.
- Expose the revised CLI contract with exact list/explain/run/resume behavior and model-declared bounds.
...
- `campaign.Start` and `campaign.Resume` accept one checked `RunnableExplorationBinding`, its exact admitted input set/environment profile, explicit time/parallelism/state/seed inputs that only tighten the binding, one exclusively locked state root, and fixed runner/conformance adapters.
- The Lean protocol uses the exact framed `umpire-exploration-bridge/v1` envelopes and limits above: `initialize(binding, seed)`, `nextBatch(state)`, and `observe(state, results)` return canonical state plus admitted batches/results or one typed rejection.
...
- Every successful state transition publishes a new exact `umpire-campaign-checkpoint/v1` set with monotone parent/generation lineage and the full closure above. Resume derives one authoritative head by validation; it never uses or writes a mutable pointer.
...
- Time exhaustion stops new leases, cancels/collects workers within declared bounds, records outstanding/expired work, publishes a resumable campaign checkpoint when valid, emits `stopping|cleanup|finished` progress, and never claims completeness.
- Worker crash, canceled runner, cleanup failure, conformance non-success, duplicate delivery, checkpoint interruption, or process restart retains an exact phase outcome; none silently drops a candidate or credits coverage.
...
- Go never decodes or edits semantic coverage/corpus/priorities, proposes mutations, selects candidates, or infers omission/absence.
- Known regressions execute independently of the exploratory budget and remain a separate result partition.

## Acceptance Criteria
<!-- scope: both -->

- **R1:** The fixed sibling bridge implements the exact 4-byte-length/canonical-JSON `umpire-exploration-bridge/v1` protocol and process lifecycle for Lean `initialize`, `nextBatch`, and `observe`, preserving the full reproduction tuple. Errors: malformed/noncanonical/trailing/oversized exchange, executable/model/checker/protocol/strategy/bounds/seed mismatch, crossed state, unknown result, timeout/cancellation/reap failure, or non-monotone transition yields no new state.
- **R2:** The Go campaign module owns only leases, bounded parallel execution, opaque state transport, checkpoint publication, and resume; Lean remains sole owner of candidate selection, mutation meaning, coverage, corpus, priorities, and semantic termination. Errors: Go-side semantic scoring/mutation, implicit adapter selection, duplicate active identity, stale lease result, or unbound artifact cannot receive credit.
- **R3:** Campaign workers execute complete ExperimentSpecs only through the shared runner/conformance interfaces and return exact operational/evidence/refinement/property/cleanup outcomes to Lean. Errors: a private runtime/evaluator, missing cleanup/result, environment-specific semantic copy, crossed receipt, or tooling failure mislabeled as a semantic result fails completion.
- **R4:** Every accepted transition publishes one immutable `umpire-campaign-checkpoint/v1` binding the unchanged coverage checkpoint plus leases/attempts/results/environment/bounds/versions/parent-generation/provenance/omissions. An exclusive safe state-root lock and exact unique-head algorithm govern resume. Errors: partial/unsafe publication, symlink/root escape, competing process, tampered lease/state, lost result, missing parent, fork, generation gap, multiple heads, incompatible resume, or identity drift rejects before new work.
- **R5:** `umpire-fuzz` provides exact fn-5-backed list/explain and checked-binding-backed run/resume behavior, useful bounded progress, and may only tighten model-declared environment/time/parallelism/state/seed bounds. Errors: second registry, CLI-authored behavior/mutation/coverage, broadened bounds, arbitrary adapter/authority, unsafe/ambiguous state directory, silent long-running operation, or runtime completeness/absence claim fails closed.
- **R6:** Fake-worker, two-process lock, lease, cancellation, duplicate, crash, cleanup, checkpoint-fork, progress, and resume matrices prove deterministic selection lineage, no active duplication, resumable time exhaustion, and pinned-regression independence. One bounded local run of the exact one-point caller-closure runtime-smoke binding proves the runner/conformance vertical path; it is not the parallel/pinned proof. Errors: a surviving mutation, wrong-layer diagnosis, nondeterministic non-duration output, lost comment, or Umpire3 reuse fails verification.
...
- No new Property, Behavior, Space, Query, target, planner, coverage, mutation, or corpus semantics in Go.
- No completeness claim from runtime fuzzing and no replacement for model-only exhaustive verification.
- No replay, minimization, promotion, qualification-policy, canary, or release behavior.
...
- Fn-5 — authoritative catalog/list/explain projection used by runnable binding discovery.
...
- Fn-19 and fn-20 — shared runner, raw evidence, refinement, and conformance Result.
...
| R3 | Shared runner/conformance execution | `.3`, `.5`, `.6` | — |
