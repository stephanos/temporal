...
Complete the first current-model semantic loop without re-executing Temporal. The command consumes the exact fn-19-admitted normal four-member local set, projects its already-bounded Run and RawEvidence through a closed Go-to-Lean checker bridge, applies fn-4's checked Observation mapping and unchanged pure Property evaluator, and publishes one fn-18-admitted six-member set containing Evidence and Result. The downstream fn-21 negative control extends this same boundary with exactly one distinct compiled duplicate-delivery closure; it does not widen the normal closure or add generic profile selection.
...
The primary user is an Umpire model/runtime engineer asking whether the exact local Nexus caller-closure run supports the properties embedded in its Plan. The user needs one deterministic command that distinguishes a successful request from accepted semantic evidence, preserves why a verdict was reached, and fails closed when evidence is missing, ambiguous, contradictory, unrelated, partial, or unusably redacted.
...
The Go controller is the only persisted-byte reader and publisher. It requires exactly Plan, RuntimeConfiguration, ExperimentRun, and RawEvidence; derives operational status through the fn-19 precedence authority; and never interprets a fact, constructs a semantic trace, or evaluates a Property.
...
One Temporal Tool checker resolves only the compiled normal Nexus caller-closure closure and the exact duplicate-delivery closure added by fn-21, including each closure's experiment, observation program, mapping, query, properties, evidence profile, and source schemas. It converts the direct request preimage to fn-4's typed EvidenceBundle, preserving source-local order, causal order, closure, gaps, and field dispositions. Crossed or unknown compiled identities are unsupported; incompatible facts are conflict; missing closure/order/discriminators are unknown. It never guesses, applies first-match semantics, or admits a third profile.
...
- `Check(admittedSet)` is the only exported production entry point. It accepts only the exact four-member normal fn-19 set or the exact four-member duplicate-delivery set added by fn-21, resolves the verified sibling checker internally, and returns an admitted six-member set in memory or one structured tooling error. The two closures remain identity-distinct and crossed combinations reject. The checker injection seam is package-private and test-only. Checking is pure with respect to Temporal and never publishes.
...
- The command rejects a six-member descendant as input, an orphan artifact, an extra member, mixed runs/configurations, semantic-reference drift, or any profile/program/source family outside the exact normal fn-19 and duplicate-delivery fn-21 closures before checking.
...
- Every Plan Property is resolved and represented exactly once in canonical order. Missing, duplicate, unexpected, or divergent verdicts make the response invalid or the strict summary incomplete according to fn-4/fn-18; no subset can report success.
...
- **R1:** The Run Evaluation controller accepts only an fn-18-admitted, exact four-member normal fn-19 local caller-closure set or the exact four-member duplicate-delivery set added by fn-21; preserves all four input members byte-for-byte; and performs no Temporal, participant, network, or publication IO before complete input admission. The normal and faulted closures remain identity-distinct, and no third or generic profile is admitted. Errors: malformed, missing, extra, duplicate, orphaned, cross-run, cross-configuration, crossed closure, unresolved-reference, wrong-profile/program/source, unsafe-path, or oversized input returns one structured input error, starts no checker, and produces no semantic artifacts.
- **R2:** One closed, 30-second, 32-MiB-per-direction private checker bridge binds the exact non-path Plan/RuntimeConfiguration/ExperimentRun/RawEvidence artifact tuples plus checker identity and carries only direct admitted projections to the fixed sibling Lean checker. The exported production API resolves that checker internally and exposes no substitution seam. Errors: missing/non-regular/misdirected checker; handshake/version/digest mismatch; spawn/cancel/timeout/nonzero exit; malformed/noncanonical/unknown/duplicate/oversized output; stderr leakage; trailing bytes; or echoed artifact/semantic binding drift returns a checker error, kills/reaps the child, and exposes no partial Evidence or Result. R2 protects against accidental or tool-controlled pathname substitution: Linux executes the descriptor-bound verified snapshot, while Darwin uses a private mode-0700 sibling pair plus embedded digest and holds `UF_IMMUTABLE` on the exact open vnode through child wait. A concurrent same-UID actor able to clear vnode flags, mutate or ptrace the launcher, or otherwise compromise the process is outside this threat boundary.
- **R3:** The compiled Temporal Nexus adapter resolves exactly the normal caller-closure mapping/profile/query/Property/source-schema closure or the identity-distinct duplicate-delivery closure added by fn-21 and converts each closure's four sources into fn-4's bounded typed EvidenceBundle without changing reusable Umpire types or adding a second evaluator. Errors: crossed or unknown compiled identity/schema/kind/field, missing source, invalid disposition, unmatched correlation, causal orphan/cycle, or unsupported declaration fails closed as the exact unsupported/unknown/conflict outcome; source or fact order never selects an interpretation.
...
- **R8:** Independent corruption, ambiguity, contradiction, disposition, bound, status, protocol, and binding fixtures plus one bounded fn-19→fn-20 local Nexus run prove fail-closed Run Evaluation and documentation states the exact command and limits. A deliberately skewed wall-clock fixture proves that treating timestamps as semantic order changes the Observation Evaluation or Property Result, while the unchanged causal/source-local order produces the expected Evidence Link and Result. Errors: a mutation surviving, a wrong-boundary diagnosis, shared implementation/oracle logic, non-deterministic output, unclosed source/cleanup omitted from the Result, timestamp authority entering production semantics, or any claim of replay, promotion, remote/CI/canary/release Observation Evaluation fails verification.
...
- No scenario, profile, program, mapping, query, or Property beyond the exact compiled normal caller-closure closure and the identity-distinct duplicate-delivery closure added by fn-21; no generic selector or third closure.
...
The command accepts only either exact four-member run set: the original normal closure or fn-21's identity-distinct duplicate-delivery closure. Rechecking a derived set, crossing the two closures, or selecting a subset would make provenance and property coverage ambiguous; deterministic idempotent publication already supplies the safe retry path.
...
| --- | --- | --- | --- |
| R1 | Strict four-member input and offline boundary | `.3`, `.4`, `.6`; fn-21 `.5` | — |
| R2 | Closed bounded Go/Lean checker bridge | `.2`, `.3`, `.5`, `.6` | — |
| R3 | Exact Nexus live-evidence adapter | `.2`, `.5`, `.7`; fn-21 `.5` | — |
