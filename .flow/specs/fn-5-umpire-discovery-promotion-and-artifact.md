...
Fn-15 owns the reusable query mechanics because API, config, and semantic catalogs need the same exact command behavior. This spec owns only the semantic adapter and retains its independently checked `Umpire.Catalog` graph.
