...
- **R5:** Focused compatibility tests, aggregate model builds, exact regressions, trust/import checks, documentation review, and lint prove the migration is semantic-only. The literal `make lint-code` gate MUST run. Its pre-edit branch-wide baseline of 1,373 golangci findings and the existing `tools/umpire/runtime/errors.go:60:9` `errortype` finding are classified as inherited only when the final run preserves that debt and reports no task Go path; the task MUST also pass a no-fix golangci run scoped from its base commit. Commits `428fc87d9` and `69ce0485f` are separately user-authorized roadmap maintenance interleaved during execution and are excluded from fn50 implementation and evidence. Errors: lost comment, stale direct-kernel claim, new axiom/trust dependency, warning, a new or changed lint finding, a task-path lint finding, a failing scoped no-fix lint run, generated drift, an fn50-authored roadmap change, or any other fn50-authored unrelated file change blocks completion.
