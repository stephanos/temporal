...
2026-09-10: the `case` block this spec was to re-anchor on is replaced by fn-85 (entities, actions with parties, observations, machines, query sets, one Case per Query). On resume, re-plan on fn-85 instead: the cancel actions and rows of `model/Temporal/Feature/Nexus/DESIGN.md` section 4, a functional Query for async reply, cancel request, delivered cancel reply and canceled callback translated from `TestNexusOperationCancelation`, a workflow instruction that requests cancellation of a started Nexus operation, a handler cancel activation with its reply instruction, and a canceled arm on fn-85's typed handler-reply and completion instructions (2026-09-12: fn-85 R10 replaces the untyped Nexus reply shape and the untyped completion result with instructions carrying the Temporal API messages; fn-86 .3 removed the old shapes). fn-85 deliberately leaves all of these out.

2026-09-20 (fn-86 .5): the cancellation-race prototype `Temporal.Feature.Nexus.Race` is deleted; its behavior is recorded here so the resumed work has it. The race Model (`temporal.nexus.race.cancellation-race.*`) starts from an already-started operation: `requestCancel` moves `started` to the nonterminal `cancelRequested` (outcome `cancellation-requested`, fact `cancel-requested`); `resolve` out of `cancelRequested` has two Target-owned alternatives, `canceled` (outcome `canceled`, facts `canceled` and `terminal`) and `succeeded` (outcome `succeeded`, facts `succeeded` and `terminal`); neither action chooses the terminal outcome and no row completes before the request is confirmed or repeats a request. Its terminal slice (`Race.Terminal`) declared `canceled` and `succeeded` as the terminal condition and fed the Implementation Link's cancellation projection until fn-86 .4 moved that closure onto the caller Model's `ends:`. Its three Properties: a cancellation request receives a terminal model response within one further transition (`eventuallyWithin`, one step); the resolution may select the canceled outcome; the resolution may select the succeeded outcome. Its Scenarios: exactly request then resolve (the witness Query, `found` with both resolutions exercised and cancellation-always-wins refuted), request only (the exercised violation, distinct from the untriggered vacuous pass of a no-trigger Scenario), and a contradictory setup (`unsatisfiable`, a planner status and not an error); budget exhaustion stayed `limit-reached`, distinct from exhaustive absence. Its baseline (`temporal.nexus.race.basic-lifecycle.*`: `scheduled` to `started` by `start`, `started` to `canceled` by `cancel`, `started` to `succeeded` by `succeed`, each a Property, Scenario and Query) matched the first-generation lifecycle's transition relation, initial states and bounded traces. On the caller Model this is: a `cancel` action of the workflow party with a row out of `started` into a cancel-requested phase, a canceled arm of the handler reply and of the completion, the completion rows out of that phase into `canceled` and `succeeded` both kept, and Properties over those rows in the forms above; the prototype's constructor-versus-frontend authoring comparison and its measurements are superseded by the commands and are not carried forward.
