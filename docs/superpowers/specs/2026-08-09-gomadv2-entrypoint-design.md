...
installed entirely from `tools/gomadv2`.

## Package layout

Add `tools/gomadv2/cmd/gomad/main.go` as `package main`. It will delegate to
...
Repository-root commands will use `go -C tools/gomadv2`; `tools/gomadv2/test.sh`
...
- build `./cmd/gomad` from `tools/gomadv2` with `-tags test_dep`;
