...
```text
tools/gomadv3/proto/gomadv3/ipc/v1/message.proto
```

Generated Go types live in the standalone Gomad module:

```text
tools/gomadv3/internal/ipcpb/v1/message.pb.go
```
...
`tools/gomadv3/internal/ipc` is a small, deep framing module shared by the
...
The root Makefile gains a `gomadv3-proto` target using Temporal's pinned
...
Because `tools/gomadv3` builds with `GOWORK=off`, its own `go.mod` and `go.sum`
