...
// Marshal an object of type AbstractionClaim to the protobuf v3 wire format
func (val *AbstractionClaim) Marshal() ([]byte, error) {
	return proto.Marshal(val)
}

// Unmarshal an object of type AbstractionClaim from the protobuf v3 wire format
func (val *AbstractionClaim) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, val)
}

// Size returns the size of the object, in bytes, once serialized
func (val *AbstractionClaim) Size() int {
	return proto.Size(val)
}

// Equal returns whether two AbstractionClaim values are equivalent by recursively
// comparing the message's fields.
// For more information see the documentation for
// https://pkg.go.dev/google.golang.org/protobuf/proto#Equal
func (this *AbstractionClaim) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	var that1 *AbstractionClaim
	switch t := that.(type) {
	case *AbstractionClaim:
		that1 = t
	case AbstractionClaim:
		that1 = &t
	default:
